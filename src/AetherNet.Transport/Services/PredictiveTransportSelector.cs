...
///
/// <para>
/// Extends the EWMA-only transport selection in <see cref="AetherNet.Transport.Services.TransportManager"/>
...
public sealed class PredictiveTransportSelector
{
    ...
    ///   <item><description><see cref="IAetherNetAiProvider.IsAvailable"/> is <c>false</c>.</description></item>
    ...
    ///
    /// <para>
    /// Bias semantics match <see cref="IAetherNetAiProvider.GetTransportBiasesAsync"/>:
    ...
    public async Task<IReadOnlyList<(ITransportService Transport, double Score, double PredictedRttMs, double RttVariance)>>
        RankWithAiAsync(
            ...
            IAetherNetAiProvider? aiProvider = null,
