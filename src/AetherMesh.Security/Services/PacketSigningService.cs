...
public sealed class PacketSigningService : IPacketSigningService, IDisposable
{
    ...
    public async Task<MeshPacket> SignPacketAsync(MeshPacket packet, CancellationToken ct = default)
    {
        ...
        using var activity = AetherMeshTelemetry.ActivitySource.StartActivity("AetherMesh.Sign.Packet");
        ...
        try
        {
            ...
            if (activity is not null)
                activity.SetTag("aethermesh.packet.type", (int)packet.Type);
        ...
        {
            AetherMeshTelemetry.SignVerifyLatency.Record(stopwatch.GetElapsedMilliseconds());
        }
    }
    ...
    public Task<bool> VerifyPacketAsync(MeshPacket packet, byte[] senderPublicKey, CancellationToken ct = default)
    {
        ...
        using var activity = AetherMeshTelemetry.ActivitySource.StartActivity("AetherMesh.Verify.Packet");
        var stopwatch = ValueStopwatch.StartNew();
        try
        {
            if (activity is not null)
            ...
            {
                AetherMeshTelemetry.StaleTimestampsRejected.Add(1);
                if (activity is not null) activity.SetTag("aethermesh.packet.valid", false);
            ...
            {
                AetherMeshTelemetry.NoncesReplayed.Add(1);
                if (activity is not null) activity.SetTag("aethermesh.packet.valid", false);
            ...
            if (valid)
            {
                AetherMeshTelemetry.SignaturesValidated.Add(1);
            }
            else
            {
                AetherMeshTelemetry.SignaturesRejected.Add(1);
            ...
            if (activity is not null)
                activity.SetTag("aethermesh.packet.valid", valid);
