...
public sealed class SignalProtocolService : ISignalProtocolService
{
    ...
    /// in the InternalsVisibleTo allow-list (AetherMesh.Storage,
    /// AetherMesh.Core.Tests) can reference it directly.
    ///
    ...
    public Task<EncryptedPayload> EncryptAsync(string peerUhid, byte[] plaintext, CancellationToken ct = default)
    {
        ...
        using var activity = AetherMeshTelemetry.ActivitySource.StartActivity("AetherMesh.Encrypt");
        ...
        try
        {
            ...
            try
            {
                ...
                if (session.PendingPreKeyMessage)
                {
                    ...
                    {
                        activity.SetTag("aethermesh.peer.uhid", LogSanitizer.SanitizeUhid(peerUhid));
                        activity.SetTag("aethermesh.message.type", 1);
                        activity.SetTag("aethermesh.message.counter", counter);
                    }
                    AetherMeshTelemetry.MessagesEncrypted.Add(1);
                ...
                {
                    activity.SetTag("aethermesh.peer.uhid", LogSanitizer.SanitizeUhid(peerUhid));
                    activity.SetTag("aethermesh.message.type", 0);
                    activity.SetTag("aethermesh.message.counter", counter);
                }
                AetherMeshTelemetry.MessagesEncrypted.Add(1);
        ...
        {
            AetherMeshTelemetry.EncryptLatency.Record(stopwatch.GetElapsedMilliseconds());
        }
    }
    ...
    public Task<byte[]> DecryptAsync(string peerUhid, EncryptedPayload payload, CancellationToken ct = default)
    {
        ...
        using var activity = AetherMeshTelemetry.ActivitySource.StartActivity("AetherMesh.Decrypt");
        var stopwatch = ValueStopwatch.StartNew();
        try
        {
                ...
                || !ConstantTimeEquals(senderRatchetPub, session.RemoteEphemeralPub))
            {
                ...
                using var ratchetActivity = AetherMeshTelemetry.ActivitySource.StartActivity("AetherMesh.DhRatchet.Step");
                if (ratchetActivity is not null)
                    ratchetActivity.SetTag("aethermesh.peer.uhid", LogSanitizer.SanitizeUhid(peerUhid));
                AetherMeshTelemetry.DhRatchetSteps.Add(1);
            }
            ...
            try
            {
                ...
                {
                    activity.SetTag("aethermesh.peer.uhid", LogSanitizer.SanitizeUhid(peerUhid));
                    activity.SetTag("aethermesh.message.type", payload.MessageType);
                    activity.SetTag("aethermesh.message.counter", payload.Counter);
                }
                AetherMeshTelemetry.MessagesDecrypted.Add(1);
        ...
        {
            AetherMeshTelemetry.DecryptLatency.Record(stopwatch.GetElapsedMilliseconds());
        }
    }
    ...
    public Task ProcessPreKeyBundleAsync(PreKeyBundle bundle, CancellationToken ct = default)
    {
        ...
        using var activity = AetherMeshTelemetry.ActivitySource.StartActivity("AetherMesh.X3DH.Initiator");
        if (activity is not null)
            activity.SetTag("aethermesh.peer.uhid", LogSanitizer.SanitizeUhid(bundle.Uhid));
        ...
        try
        {
            ...
            AetherMeshTelemetry.SessionsEstablished.Add(1);
    ...
    {
        using var activity = AetherMeshTelemetry.ActivitySource.StartActivity("AetherMesh.X3DH.Responder");
        if (activity is not null)
            activity.SetTag("aethermesh.peer.uhid", LogSanitizer.SanitizeUhid(peerUhid));
