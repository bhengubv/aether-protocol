...
public sealed class SignalProtocolService : ISignalProtocolService
{
    ...
    public bool DropSession(string peerUhid)
    {
        ...
        // start and the node is stuck with it again.
        if (removed && _sessionStore is not null)
        {
            try { _sessionStore.DeleteAsync(peerUhid).GetAwaiter().GetResult(); }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Failed to delete the stored session for {Peer}.",
                    LogSanitizer.SanitizeUhid(peerUhid));
            }
        }
