...
public sealed class RoutingService : IRoutingService
{
    ...
    private readonly IAetherMeshTelemetry? _telemetry;
    ...
    public RoutingService(
        ...
        IAetherMeshTelemetry? telemetry = null)
    {
    ...
    public async Task<RouteEntry?> FindRouteAsync(string destinationUhid, CancellationToken cancellationToken = default)
    {
        ...
        using var activity = AetherMeshTelemetry.ActivitySource.StartActivity("AetherMesh.Route.Lookup");
        if (activity is not null)
            activity.SetTag("aethermesh.destination.uhid", AetherMeshTelemetry.SanitizeUhid(destinationUhid));
        ...
        try
        {
            ...
            {
                AetherMeshTelemetry.RouteCacheHits.Add(1);
                if (activity is not null)
                {
                    activity.SetTag("aethermesh.route.source", "cache");
                    activity.SetTag("aethermesh.route.hops", cached.HopCount);
                }
                return cached;
            }
            ...
            if (stored is not null && !stored.IsExpired)
            {
                ...
                if (activity is not null)
                {
                    activity.SetTag("aethermesh.route.source", "store");
                    activity.SetTag("aethermesh.route.hops", stored.HopCount);
                }
                return stored;
            }
            ...
            if (activity is not null)
            {
                activity.SetTag("aethermesh.route.source", "discovery");
                if (discovered is not null)
                    activity.SetTag("aethermesh.route.hops", discovered.HopCount);
            }
            return discovered;
        }
        finally
        {
            AetherMeshTelemetry.RouteLookupLatency.Record(stopwatch.GetElapsedMilliseconds());
        }
    }
    ...
    private async Task<RouteEntry?> DiscoverAsync(string destinationUhid, CancellationToken cancellationToken)
    {
        ...
        AetherMeshTelemetry.RouteRequestsEmitted.Add(1);
        ...
        catch (OperationCanceledException)
        {
            ...
            _telemetry?.Publish(new AetherMeshRouteEvent(
                ...
                Path:              [],
                Kind:              AetherMeshRouteEventKind.Failed,
