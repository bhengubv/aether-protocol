...
//    hunt-auth-bypass       → AetherMeshVulnerabilityClass.AuthBypass
//    hunt-race-condition    → AetherMeshVulnerabilityClass.RaceCondition
//    hunt-idor              → AetherMeshVulnerabilityClass.InformationDisclosure
//    hunt-rce               → AetherMeshVulnerabilityClass.RemoteCodeExecution
//    hunt-business-logic    → AetherMeshVulnerabilityClass.BusinessLogic (Sybil, free-rider)
//    hunt-api-misconfig     → AetherMeshVulnerabilityClass.ProtocolMisconfiguration
//    hunt-ssrf              → AetherMeshVulnerabilityClass.RelayAbuse
//    hunt-dos               → AetherMeshVulnerabilityClass.DenialOfService
//    supply-chain-attack-recon → AetherMeshVulnerabilityClass.SupplyChain
// ─────────────────────────────────────────────────────────────────────────────
...
/// </summary>
public enum AetherMeshVulnerabilityClass : byte
{
    /// <summary>
...
/// <summary>
/// A single security finding produced by <see cref="IAetherMeshSecurityAudit"/>.
/// </summary>
...
public sealed record AetherMeshAuditFinding(
    string                   Id,
    AetherMeshVulnerabilityClass VulnerabilityClass,
...
/// taxonomy (<see cref="AetherMeshVulnerabilityClass"/>). Three intended callers:
/// </para>
/// <list type="number">
...
/// <see cref="NullAetherMeshSecurityAudit"/> is used — all audits return empty finding lists
...
/// </para>
/// </summary>
public interface IAetherMeshSecurityAudit
{
    /// <summary>
    ...
    Task<IReadOnlyList<AetherMeshAuditFinding>> AuditPacketsAsync(
        IReadOnlyList<MeshPacket> packets,
        CancellationToken         cancellationToken = default)
        => Task.FromResult<IReadOnlyList<AetherMeshAuditFinding>>([]);
    ...
    /// <param name="cancellationToken">Cancellation token.</param>
    Task<IReadOnlyList<AetherMeshAuditFinding>> AuditNodeAsync(
        string            nodeId,
        CancellationToken cancellationToken = default)
        => Task.FromResult<IReadOnlyList<AetherMeshAuditFinding>>([]);
    ...
    /// </summary>
    /// <param name="cancellationToken">Cancellation token.</param>
    Task<IReadOnlyList<AetherMeshAuditFinding>> AuditProtocolConfigAsync(
        CancellationToken cancellationToken = default)
        => Task.FromResult<IReadOnlyList<AetherMeshAuditFinding>>([]);
}

/// <summary>
/// No-op <see cref="IAetherMeshSecurityAudit"/> — returns empty findings for all audits.
/// </summary>
public sealed class NullAetherMeshSecurityAudit : IAetherMeshSecurityAudit
{
    /// <summary>The singleton instance.</summary>
    public static readonly NullAetherMeshSecurityAudit Instance = new();

    private NullAetherMeshSecurityAudit() { }
}
