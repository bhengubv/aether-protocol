...
public sealed record AetherMeshMemoryEntry(
    string         Id,
    string?        NodeId,
    string         Category,
    string         Content,
...
/// <see cref="IAetherMeshAiProvider"/> implementations to persist mesh observations
...
/// <see cref="NullAetherMeshContextMemory"/> is used and no context persists between
/// sessions — the AI provider starts fresh each time.
/// </para>
/// </summary>
public interface IAetherMeshContextMemory
{
    /// <summary>
    /// Stores an observation. Implementations should deduplicate by
    /// <see cref="AetherMeshMemoryEntry.Id"/> and update existing entries if the same
    /// observation arrives again (e.g. a trust score update for the same node).
    /// </summary>
    Task StoreAsync(AetherMeshMemoryEntry entry, CancellationToken cancellationToken = default);

    /// <summary>
    /// Retrieves recent entries for a specific node, ordered by
    /// <see cref="AetherMeshMemoryEntry.RecordedAt"/> descending (newest first).
    ...
    Task<IReadOnlyList<AetherMeshMemoryEntry>> RecallAsync(
        string            nodeId,
        int               limit              = 20,
    ...
    /// ordered by <see cref="AetherMeshMemoryEntry.Importance"/> descending, then by
    /// <see cref="AetherMeshMemoryEntry.RecordedAt"/> descending.
    /// </summary>
    ...
    /// <param name="cancellationToken">Cancellation token.</param>
    Task<IReadOnlyList<AetherMeshMemoryEntry>> RecallByCategoryAsync(
        string            category,
        int               limit              = 50,
        CancellationToken cancellationToken  = default);
...
/// <summary>
/// No-op <see cref="IAetherMeshContextMemory"/> — used when no memory backend is
/// registered. All stores are silently discarded; recalls return empty lists.
/// </summary>
public sealed class NullAetherMeshContextMemory : IAetherMeshContextMemory
{
    /// <summary>The singleton instance.</summary>
    public static readonly NullAetherMeshContextMemory Instance = new();

    private NullAetherMeshContextMemory() { }

    /// <inheritdoc/>
    public Task StoreAsync(AetherMeshMemoryEntry entry, CancellationToken cancellationToken = default)
        => Task.CompletedTask;

    /// <inheritdoc/>
    public Task<IReadOnlyList<AetherMeshMemoryEntry>> RecallAsync(
        string nodeId, int limit = 20, CancellationToken cancellationToken = default)
        => Task.FromResult<IReadOnlyList<AetherMeshMemoryEntry>>([]);

    /// <inheritdoc/>
    public Task<IReadOnlyList<AetherMeshMemoryEntry>> RecallByCategoryAsync(
