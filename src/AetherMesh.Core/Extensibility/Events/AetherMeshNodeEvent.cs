...
public enum AetherMeshNodeEventKind : byte
{
...
public sealed record AetherMeshNodeHealth(
    double    TrustScore,
    bool      IsReachable,
    TimeSpan  Latency,
    int       HopCount)
...
/// Consumed by <see cref="IAetherMeshTelemetry"/> subscribers — the AI layer
...
public sealed record AetherMeshNodeEvent(
    string            NodeId,
    AetherMeshNodeEventKind Kind,
    AetherMeshNodeHealth  Health,
    DateTimeOffset    OccurredAt)
{
    /// <summary>Convenience: <c>true</c> when this is a departure event.</summary>
    public bool IsExit => Kind is AetherMeshNodeEventKind.Left;
}
