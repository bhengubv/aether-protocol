...
public sealed class DtnService : IDtnService
{
    ...
    public async Task<int> ExpireStaleAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (expired > 0)
            AetherNetTelemetry.DtnBundlesExpired.Add(expired);
        return expired;
    }
    ...
    private async Task HandleBundleAsync(MeshPacket packet, CancellationToken cancellationToken)
    {
        ...
        if (string.Equals(bundle.RecipientUhid, _sender.LocalUhid, StringComparison.Ordinal))
        {
            bundle.Status = BundleStatus.Delivered;
            await _store.SaveAsync(bundle, cancellationToken).ConfigureAwait(false);
            AetherNetTelemetry.DtnBundlesDelivered.Add(1);
            ...
            // Fire BundleReceived BEFORE the delivery receipt so subscribers see the
            // inbound bundle in the same event-loop turn as it lands locally.
            BundleReceived?.Invoke(this, new DtnBundleReceivedEventArgs
            {
                BundleId = bundle.Id,
        ...
        using var custodyActivity = AetherNetTelemetry.ActivitySource.StartActivity("AetherNet.Dtn.Custody");
        if (custodyActivity is not null)
        {
            custodyActivity.SetTag("aethernet.bundle.id", bundle.Id);
            custodyActivity.SetTag("aethernet.recipient.uhid", AetherNetTelemetry.SanitizeUhid(bundle.RecipientUhid));
        }
        ...
        AetherNetTelemetry.DtnBundlesAccepted.Add(1);
