...
public enum PacketType : byte
{
    ...
    /// registered the <c>aethernet.space/v1</c> capability process this type;
    ...
    /// to announce it to mesh peers with the <c>aethernet.forge/v1</c> capability.
    /// </summary>
    ForgeAnnounce = 41,
    ...
    /// <summary>Native circuit-relay-v2 control/data frame (RESERVE/CONNECT/STOP/DATA + responses).
    /// Payload is a serialized <c>RelayFrame</c> (see <c>AetherNet.CircuitRelay.RelayFrameSerializer</c>),
    /// carried exactly one hop between a client / relay / target. The relay transport
    /// (<c>CircuitRelayTransportService</c>, an <c>ITransportService</c>) processes these via its
    /// <c>IRelayLink</c>; only the final DATA delivery to the destination surfaces as transport
    /// <c>DataReceived</c>. Lets a node with no direct path to a peer reach it through a third node
    /// that can reach both — the decentralised, no-libp2p relay.</summary>
