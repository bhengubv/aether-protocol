...
public enum AetherNetNodeEventKind : byte
{
...
public sealed record AetherNetNodeHealth(
    double    TrustScore,
    bool      IsReachable,
    TimeSpan  Latency,
    int       HopCount)
...
/// Consumed by <see cref="IAetherNetTelemetry"/> subscribers — the AI layer
...
public sealed record AetherNetNodeEvent(
    string            NodeId,
    AetherNetNodeEventKind Kind,
    AetherNetNodeHealth  Health,
    DateTimeOffset    OccurredAt)
{
    /// <summary>Convenience: <c>true</c> when this is a departure event.</summary>
    public bool IsExit => Kind is AetherNetNodeEventKind.Left;
}
