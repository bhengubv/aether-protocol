...
public sealed record AetherNetMemoryEntry(
    string         Id,
    string?        NodeId,
    string         Category,
    string         Content,
...
/// <see cref="IAetherNetAiProvider"/> implementations to persist mesh observations
...
/// <see cref="NullAetherNetContextMemory"/> is used and no context persists between
/// sessions — the AI provider starts fresh each time.
/// </para>
/// </summary>
public interface IAetherNetContextMemory
{
    /// <summary>
    /// Stores an observation. Implementations should deduplicate by
    /// <see cref="AetherNetMemoryEntry.Id"/> and update existing entries if the same
    /// observation arrives again (e.g. a trust score update for the same node).
    /// </summary>
    Task StoreAsync(AetherNetMemoryEntry entry, CancellationToken cancellationToken = default);

    /// <summary>
    /// Retrieves recent entries for a specific node, ordered by
    /// <see cref="AetherNetMemoryEntry.RecordedAt"/> descending (newest first).
    ...
    Task<IReadOnlyList<AetherNetMemoryEntry>> RecallAsync(
        string            nodeId,
        int               limit              = 20,
    ...
    /// ordered by <see cref="AetherNetMemoryEntry.Importance"/> descending, then by
    /// <see cref="AetherNetMemoryEntry.RecordedAt"/> descending.
    /// </summary>
    ...
    /// <param name="cancellationToken">Cancellation token.</param>
    Task<IReadOnlyList<AetherNetMemoryEntry>> RecallByCategoryAsync(
        string            category,
        int               limit              = 50,
        CancellationToken cancellationToken  = default);
...
/// <summary>
/// No-op <see cref="IAetherNetContextMemory"/> — used when no memory backend is
/// registered. All stores are silently discarded; recalls return empty lists.
/// </summary>
public sealed class NullAetherNetContextMemory : IAetherNetContextMemory
{
    /// <summary>The singleton instance.</summary>
    public static readonly NullAetherNetContextMemory Instance = new();

    private NullAetherNetContextMemory() { }

    /// <inheritdoc/>
    public Task StoreAsync(AetherNetMemoryEntry entry, CancellationToken cancellationToken = default)
        => Task.CompletedTask;

    /// <inheritdoc/>
    public Task<IReadOnlyList<AetherNetMemoryEntry>> RecallAsync(
        string nodeId, int limit = 20, CancellationToken cancellationToken = default)
        => Task.FromResult<IReadOnlyList<AetherNetMemoryEntry>>([]);

    /// <inheritdoc/>
    public Task<IReadOnlyList<AetherNetMemoryEntry>> RecallByCategoryAsync(
