...
public sealed class RoutingService : IRoutingService
{
        ...
        EridRouteResolver? routeResolver = null)
    {
        ...
        // Fail-closed: with no verifier supplied, REJECT every RREP rather than trusting
        // unverified route replies (which would let any forwarder hijack routes). A host
        // wires a real signature verifier (e.g. AetherNet.Security's Ed25519RouteReplyVerifier)
        // to permit legitimate, signed RREPs.
        _verifier = verifier ?? new RejectAllRouteReplyVerifier();
