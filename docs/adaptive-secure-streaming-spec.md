...
| **Reputation primitive (Qi / Karma)** | Inputs to red / grey / green route classification; publisher trust state behavioural signals | `IAetherNetIncentiveProvider` extension seam (no-op default in OSS, real implementation in Aether by Circle) |
| **Pre-key bundle distribution** | New-pair X3DH bootstrap when the peer is not in direct mesh range | `CircleAetherNetAPI` `/api/aether/prekey-bundles/{uhid}` |
...
- **Pre-key bundle distribution endpoint:** present in `CircleAetherNetAPI` (server + typed clients).
...
The list is published to the mesh DHT and to `CircleAetherNetAPI` (when reachable). Sources fetch the latest list before establishing new sessions.
