# AetherNet — অফলাইন-প্রথম মেশ নেটওয়ার্কিং প্রোটোকল
...
**AetherNet একটি ওপেন-সোর্স, MIT-লাইসেন্সকৃত মেশ নেটওয়ার্কিং প্রোটোকল** যা কাছাকাছি থাকা মানুষদের কাছে বার্তা, ফাইল, ভয়েস এবং ভিডিও পাঠানোর জন্য — **কোনো ইন্টারনেট নেই, কোনো সার্ভার নেই, এবং কোনো সাইন-আপ নেই**। ডিভাইসগুলো Bluetooth, Wi-Fi Direct, NearLink এবং LoRa-এর ওপর দিয়ে সরাসরি সংযুক্ত হয়; প্রাপক যখন নাগালের বাইরে থাকে, বার্তাগুলো অন্য ডিভাইসগুলোর মধ্য দিয়ে লাফিয়ে যায় এবং একটি রুটের জন্য ৭২ ঘণ্টা পর্যন্ত অপেক্ষা করে। এটি **আটটি প্রোগ্রামিং ভাষায় বাইট-বাই-বাইট অভিন্ন বাস্তবায়ন** শিপ করে — C#, Rust, TypeScript, Python, Go, Kotlin, Swift, এবং C।
...
তার-সার্ভিস স্যুটের বাইরেও, Aether একটি ছোট **নিরাপত্তা ও গোপনীয়তা লেয়ার** সরবরাহ করে — পরিচয়-কী ব্যবস্থাপনা এবং লিঙ্ক-লেয়ার অ্যান্টি-ট্র্যাকিং। বাকি সবকিছুর মতোই, প্রতিটি **সব ৮টি ভাষায়** বাস্তবায়িত এবং `fixtures/<feature>/`-এর অধীনে একটি শেয়ার্ড ক্রস-ল্যাঙ্গুয়েজ ফিক্সচারে পিন করা (Swift ও C অতিরিক্তভাবে macOS build server-এ যাচাইকৃত)। এগুলো ১৮টি তার সার্ভিসের মধ্যে আরও চারটি *নয়*: তিনটি কোনো **নতুন তার প্যাকেট টাইপ সংজ্ঞায়িতই করে না**, এবং চতুর্থটি একটি নতুন সংরক্ষিত প্যাকেট হিসেবে না গিয়ে তার নিজস্ব envelope **বিদ্যমান DTN/mesh পথের ভেতরে** বহন করে।

| সক্ষমতা | এটি যা করে | লেয়ার | ফিক্সচার | 8/8 |
|---|---|---|---|:-:|
| **রিকভারি-ফ্রেজ ব্যাকআপ** | একটি পরিচয়কে **২৪-শব্দের BIP-39** ফ্রেজ হিসেবে ব্যাকআপ করুন এবং যেকোনো ডিভাইসে পুনরুদ্ধার করুন। স্ট্যান্ডার্ড BIP-39 (অফিসিয়াল Trezor ভেক্টরের বিপরীতে যাচাইকৃত), SHA-256-চেকসামযুক্ত যাতে ভুল টাইপ করা শব্দ *প্রত্যাখ্যাত* হয়, কখনও নীরবে ভুল না হয়। কোনো server নেই, কোনো কাস্টোডিয়ান নেই — ফ্রেজটিই **হলো** পরিচয়। | স্থানীয় | `fixtures/bip39/` | ✅ |
| **Bluetooth ট্র্যাকিং-সুরক্ষা** | একটি ঘূর্ণায়মান, কী-থেকে-উদ্ভূত BLE **Service UUID** (HMAC-SHA256, ১৫-মিনিটের উইন্ডো) এবং **resolvable private addresses** (IRK + RFC `ah` ফাংশন, AES-128) উদ্ভূত করে — যে অ্যান্টি-ট্র্যাকিং উপাদান একটি BLE advertiser-এর প্রয়োজন যাতে একটি প্যাসিভ স্ক্যানার সময় বা স্থানের ওপারে এটিকে যুক্ত করতে না পারে। | লিঙ্ক-লেয়ার | `fixtures/bleprivacy/` | ✅ |
| **Panic-wipe** | একটি **duress PIN** (SHA-256, constant-time তুলনাকৃত) যা, জবরদস্তির অধীনে, প্রতিটি পরিচয়-কী নিরাপদে মুছে ফেলে — random-দিয়ে-ওভাররাইট তারপর শূন্য — পুনরুদ্ধারের জন্য কিছুই অবশিষ্ট থাকে না। | স্থানীয় | `fixtures/panicwipe/` | ✅ |
| **মাল্টি-ডিভাইস সিঙ্ক** | আপনার *নিজের* ডিভাইসগুলোর মধ্যে **বিকেন্দ্রীভূত, server-বিহীন** সিঙ্ক: একটি Ed25519-স্বাক্ষরিত **DeviceLink** সেগুলোকে জোড়া দেয়, এবং last-write-wins **SyncRecord** envelope স্টেট মিলিয়ে নেয় — কোনো cloud অ্যাকাউন্ট ও কোনো সিঙ্ক server ছাড়াই বিদ্যমান DTN/mesh-এর ওপর E2E-এনক্রিপ্টেড অবস্থায় বহন করা। | DTN-এ চলে | `fixtures/sync/` | ✅ |

**একটি সৎ অসামঞ্জস্য।** মাল্টি-ডিভাইস **DeviceLink** Ed25519-স্বাক্ষরিত, এবং সেই স্বাক্ষর **৮টির মধ্যে ৭টি ভাষায় বাইট-অভিন্ন**। Apple-এর CryptoKit ইচ্ছাকৃতভাবে Ed25519 স্বাক্ষরকে *randomize* করে, তাই Swift-এ ৬৪টি স্বাক্ষর বাইট প্রতিবার ভিন্ন হয় — কিন্তু **স্বাক্ষরিত বডি বাইট-অভিন্ন** এবং প্রতিটি লিঙ্ক এখনও সব ৮টি SDK-তে যাচাই হয়, তাই Swift স্বাক্ষর-বাইট প্যারিটির পরিবর্তে **যাচাইকরণ** প্যারিটিতে পৌঁছায়। এটি একটি platform-crypto বৈশিষ্ট্য, ত্রুটি নয়, এবং এই চারটি ফিচারের মধ্যে এটিই একমাত্র জায়গা যেখানে "বাইট-অভিন্ন" একটি তারকাচিহ্ন (asterisk) বহন করে। সম্পূর্ণ তার ফরম্যাট আছে [`PROTOCOL_SPEC.md`](../../PROTOCOL_SPEC.md) §12-এ; থ্রেট মডেল আছে [`THREAT_MODEL.md`](../../THREAT_MODEL.md)-এ।
...
সব ৮টি ভাষা বাইট-অভিন্ন তার প্যাকেট তৈরি করে, CI-তে চালানো ১৭টি ক্যানোনিক্যাল তার-ফরম্যাট ফিক্সচার এবং ৬টি Signal টেস্ট ভেক্টর দিয়ে যাচাইকৃত (`fixtures/expected/*.bin`, `fixtures/signal/expected/*.json`)। রাউটিং (AODV-স্টাইল RREQ/RREP), DTN স্টোর-অ্যান্ড-ফরোয়ার্ড, SOS সম্প্রচার, ভয়েস, স্ট্রিমিং, এবং নিরাপত্তা-শক্তিশালীকরণ সার্ভিস প্রতিটি ভাষায় বাস্তবায়িত সব ৮টি বাস্তবায়ন জুড়ে **~3,000 টেস্ট** সহ:
...
ক্রস-ল্যাঙ্গুয়েজ Signal ইন্টারঅপ `fixtures/signal/`-এ নোঙর করা, X3DH (`x3dh_basic`), সিমেট্রিক র‍্যাচেট (`ratchet_step_basic`, `ratchet_step_three_iterations`), KDF_RK (`kdf_rk_basic`), এবং সম্পূর্ণ X3DH সেশন রাউন্ড-ট্রিপ (`x3dh_session_msg1`, `x3dh_session_reply`)-এর জন্য ভাগ করা টেস্ট ভেক্টর সহ। প্রতিটি বাস্তবায়নকে সেই ফিক্সচারগুলোর বিপরীতে বাইট-অভিন্ন আউটপুট তৈরি করতে হবে। সব ৮টি ভাষা এখন একটি সম্পূর্ণ Signal সেশন শিপ করে (`generate_pre_key_bundle`, `process_pre_key_bundle`, `encrypt`, `decrypt`)।
...
- তার ফরম্যাট: ৮টি ভাষা জুড়ে বাইট-অভিন্ন, ১৭টি ক্যানোনিক্যাল ফিক্সচার এবং CI-তে ক্রস-ল্যাঙ্গুয়েজ অ্যাসারশন দিয়ে নোঙর করা (`fixtures/expected/*.bin`)
...
**AetherNet কি ইন্টারনেট ছাড়া কাজ করে?**
হ্যাঁ — এটি অফলাইন-প্রথম। ডিভাইসগুলো Bluetooth, Wi-Fi Direct, NearLink, বা LoRa-এর ওপর দিয়ে সরাসরি কথা বলে এবং অন্য ডিভাইসগুলোর মধ্য দিয়ে হপ-বাই-হপ বার্তা রিলে করে, কোনো ইন্টারনেট সংযোগ, সেল টাওয়ার, বা সার্ভারের প্রয়োজন ছাড়াই। যখন কোনো লাইভ রুট বিদ্যমান নেই, একটি রুট খোলা না হওয়া পর্যন্ত বার্তাগুলো ৭২ ঘণ্টা পর্যন্ত ধরে রাখা হয় (বিলম্ব-সহনশীল স্টোর-অ্যান্ড-ফরোয়ার্ড)।

**এটি কি এন্ড-টু-এন্ড এনক্রিপ্ট করা?**
হ্যাঁ। AetherNet এন্ড-টু-এন্ড এনক্রিপশনের জন্য Signal Protocol (X3DH কি চুক্তি এবং X25519-এর ওপর Double Ratchet), বার্তা পেলোডের জন্য AES-256-GCM, এবং প্রতিটি প্যাকেটে Ed25519 স্বাক্ষর ব্যবহার করে। যেসব ডিভাইস একটি বার্তা রিলে করে তারা এটি পড়তে পারে না।

**এটি কোন ট্রান্সপোর্ট ব্যবহার করে?**
Bluetooth LE, Wi-Fi Direct, NearLink (SLE), একটি LoRa/CircleLink সিরিয়াল রেডিও, একটি HTTP/QUIC রিলে, এবং সরাসরি ইন্টারনেট পিয়ার-টু-পিয়ারের জন্য WebRTC। প্রোটোকল স্বয়ংক্রিয়ভাবে প্রতিটি প্যাকেটের জন্য সর্বনিম্ন-পাওয়ার উপলব্ধ ট্রান্সপোর্ট নির্বাচন করে এবং পরবর্তীটিতে ফিরে আসে।

**এটি কোন প্রোগ্রামিং ভাষায় উপলব্ধ?**
আটটি — C#, Rust, TypeScript, Python, Go, Kotlin, Swift, এবং C। প্রতিটি বাস্তবায়ন বাইট-অভিন্ন তার প্যাকেট তৈরি করে, যা CI-তে একটি ভাগ করা ক্রস-ল্যাঙ্গুয়েজ ফিক্সচার কর্পাস দিয়ে বলবৎ করা হয়, তাই একটি ভাষা দ্বারা তৈরি একটি প্যাকেট অন্য যেকোনো ভাষা দ্বারা অপরিবর্তিতভাবে ডিকোড করা হয়।

**এটি Meshtastic, Briar, বা Bridgefy থেকে কীভাবে আলাদা?**
Meshtastic শুধুমাত্র-LoRa; AetherNet মাল্টি-ট্রান্সপোর্ট (Bluetooth + Wi-Fi + NearLink + LoRa) এবং বার্তার পাশাপাশি ভয়েস, ভিডিও এবং স্ট্রিমিংও বহন করে। Briar শুধুমাত্র-Android এবং Tor-এর ওপর দিয়ে রুট করে; AetherNet ক্রস-প্ল্যাটফর্ম এবং বিশুদ্ধ মেশ। বন্ধ SDK-এর বিপরীতে, AetherNet MIT-লাইসেন্সকৃত এবং আটটি ভাষায় খোলাখুলিভাবে বাস্তবায়িত। উপরের তুলনা টেবিলে বিস্তারিত আছে।

**এটি কি প্রোডাকশন-প্রস্তুত?**
প্রোটোকল লেয়ার — তার ফরম্যাট, Signal নিরাপত্তা, রাউটিং, DTN স্টোর-অ্যান্ড-ফরোয়ার্ড, এবং সম্পূর্ণ সার্ভিস স্যুট — সব আটটি ভাষা জুড়ে বাস্তবায়িত ও পরীক্ষিত। রেডিও ট্রান্সপোর্টগুলো সেখানে প্রকৃত যেখানে প্ল্যাটফর্ম কোড বিদ্যমান (Windows এবং Android-এ Bluetooth ও Wi-Fi, সর্বত্র WebRTC) এবং হার্ডওয়্যার ব্রিং-আপ বাকি থাকায় অন্যত্র ফিল্ড-অযাচাইকৃত, যা `OPEN_ISSUES.md`-এ সততার সঙ্গে ট্র্যাক করা হয়। ডিপ্লয় করার আগে প্রতিটি বিভাগের স্ট্যাটাস নোট পড়ুন।

**এটি কোন লাইসেন্সের অধীনে?**
MIT — বাণিজ্যিক ও ওপেন-সোর্স ব্যবহারের জন্য বিনামূল্যে। দেখুন [LICENSE](LICENSE)।

**AetherNet কে তৈরি করে?**
এটি The Geek Network-এর মেশ ইকোসিস্টেমের পেছনের ওপেন প্রোটোকল হিসেবে বিকশিত, দক্ষিণ আফ্রিকায় নির্মিত এমন যোগাযোগের জন্য যা মোবাইল ডেটা সহ বা ছাড়াই কাজ করে।
