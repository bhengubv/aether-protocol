...
تُنتج جميع اللغات الثماني حزمًا متطابقة بالبايت على مستوى الأسلاك، مُتحقق منها بـ17 مخرج قانونياً لتنسيق الأسلاك و6 متجهات اختبار Signal تُشغَّل في CI (`fixtures/expected/*.bin`، `fixtures/signal/expected/*.json`). التوجيه (بأسلوب AODV RREQ/RREP)، وDTN للتخزين والإعادة، وبث SOS، والصوت، والبث، وخدمات تصليب الأمان مُطبَّقة في كل لغة مع **~3,000 اختبار** عبر جميع التطبيقات الثمانية:
...
- تنسيق الأسلاك: متطابق بالبايت عبر 8 لغات، مرتبط بـ17 مخرج قانونياً وتأكيدات متعددة اللغات في CI (`fixtures/expected/*.bin`)
