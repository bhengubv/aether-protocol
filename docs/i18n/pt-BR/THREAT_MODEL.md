...
`src/Aether.Security/` é a autoridade.
...
`BuildSignableData(packet)` canônico (`src/Aether.Security/Services/PacketSigningService.cs`,
...
chave raiz e de cadeia — `src/Aether.Security/Services/SignalProtocolService.cs`).
...
`tests/Aether.Core.Tests/PreKeyPoolTests.cs`). Uma OPK é removida e zerada no momento
...
`Aether.Security` disponibiliza uma costura `IGroupKeyProvider`, mas o protocolo
...
no `Aether.Security`; rastreado como lacuna.
