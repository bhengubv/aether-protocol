...
- `aethernet_mesh_packet_t *aethernet_packet_new(void)` — Cria um novo pacote
- `void aethernet_packet_free(aethernet_mesh_packet_t *packet)` — Libera um pacote
- `aethernet_mesh_packet_t *aethernet_packet_clone(const aethernet_mesh_packet_t *packet)` — Clona um pacote

#### Serialização
- `int aethernet_packet_serialize(const aethernet_mesh_packet_t *packet, uint8_t *buffer, size_t buffer_len)` — Serializa para o formato de transmissão
- `aethernet_mesh_packet_t *aethernet_packet_deserialize(const uint8_t *data, size_t data_len)` — Desserializa do formato de transmissão
- `size_t aethernet_packet_estimate_size(const aethernet_mesh_packet_t *packet)` — Estima o tamanho no formato de transmissão

#### Campos do Pacote
- `bool aethernet_packet_set_source_uhid(aethernet_mesh_packet_t *packet, const char *uhid)` — Define a origem
- `bool aethernet_packet_set_destination_uhid(aethernet_mesh_packet_t *packet, const char *uhid)` — Define o destino
- `bool aethernet_packet_set_payload(aethernet_mesh_packet_t *packet, const uint8_t *data, size_t len)` — Define o payload
- `bool aethernet_packet_set_signature(aethernet_mesh_packet_t *packet, const uint8_t *sig, size_t len)` — Define a assinatura

#### Validação
- `bool aethernet_packet_is_expired(const aethernet_mesh_packet_t *packet, int max_age_seconds)` — Verifica se expirou
- `bool aethernet_packet_can_forward(const aethernet_mesh_packet_t *packet)` — Verifica se o TTL > 0

#### Dados para Assinatura
- `uint8_t *aethernet_packet_get_signable_data(const aethernet_mesh_packet_t *packet, size_t *out_len)` — Obtém os bytes assináveis determinísticos (o chamador deve liberar)

### Segurança

#### Ed25519
- `bool aethernet_ed25519_generate_keypair(uint8_t *out_private, uint8_t *out_public)` — Gera chaves de 32+32 bytes
- `bool aethernet_ed25519_sign(const uint8_t *private_key, const uint8_t *data, size_t data_len, uint8_t *out_signature)` — Assina (produz 64 bytes)
- `bool aethernet_ed25519_verify(const uint8_t *public_key, const uint8_t *data, size_t data_len, const uint8_t *signature)` — Verifica

#### AES-256-GCM
- `bool aethernet_aes256_gcm_encrypt(const uint8_t *plaintext, size_t plaintext_len, const uint8_t *key, const uint8_t *nonce, const uint8_t *aad, size_t aad_len, uint8_t *out_ciphertext, uint8_t *out_tag, uint8_t *out_nonce)` — Criptografa (nonce gerado automaticamente se NULL)
- `bool aethernet_aes256_gcm_decrypt(const uint8_t *ciphertext, size_t ciphertext_len, const uint8_t *key, const uint8_t *nonce, const uint8_t *tag, const uint8_t *aad, size_t aad_len, uint8_t *out_plaintext)` — Descriptografa
...
#### Utilitários
- `void aethernet_zeroize(void *mem, size_t len)` — Limpeza de memória em tempo constante
- `bool aethernet_random_bytes(uint8_t *out, size_t len)` — Bytes aleatórios criptograficamente seguros

### Transporte

#### Funções Genéricas
- `bool aethernet_transport_send(aethernet_transport_t *transport, const char *peer_uhid, const uint8_t *data, size_t data_len)` — Envia dados
- `bool aethernet_transport_is_connected(aethernet_transport_t *transport, const char *peer_uhid)` — Verifica conexão
- `void aethernet_transport_set_on_data_received(aethernet_transport_t *transport, aethernet_transport_on_data_received callback, void *user_data)` — Registra callback
- `void aethernet_transport_destroy(aethernet_transport_t *transport)` — Limpeza

#### Transporte em Processo
- `aethernet_transport_t *aethernet_inprocess_transport_new(void)` — Cria transporte compartilhado em processo
- `bool aethernet_inprocess_transport_register_node(aethernet_transport_t *transport, const char *uhid)` — Registra um nó
- `bool aethernet_inprocess_transport_unregister_node(aethernet_transport_t *transport, const char *uhid)` — Cancela registro de um nó
...
- Pacote máximo: 65KB (configurável via `AETHERNET_MAX_PAYLOAD_LEN`)
...
- **AetherNetAPI** (C#) — relay de malha e análises no lado do servidor
- **AetherNet.Core** (C#) — implementação de referência (formato de transmissão interoperável)
...
- Referência C#: `/Users/admin/Code/Dev/aether-protocol/src/AetherNet.Core/`
