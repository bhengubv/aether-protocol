...
> - 레퍼런스 직렬화기: `src/Aether.Core/Protocol/PacketSerializer.cs`
> - 레퍼런스 Signal 스택: `src/Aether.Security/Services/SignalProtocolService.cs`
> - 레퍼런스 라우팅: `src/Aether.Core/Routing/RoutingService.cs`
> - 레퍼런스 DTN: `src/Aether.Core/Dtn/DtnService.cs`
...
> 2026-05-05에 `src/Aether.Core/Protocol/PacketSerializer.cs` 및
...
레퍼런스 구현은 `src/Aether.Security/Services/
...
> 2026-05-05에 `src/Aether.Security/Services/SignalProtocolService.cs`의 C# 레퍼런스 구현 및
...
레퍼런스: `Aether.Security.Models.PreKeyBundle`. 와이어 형상 계약은
모든 8개 언어에서 동일합니다.
...
`tests/Aether.Core.Tests/PreKeyPoolTests.cs`에서 검증됩니다.
...
레퍼런스: `Aether.Security.Models.EncryptedPayload` (`SecurityModels.cs` 55–66번 줄).
...
5. **폴백:** 로컬 전송 수단이 목적지에 도달할 수 없으면 패킷은 AetherAPI를 통한 서버 릴레이를 위해 큐에 대기합니다.
...
3. **이중 경로 전송:** SOS는 동시에 다음을 통해 전송됩니다:
   ...
   - **API 호출:** 서버 측 배포 및 PanikAPI 연결(SMS/이메일 전송)을 위해 AetherAPI로 전송.
...
3. **서버 릴레이 시도:** 메시 라우팅이 실패하면 송신자는 AetherAPI를 통한 릴레이를 시도합니다. 서버가 수신자에 도달할 수 있으면 (또는 메시지를 큐에 넣을 수 있으면) 전달에 성공합니다.
...
4. 영수증은 분석을 위해 AetherAPI에도 동기화됩니다.
...
> C# `Aether.Streaming` 모듈은 인터페이스, 모델, 스켈레톤 서비스
...
> 픽스처 테스트가 완료되었습니다. `Aether.Streaming.WatchTogetherService`는
...
| Flag | Parent | 설명 |
|------|--------|-------------|
| AETHER_VIDEO_CALL | AETHER_VOICE | P2P 및 그룹 비디오 통화 |
| AETHER_VIDEO_GROUP | AETHER_VIDEO_CALL | 다중 참가자 비디오 세션 |
| AETHER_SCREEN_SHARE | AETHER_VIDEO_CALL | 비디오 통화 중 화면 공유 |
| AETHER_WATCH_TOGETHER | AETHER_CONTENT_P2P | 동기화된 미디어 재생 |
| AETHER_WATCH_REACTIONS | AETHER_WATCH_TOGETHER | 이모지 및 음성 반응 |
| AETHER_TORRENT_INGEST | AETHER_CONTENT_P2P | 메시 배포를 위한 BitTorrent 파일 수락 |
