...
> - Serializador de referencia: `src/Aether.Core/Protocol/PacketSerializer.cs`
> - Pila Signal de referencia: `src/Aether.Security/Services/SignalProtocolService.cs`
> - Enrutamiento de referencia: `src/Aether.Core/Routing/RoutingService.cs`
> - DTN de referencia: `src/Aether.Core/Dtn/DtnService.cs`
...
> Reconciliado el 2026-05-05 con `src/Aether.Core/Protocol/PacketSerializer.cs`
...
La implementación de referencia se encuentra en `src/Aether.Security/Services/
...
> `src/Aether.Security/Services/SignalProtocolService.cs` y el corpus de
...
Referencia: `Aether.Security.Models.PreKeyBundle`. El contrato de forma en cable es
...
del pool es ejercida por `tests/Aether.Core.Tests/PreKeyPoolTests.cs`.
...
Referencia: `Aether.Security.Models.EncryptedPayload` (líneas 55–66 de
...
5. **Respaldo:** Si ningún transporte local puede alcanzar el objetivo, el paquete se pone en cola para relay vía AetherAPI.
...
3. **Despacho de doble ruta:** El SOS se envía simultáneamente mediante:
   ...
   - **Llamada API:** Enviado a AetherAPI para distribución del lado del servidor y puente con PanikAPI (despacho por SMS/correo electrónico).
...
3. **Intento de relay por servidor:** Si el enrutamiento en malla falla, el emisor intenta hacer relay a través de AetherAPI. Si el servidor puede alcanzar al destinatario (o poner el mensaje en cola), la entrega tiene éxito.
...
4. El recibo también se sincroniza con AetherAPI para análisis.
...
> El módulo C# `Aether.Streaming` incluye interfaces, modelos y servicios skeleton
...
> probados con fixtures. `Aether.Streaming.WatchTogetherService` proporciona el
...
|------|--------|-------------|
| AETHER_VIDEO_CALL | AETHER_VOICE | Videollamadas P2P y grupales |
| AETHER_VIDEO_GROUP | AETHER_VIDEO_CALL | Sesiones de video con múltiples partes |
| AETHER_SCREEN_SHARE | AETHER_VIDEO_CALL | Compartir pantalla en videollamadas |
| AETHER_WATCH_TOGETHER | AETHER_CONTENT_P2P | Reproducción sincronizada de medios |
| AETHER_WATCH_REACTIONS | AETHER_WATCH_TOGETHER | Reacciones de emoji y voz |
| AETHER_TORRENT_INGEST | AETHER_CONTENT_P2P | Aceptación de archivos BitTorrent para distribución en malla |
