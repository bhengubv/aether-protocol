...
`AetherNet.DependencyInjection` 会传递引入 `AetherNet.Core`、
`AetherNet.Security`、`AetherNet.Messaging`、`AetherNet.Transport`、`AetherNet.Streaming`、
`AetherNet.Voice` 和 `AetherNet.Content`——消息栈所需的一切。`AetherNet.Storage` 是一个单独的依赖项，仅当你需要磁盘持久化时才需要（参见第 6 节）。
...
DI 扩展方法 `AddAetherNetProtocol(...)` 返回一个流式构建器。每个能力都是可选的：只需要路由的主机调用 `.AddRouting()` 后即可停止。以下是典型采用者所需的完整栈。
...
`AddAetherNetProtocol` 以及每个链式方法在同一 `IServiceCollection` 上是幂等的——调用两次不会重复注册。顺序在一处有影响：如果未先调用 `AddSignalProtocol()` 或 `AddRouting()`，`AddMessaging()` 会抛出 `InvalidOperationException`。

`InProcessTransport` 用于测试和演示。在生产环境中，你需要为物理层（BLE GATT、Wi-Fi Direct、NearLink、LoRa……）实现 `AetherNet.Transport.Abstractions.ITransportService`，并注册一个 `IMeshSender` 将数据包桥接到它上面。路由/DTN/消息服务随后在其之上不变地运行。
...
这是一个可运行的脚本。将其复制到 `Program.cs`，添加对 `AetherNet.Security.csproj` 的 `<ProjectReference>`（它会引入 `AetherNet.Core` 和 BCL 加密），然后 `dotnet run`。
...
`AetherNet.Storage.FileSystemKeyValueStore` 是一个最小的磁盘支持的 `IKeyValueStore`（每个条目一个文件，原子临时文件重命名）。通过 `KeyValue*Store` 适配器接入：
...
你也可以在链式调用 `.AddRouting()` / `.AddDtn()` / `.AddMessaging()` 之前，向 DI 容器注册非默认的 `IRouteStore`、`IDtnBundleStore` 和 `IMessageStore`——构建器使用 `TryAdd*` 并尊重你首先放入容器的内容。`AetherNet.Storage` 中的 `KeyValueRouteStore`、`KeyValueDtnBundleStore` 和 `KeyValueMessageStore` 适配器可针对任意 `IKeyValueStore` 覆盖这些插槽。
...
- **计数器**：`aethernet.messages.encrypted`、`aethernet.messages.decrypted`、
...
- **直方图**（毫秒）：`aethernet.encrypt.latency`、`aethernet.decrypt.latency`、
  `aethernet.route.lookup_latency`、`aethernet.sign.verify_latency`。
- **活动**（带 PII 脱敏 UHID 标签）：
  `AetherNet.Encrypt`、`AetherNet.Decrypt`、`AetherNet.DhRatchet.Step`、
  `AetherNet.Sign.Packet`、`AetherNet.Verify.Packet`，以及路由和 DTN 跨度。

当没有监听器附加时，热路径不分配内存——计数器 `Add` 退化为 volatile 读取，`StartActivity` 返回 `null`。

完整的检测清单和 PII 契约位于 `src/AetherNet.Core/Diagnostics/AetherNetTelemetry.cs`。
...
通过 `AetherNetOptions.Routing`、`Dtn`、`Signal` 和 `Messaging` 配置包进行调整。主机必须在 Aether 构建器的 `.AddHealthChecks()` 之前调用 `services.AddHealthChecks()`，才能使注册对 `MapHealthChecks(...)` 可见。
...
- **`SECURITY.md`** — 负责任披露政策。
- **`samples/AetherNet.Demo.Console/Program.cs`** — 可运行的 9 步端到端演练。步骤 9（MessagingService + DTN）是生产接线模式。
