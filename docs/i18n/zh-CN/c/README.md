...
#### 数据包管理
- `aether_mesh_packet_t *aether_packet_new(void)` — 创建新数据包
- `void aether_packet_free(aether_mesh_packet_t *packet)` — 释放数据包
- `aether_mesh_packet_t *aether_packet_clone(const aether_mesh_packet_t *packet)` — 克隆数据包

#### 序列化
- `int aether_packet_serialize(const aether_mesh_packet_t *packet, uint8_t *buffer, size_t buffer_len)` — 序列化为线格式
- `aether_mesh_packet_t *aether_packet_deserialize(const uint8_t *data, size_t data_len)` — 从线格式反序列化
- `size_t aether_packet_estimate_size(const aether_mesh_packet_t *packet)` — 估算线格式大小

#### 数据包字段
- `bool aether_packet_set_source_uhid(aether_mesh_packet_t *packet, const char *uhid)` — 设置来源
- `bool aether_packet_set_destination_uhid(aether_mesh_packet_t *packet, const char *uhid)` — 设置目标
- `bool aether_packet_set_payload(aether_mesh_packet_t *packet, const uint8_t *data, size_t len)` — 设置载荷
- `bool aether_packet_set_signature(aether_mesh_packet_t *packet, const uint8_t *sig, size_t len)` — 设置签名

#### 验证
- `bool aether_packet_is_expired(const aether_mesh_packet_t *packet, int max_age_seconds)` — 检查是否已过期
- `bool aether_packet_can_forward(const aether_mesh_packet_t *packet)` — 检查 TTL 是否大于 0

#### 签名数据
- `uint8_t *aether_packet_get_signable_data(const aether_mesh_packet_t *packet, size_t *out_len)` — 获取确定性可签名字节（调用方负责释放）

### 安全

#### Ed25519
- `bool aether_ed25519_generate_keypair(uint8_t *out_private, uint8_t *out_public)` — 生成 32+32 字节密钥对
- `bool aether_ed25519_sign(const uint8_t *private_key, const uint8_t *data, size_t data_len, uint8_t *out_signature)` — 签名（生成 64 字节）
- `bool aether_ed25519_verify(const uint8_t *public_key, const uint8_t *data, size_t data_len, const uint8_t *signature)` — 验证

#### AES-256-GCM
- `bool aether_aes256_gcm_encrypt(const uint8_t *plaintext, size_t plaintext_len, const uint8_t *key, const uint8_t *nonce, const uint8_t *aad, size_t aad_len, uint8_t *out_ciphertext, uint8_t *out_tag, uint8_t *out_nonce)` — 加密（若 nonce 为 NULL 则自动生成）
- `bool aether_aes256_gcm_decrypt(const uint8_t *ciphertext, size_t ciphertext_len, const uint8_t *key, const uint8_t *nonce, const uint8_t *tag, const uint8_t *aad, size_t aad_len, uint8_t *out_plaintext)` — 解密

#### HMAC 与哈希
- `bool aether_hmac_sha256(const uint8_t *key, size_t key_len, const uint8_t *data, size_t data_len, uint8_t *out_hash)` — HMAC-SHA256（32 字节）
- `bool aether_sha256(const uint8_t *data, size_t data_len, uint8_t *out_hash)` — SHA-256（32 字节）
...
#### 工具函数
- `void aether_zeroize(void *mem, size_t len)` — 常量时间内存清零
- `bool aether_random_bytes(uint8_t *out, size_t len)` — 密码学安全随机字节

### 传输

#### 通用函数
- `bool aether_transport_send(aether_transport_t *transport, const char *peer_uhid, const uint8_t *data, size_t data_len)` — 发送数据
- `bool aether_transport_is_connected(aether_transport_t *transport, const char *peer_uhid)` — 检查连接状态
- `void aether_transport_set_on_data_received(aether_transport_t *transport, aether_transport_on_data_received callback, void *user_data)` — 注册回调
- `void aether_transport_destroy(aether_transport_t *transport)` — 清理资源

#### 进程内传输
- `aether_transport_t *aether_inprocess_transport_new(void)` — 创建共享进程内传输
- `bool aether_inprocess_transport_register_node(aether_transport_t *transport, const char *uhid)` — 注册节点
- `bool aether_inprocess_transport_unregister_node(aether_transport_t *transport, const char *uhid)` — 注销节点
...
### 内存使用
- 最小数据包：约 52 字节
- 最大数据包：65KB（可通过 `AETHER_MAX_PAYLOAD_LEN` 配置）
- 256 节点对等表：约 32KB
...
本 C 库设计为与以下系统集成：
- **AetherAPI**（C#）— 服务端网状中继与分析
- **Aether.Core**（C#）— 参考实现（可互操作的线格式）
...
- C# 参考实现：`/Users/admin/Code/Dev/aether-protocol/src/Aether.Core/`
