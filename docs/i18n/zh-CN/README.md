...
| 🟣 Aether Purple | 蜂窝 HTTP 中继 | 无限 | ~10 Mbps | ✅ Windows——中继服务器位于 `samples/Aether.RelayServer/` |
...
- ✅ **C：完整 Signal 会话** — `c/src/signal_protocol.c` 中的 `aether_signal_service_init`、`generate_pre_key_bundle`、`process_pre_key_bundle`、`encrypt`、`decrypt`；`c/tests/test_signal_session.c` 中的 6 个双节点端到端测试。所有 8 种语言现已具备完整会话能力的 Signal Protocol。

**已完成（仅 C# 参考实现）：**
- ✅ **演示步骤 9 — MessagingService + DTN 回退端到端** — `samples/Aether.Demo.Console` 演示在接收方离线时使用 DTN 存储转发的真实 Signal 加密消息传递。
- ✅ **`Aether.Messaging` ↔ `Aether.Security` 桥接** — `SignalMessageEnvelopeCipher` 使消息层默认端到端加密；没有 Signal 会话的消息会进入队列，永不以明文发送。
...
**已完成（C# 传输层——全部快速失败）：**
- ✅ **BLE GATT 真实传输** — `WinBleGattTransportService`（Windows WinRT）+ `android/blue/`（Android GATT 服务器）。完整 RF 启动测试位于 `samples/Aether.BleRfTest/`。
- ✅ **Wi-Fi Direct 真实传输** — `WinWifiDirectTransportService`（WinRT，`WiFiDirectAdvertisementPublisher` + TCP StreamSocket 端口 8888）+ `android/green/`（`WifiP2pManager`）。RF 测试位于 `samples/Aether.WifiDirectRfTest/`。
- ✅ **HTTP 中继传输（Aether Purple）** — 带 10 秒长轮询、`PowerCostRelative = 100`、始终作为最后手段的 `HttpRelayTransportService`。中继服务器位于 `samples/Aether.RelayServer/`（ASP.NET Core minimal API，端口 5200）。RF 测试位于 `samples/Aether.RelayRfTest/`。
...
- `IAetherIncentiveProvider` — 奖励中继流量的节点（无操作默认值：利他中继）
- `IAetherBackendClient` — 有互联网时与服务器同步（无操作默认值：完全离线）
- `IAetherFeatureFlagProvider` — 在运行时切换协议功能（无操作默认值：全部启用）
