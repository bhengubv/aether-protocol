...
> **یک پروتکل، هشت زبان، یکسان روی سیم.** Aether در **C#، Rust، TypeScript، Python، Go، Kotlin، Swift و C** پیاده‌سازی شده است — و هر بسته در تمام آن‌ها بایت‌به‌بایت یکسان است، که توسط یک مجموعه fixture مشترک بین‌زبانی اعمال می‌شود که هر پیاده‌سازی باید بایت‌به‌بایت با آن مطابقت داشته باشد. گره خود را در هر یک از این هشت زبان بسازید؛ با تمام بقیه تعامل‌پذیر است. این README در ۲۰ زبان انسانی نیز موجود است (پیوندهای بالا).

## به زبان ساده

**AetherNet به تلفن‌ها و لپ‌تاپ‌ها اجازه می‌دهد مستقیماً با یکدیگر صحبت کنند — بدون اینترنت، بدون شرکت مخابراتی و بدون حساب کاربری.** اگر افراد اطراف شما این برنامه را داشته باشند، می‌توانید به آن‌ها پیام بدهید، عکس و فایل‌های بزرگ بفرستید، تماس‌های صوتی و تصویری برقرار کنید و یک پخش زنده را به اشتراک بگذارید، تنها با استفاده از رادیوهای برد کوتاهی که همین حالا درون هر تلفن هستند (Bluetooth و Wi-Fi). اگر کسی برای دسترسی مستقیم بیش از حد دور باشد، پیام شما بی‌سروصدا از یک تلفن به تلفن بعدی هاپ می‌کند تا برسد — و در صورت لزوم تا سه روز منتظر یک مسیر می‌ماند. حتی می‌تواند به شبکه‌های بزرگ عمومی اشتراک‌گذاری فایلِ جهان دست پیدا کند (همان فناوریِ پشتِ دانلودهای قانونی مانند Linux و به‌روزرسانی‌های بازی)، یک فایل را واکشی کند و آن را به سمت داخل، به دوستی که اصلاً اینترنت ندارد، برساند.

همه چیز به‌صورت انتها-به-انتها درهم‌ریخته می‌شود، بنابراین فقط کسی که با او صحبت می‌کنید می‌تواند آن را بخواند — تلفن‌هایی که آن را عبور می‌دهند نمی‌توانند. این پروتکل **رایگان و باز** است تا هر کسی از آن استفاده کند یا آن را وارسی کند، و هشت بار، در هشت زبان برنامه‌نویسی، نوشته شده است تا بتواند تقریباً روی هر دستگاهی اجرا شود.

**چقدر تکمیل شده است؟** «مغزِ» شبکه — فرمت‌های پیام، رمزگذاری، مسیریابی و اشتراک‌گذاری فایل — در تمام هشت زبان ساخته و به‌صورت ماشینی بررسی شده است. آنچه هنوز به آزمایش در دنیای واقعی نیاز دارد، صحبت‌کردن خودِ رادیوها با یکدیگر از طریق هوا میان دو تلفن فیزیکی است؛ آن گام سخت‌افزاری چیزی است که باقی مانده، و ما آن را آشکارا در `OPEN_ISSUES.md` ردیابی می‌کنیم. هر آنچه در پایین می‌آید همان داستان با جزئیات بیشتر است.
...
**یک فایل بزرگ را همان‌گونه که کل اینترنت پیش‌تر آن‌ها را به اشتراک می‌گذارد، دریافت کنید.**

BitTorrent فناوریِ پشتِ بخش بزرگی از اشتراک‌گذاری قانونی فایل در جهان است — نسخه‌های Linux، به‌روزرسانی‌های بازی، Internet Archive. Aether اکنون *واقعاً* آن را صحبت می‌کند: یک گره Aether می‌تواند به یک swarm عادی BitTorrent بپیوندد و یک فایل را مستقیماً از جمعیت واکشی کند، بدون هیچ سرور مرکزی. و اینجا نکته جالب برای افرادی که داده ندارند — یک گره Aether که اینترنت *دارد* می‌تواند یک torrent را واکشی کند و **آن را در سراسر mesh آفلاین بازنشر کند**، بنابراین دوستی که کاملاً آفلاین است همچنان فایل را، هاپ‌به‌هاپ، از طریق Bluetooth و Wi-Fi دریافت می‌کند. بزرگ‌ترین شبکه اشتراک‌گذاری فایل جهان، که به مردمی می‌رسد که اینترنت به آن‌ها نمی‌رسد.
...
## BitTorrent — واقعی، و پل‌زده به mesh

Aether اکنون شامل یک **پیاده‌سازی اصیل و تعامل‌پذیر BitTorrent** است — همان پروتکل واقعی‌ای که کلاینت‌های واقعی torrent استفاده می‌کنند، نه یک بدل ظاهری. بنابراین یک گره Aether می‌تواند به یک swarm عادی بپیوندد و قطعات یک فایل را با غریبه‌ها روی اینترنت مبادله کند، بدون هیچ سروری در میانه.

ما فقط ادعا نکردیم که واقعی است — آن را ثابت کردیم. Aether در برابر **MonoTorrent** بررسی شد، یک کتابخانه BitTorrent بالغ و مستقل که افراد دیگری آن را ساخته‌اند: با یک فایل یکسان، هر دو اثرانگشت *یکسانی* تولید می‌کنند، بنابراین هر کلاینت واقعی torrent با Aether مانند یکی از خودش رفتار می‌کند. هر کسی می‌تواند یک کلاینت واقعی BitTorrent را به آن نشانه بگیرد و خودش ببیند.

افزون بر این، Aether یک **پل** اضافه می‌کند: یک گره با اینترنت می‌تواند یک torrent را از وبِ گسترده‌تر واکشی کند، قطعات آن را به‌صورت قطعه‌های mesh رمزگذاری‌شده‌ی خودِ Aether بسته‌بندی مجدد کند و آن را به جلو به اشتراک بگذارد — بنابراین کسی که **اصلاً اینترنت ندارد** همچنان می‌تواند آن فایل را در سراسر mesh آفلاین دریافت کند. نکته همین است: بزرگ‌ترین شبکه اشتراک‌گذاری فایل جهان را به مردمی وصل کنید که معمولاً نمی‌تواند به آن‌ها برسد.

**وضعیت، صادقانه، کجاست.** *فرمت‌های* BitTorrent — اینکه یک torrent چگونه توصیف، اثرانگشت‌گیری و روی سیم قاب‌بندی می‌شود — ساخته شده‌اند و **بایت‌به‌بایت در تمام ۸ زبان یکسان** هستند، پین‌شده به یک مجموعه fixture مشترک در `fixtures/bittorrent/`. کلاینت کامل و کارآمد و پل mesh در **مرجع C#** کامل و تأیید شده‌اند؛ هفت زبان دیگر فرمت‌های پروتکلِ یکسان را حمل می‌کنند، که لایه شبکه‌ی زنده‌شان گام بعدی است.

> **برای توسعه‌دهندگان.** پوشش: bencode + `.torrent`/magnet + info-hash با SHA-1 و peer-wire مطابق BEP-3 (rarest-first)، ترکرهای HTTP + UDP (BEP-3/15/23)، Mainline DHT + PEX + ut_metadata (BEP-5/11/9/10)، µTP (BEP-29)، و merkle با SHA-256 در BitTorrent v2 (BEP-52)، به‌علاوه یک **دروازه**‌ی piece↔chunk به داخل سرویس محتوا و یک دانلودر سگمنتیِ همزمان و قابل‌ازسرگیری. مرجع C# (`src/AetherNet.BitTorrent`، `src/AetherNet.BitTorrent.Gateway`) کلاینت زنده‌ی TCP/µTP، گره DHT، ترکرها، دروازه و دانلودر را ارائه می‌دهد، همراه با آزمون تعامل‌پذیری MonoTorrent در `tests/AetherNet.BitTorrent.Interop.Tests`. مجموعه‌ی هویت بایتیِ ۸-زبانه (`fixtures/bittorrent/vectors.json`، ۷ دسته) شامل bencode، info-hash، peer-wire، µTP، merkle، compact-info و KRPC است؛ هر SDK یک آزمون fixture متناظر ارائه می‌دهد.
...
| زبان | پوشه | فرمت سیم | مسیریابی/DTN/SOS | X3DH | Double Ratchet | مخزن OPK | صدا/گروه | استریمینگ/تصویر/تماشا | BitTorrent |
|----------|-----------|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
| C# (.NET 10) | `src/` | ✅ | ✅ | ✅ | ✅ | ✅ (100) | ✅ | ✅ | ✅ |
| Rust | `rust/` | ✅ | ✅ | ✅ | ✅ | ✅ (100) | ✅ | ✅ | ◐ |
...
**ستون BitTorrent:** ✅ = کلاینت کامل و کارآمد + دروازه mesh (مرجع C#). ◐ = **فرمت‌های سیم** BitTorrent اینجا بایت‌به‌بایت یکسان هستند (پین‌شده به `fixtures/bittorrent/`)، که لایه شبکه زنده گام بعدی است — به [BitTorrent — واقعی، و پل‌زده به mesh](#bittorrent-bridge) مراجعه کنید. هر ستون دیگر در تمام ۸ زبان واقعی و کارآمد است.

تمام ۸ زبان بسته‌های سیم بایت‌به‌بایت یکسانی تولید می‌کنند که توسط ۱۷ fixture استاندارد فرمت سیم و ۶ بردار آزمون Signal تأیید می‌شوند (`fixtures/expected/*.bin`، `fixtures/signal/expected/*.json`) — هر زبان در برابر همان بایت‌ها بررسی می‌شود. مسیریابی (RREQ/RREP به‌سبک AODV)، DTN store-and-forward، پخش SOS، صدا، استریمینگ و سرویس‌های سخت‌گیری امنیتی در هر زبان با **~۳,۰۰۰ آزمون** در تمام ۸ پیاده‌سازی اجرا می‌شوند:

| زبان | آزمون‌ها | پلتفرم آزمون |
|----------|------:|-------------|
| C# (.NET 10) | ۵۳۰ | Linux |
| TypeScript / Node 20 | ۴۵۹ | Linux |
| Kotlin / JVM 21 | ۴۵۷ | Linux |
| Go 1.22 | ۴۲۳ | Linux |
| Python 3.12 | ۳۸۷ | Linux |
| Swift 6 | ۲۹۵ | macOS |
| C (GCC) | ۲۵۳ | Linux |
| Rust (stable) | ~۱۹۵ | Linux |
| **مجموع** | **~۳,۰۰۰** | |
...
فراتر از فرمت سیم و Signal، **کل مجموعه سرویس‌های سیم** — حضور، ضربان قلب، همگام‌سازی پروفایل، اعلام شناسه موقت، تبادل پیش‌کلید، کانال‌ها، فشار برای صحبت، اشتراک صفحه، کنترل تماس، تأیید SOS، رد پای فضا، اعلام forge، درخواست شارد vault، و اندازه‌گیری پهنای باند (به [**آنچه دریافت می‌کنید**](#what-you-get) مراجعه کنید) — به همین ترتیب در تمام ۸ زبان پیاده‌سازی شده و به fixture‌های خودش پین شده است (`fixtures/presence/`، `fixtures/media/`، `fixtures/bandwidth/`، `fixtures/prekey/`، `fixtures/videocall/`، `fixtures/vaultshard/` و خواهرها). هیچ ویژگی‌ای در لایه پروتکل فقط-C# نیست.
...
- فرمت سیم: بایت‌به‌بایت یکسان در ۸ زبان، لنگرانداخته‌شده با ۱۷ fixture استاندارد و ادعاهای بین‌زبانی (`fixtures/expected/*.bin`)
- **گردش‌کار GitHub Actions (تعریف‌شده، نه دروازه فعلی)** — یک ماتریس ۹-شغلی (C#/.NET 10، Go 1.22، TypeScript/Node 20، Python 3.12، Kotlin/JVM 21، Swift/macOS، Rust stable، C/GCC، به‌علاوه یک شغل یکپارچگی fixture) در `.github/workflows/ci.yml` تعریف شده است. کامیت‌ها در حال حاضر با `[skip ci]` push می‌شوند، بنابراین اعمال واقعی، اجرای مجموعه fixture به‌صورت **محلی، به‌ازای هر زبان** است (Swift و C روی سرور بیلد macOS)؛ CI می‌تواند بدون تغییر کد دوباره روشن شود.
...
- **~۳,۰۰۰ آزمون** در تمام ۸ زبان (C# 530، TypeScript 459، Kotlin 457، Go 423، Python 387، Swift 295، C 253، Rust ~195) — همه سبز، اجراشده به‌ازای هر زبان (Swift و C روی سرور بیلد macOS)
...
هشت — C#، Rust، TypeScript، Python، Go، Kotlin، Swift و C. هر پیاده‌سازی بسته‌های سیم بایت‌به‌بایت یکسانی تولید می‌کند که توسط یک مجموعه fixture مشترک بین‌زبانی اعمال می‌شود که هر پیاده‌سازی در برابر آن بررسی می‌شود، بنابراین بسته‌ای که توسط یک زبان ساخته شده بدون تغییر توسط هر زبان دیگری رمزگشایی می‌شود.
...
این README همچنین به زبان‌های دیگرِ فهرست‌شده در نوار زبان بالای این فایل، در [`docs/i18n/`](docs/i18n/) نگهداری می‌شود — که زبان‌های اروپایی، آسیای شرقی، خاورمیانه، آسیای جنوبی، آسیای جنوب شرقی و آفریقایی را در بر می‌گیرد، زیرا شبکه‌ای که برای افرادِ بدون داده ساخته شده نباید دری ورودی داشته باشد که فقط افرادِ با اتصالِ خوب بتوانند آن را بخوانند. **نسخه انگلیسی منبع حقیقت است**: جایی که یک ترجمه و متن انگلیسی با هم اختلاف دارند، متن انگلیسی معتبر است و ممکن است ترجمه‌ها یک یا دو نسخه از آن عقب باشند. پروتکل، کد، fixture‌ها و رفتار توصیف‌شده، مهم نیست کدام زبان را می‌خوانید، یکسان هستند.
