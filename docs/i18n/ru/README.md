...
**Столбец BitTorrent:** ✅ = полноценный рабочий клиент + mesh-шлюз (эталонная реализация C#). ◐ = **форматы проводного протокола** BitTorrent здесь побайтово идентичны (закреплены в `fixtures/bittorrent/`), а действующий сетевой уровень — следующий шаг; см. [BitTorrent — настоящий и встроенный в сеть через мост](#bittorrent-bridge). Все остальные столбцы реальны и работают на всех 8 языках.
