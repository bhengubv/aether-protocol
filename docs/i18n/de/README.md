...
Jeder Transport hat einen Farbnamen, der im gesamten Quellcode verwendet wird. `IsAvailable` sperrt hardwareblockierte Pfade — der `TransportManager` überspringt sie automatisch und fällt auf den nächsten verfügbaren Transport zurück.
...
| 🟣 Aether Purple | Cellular HTTP relay | Unbegrenzt | ~10 Mbps | ✅ Windows — Relay-Server in `samples/Aether.RelayServer/` |
...
Prioritätsreihenfolge im `TransportManager`: NearLink → BLE (≤ 1 KB) → Wi-Fi Direct → NFC → LoRa → HTTP Relay (letztes Mittel, `PowerCostRelative = 100`).
...
Aether läuft vollständig auf jedem Gerät mit Bluetooth- oder WLAN-Hardware. Wo ein Funk physisch fehlt, wird jeder blockierte Transport durch das Verfügbare angenähert:

- **NearLink (Aether Teal)** — Annäherung über BLE GATT mit der kanonischen Aether-SLE-Service-UUID (`61657468-6572-0003-0000-000000000000`). Die SSAP-Anwendungsprotokollschicht ist API-identisch zu GATT. Die Funkschicht (BPSK/QPSK/8PSK, Polar-Codes, 1–4 MHz-Kanäle) ist es nicht — Knoten, die die Standardstufe verwenden, können keine rohen Bytes mit echter NearLink-Hardware austauschen; sie arbeiten mit anderen Aether-Knoten der Standardstufe zusammen.
- **LoRa (Aether Red)** — Annäherung mit dem vollständigen Meshtastic-Leitungsformat über BLE 5.0 Coded PHY (S=8, ~1,3 km outdoor). Bridge-Node-Verbund mit echter LoRa-Hardware funktioniert automatisch — dasselbe Meshtastic-Paketformat reitet auf allen Hops ohne Übersetzung.
- **NFC (Aether White)** — Annäherung über NDEF-over-BLE-GATT mit einem RSSI-Näherungs-Gate (≥ −40 dBm ≈ 5–10 cm), das Tap-to-Connect-Semantik reproduziert. Der PC/SC-Pfad über USB-NFC-Lesegerät wird unter Windows ebenfalls unterstützt.

Alle anderen Fähigkeiten — BLE, Wi-Fi Direct, HTTP-Relay, Signal-Protokoll-Sicherheit (X3DH + Double Ratchet), AODV-Routing, DTN-Store-and-Forward, SOS-Broadcast, Sprache, Streaming — sind nativ und identisch zur nativen Stufe.

**Dies ist ein vollständig fähiger, produktionsreifer Einsatz.** Die meisten Apps beginnen hier.
...
- ✅ **C: vollständige Signal-Sitzung** — `aether_signal_service_init`, `generate_pre_key_bundle`, `process_pre_key_bundle`, `encrypt`, `decrypt` in `c/src/signal_protocol.c`; 6 Zwei-Knoten-E2E-Tests in `c/tests/test_signal_session.c`. Alle 8 Sprachen haben nun ein vollständiges sitzungsfähiges Signal-Protokoll.

**Erledigt (nur C#-Referenz):**
- ✅ **Demo-Schritt 9 — MessagingService + DTN-Fallback Ende-zu-Ende** — `samples/Aether.Demo.Console` führt durch echtes Signal-verschlüsseltes Messaging mit DTN-Store-and-Forward, wenn der Empfänger offline ist.
- ✅ **`Aether.Messaging` ↔ `Aether.Security`-Bridge** — `SignalMessageEnvelopeCipher` macht die Messaging-Schicht standardmäßig Ende-zu-Ende-verschlüsselt; Nachrichten ohne Signal-Sitzung werden in die Warteschlange gestellt, nie unsicher gesendet.
...
- ✅ **BLE-GATT-Echtzeit-Transport** — `WinBleGattTransportService` (Windows WinRT) + `android/blue/` (Android GATT-Server). Vollständiger RF-Bring-up-Test in `samples/Aether.BleRfTest/`.
- ✅ **Wi-Fi-Direct-Echtzeit-Transport** — `WinWifiDirectTransportService` (WinRT, `WiFiDirectAdvertisementPublisher` + TCP StreamSocket Port 8888) + `android/green/` (`WifiP2pManager`). RF-Test in `samples/Aether.WifiDirectRfTest/`.
- ✅ **HTTP-Relay-Transport (Aether Purple)** — `HttpRelayTransportService` mit 10-sekündigem Long-Poll, `PowerCostRelative = 100`, immer letztes Mittel. Relay-Server in `samples/Aether.RelayServer/` (ASP.NET Core Minimal-API, Port 5200). RF-Test in `samples/Aether.RelayRfTest/`.
...
aether-protocol/
  src/
    Aether.Core/          Protokollmodelle, Konstanten, Paketserialisierung
    Aether.Security/      Signal-Protokoll, Ed25519, Paketsignierung
    Aether.Transport/     Transport-Abstraktionen, NearLink, In-Process-Simulator
    Aether.Messaging/     Nachrichtenbehandlung und -weiterleitung
    Aether.Storage/       DTN-Store-and-Forward-Persistenz
    Aether.Streaming/     Adaptives Bitrate-Streaming, Videomodelle und -schnittstellen
    Aether.Voice/         Sprachanrufe und Gruppensprache
    Aether.Content/       Inhaltsverifizierung und segmentierter Transfer
  samples/
    Aether.Demo.Console/  Interaktive Demo
  tests/
...
- `IAetherIncentiveProvider` — Knoten belohnen, die Traffic weiterleiten (Noop-Standard: altruistische Weiterleitung)
- `IAetherBackendClient` — Mit einem Server synchronisieren, wenn Internet verfügbar ist (Noop-Standard: vollständig offline)
- `IAetherFeatureFlagProvider` — Protokollfunktionen zur Laufzeit umschalten (Noop-Standard: alles aktiviert)
