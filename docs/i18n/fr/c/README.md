...
#### Gestion des paquets
- `aether_mesh_packet_t *aether_packet_new(void)` — Créer un nouveau paquet
- `void aether_packet_free(aether_mesh_packet_t *packet)` — Libérer un paquet
- `aether_mesh_packet_t *aether_packet_clone(const aether_mesh_packet_t *packet)` — Cloner un paquet

#### Sérialisation
- `int aether_packet_serialize(const aether_mesh_packet_t *packet, uint8_t *buffer, size_t buffer_len)` — Sérialiser au format filaire
- `aether_mesh_packet_t *aether_packet_deserialize(const uint8_t *data, size_t data_len)` — Désérialiser depuis le format filaire
- `size_t aether_packet_estimate_size(const aether_mesh_packet_t *packet)` — Estimer la taille filaire

#### Champs du paquet
- `bool aether_packet_set_source_uhid(aether_mesh_packet_t *packet, const char *uhid)` — Définir la source
- `bool aether_packet_set_destination_uhid(aether_mesh_packet_t *packet, const char *uhid)` — Définir la destination
- `bool aether_packet_set_payload(aether_mesh_packet_t *packet, const uint8_t *data, size_t len)` — Définir la charge utile
- `bool aether_packet_set_signature(aether_mesh_packet_t *packet, const uint8_t *sig, size_t len)` — Définir la signature

#### Validation
- `bool aether_packet_is_expired(const aether_mesh_packet_t *packet, int max_age_seconds)` — Vérifier si le paquet est expiré
- `bool aether_packet_can_forward(const aether_mesh_packet_t *packet)` — Vérifier si TTL > 0

#### Données de signature
- `uint8_t *aether_packet_get_signable_data(const aether_mesh_packet_t *packet, size_t *out_len)` — Obtenir les octets signables déterministes (l'appelant doit libérer la mémoire)

### Sécurité

#### Ed25519
- `bool aether_ed25519_generate_keypair(uint8_t *out_private, uint8_t *out_public)` — Générer des clés de 32+32 octets
- `bool aether_ed25519_sign(const uint8_t *private_key, const uint8_t *data, size_t data_len, uint8_t *out_signature)` — Signer (produit 64 octets)
- `bool aether_ed25519_verify(const uint8_t *public_key, const uint8_t *data, size_t data_len, const uint8_t *signature)` — Vérifier

#### AES-256-GCM
- `bool aether_aes256_gcm_encrypt(const uint8_t *plaintext, size_t plaintext_len, const uint8_t *key, const uint8_t *nonce, const uint8_t *aad, size_t aad_len, uint8_t *out_ciphertext, uint8_t *out_tag, uint8_t *out_nonce)` — Chiffrer (nonce auto-généré si NULL)
- `bool aether_aes256_gcm_decrypt(const uint8_t *ciphertext, size_t ciphertext_len, const uint8_t *key, const uint8_t *nonce, const uint8_t *tag, const uint8_t *aad, size_t aad_len, uint8_t *out_plaintext)` — Déchiffrer

#### HMAC et hachage
- `bool aether_hmac_sha256(const uint8_t *key, size_t key_len, const uint8_t *data, size_t data_len, uint8_t *out_hash)` — HMAC-SHA256 (32 octets)
- `bool aether_sha256(const uint8_t *data, size_t data_len, uint8_t *out_hash)` — SHA-256 (32 octets)
...
#### Utilitaires
- `void aether_zeroize(void *mem, size_t len)` — Effacement mémoire en temps constant
- `bool aether_random_bytes(uint8_t *out, size_t len)` — Octets aléatoires cryptographiquement sûrs

### Transport

#### Fonctions génériques
- `bool aether_transport_send(aether_transport_t *transport, const char *peer_uhid, const uint8_t *data, size_t data_len)` — Envoyer des données
- `bool aether_transport_is_connected(aether_transport_t *transport, const char *peer_uhid)` — Vérifier la connexion
- `void aether_transport_set_on_data_received(aether_transport_t *transport, aether_transport_on_data_received callback, void *user_data)` — Enregistrer un rappel
- `void aether_transport_destroy(aether_transport_t *transport)` — Nettoyage

#### Transport en cours de processus
- `aether_transport_t *aether_inprocess_transport_new(void)` — Créer un transport en cours de processus partagé
- `bool aether_inprocess_transport_register_node(aether_transport_t *transport, const char *uhid)` — Enregistrer un nœud
- `bool aether_inprocess_transport_unregister_node(aether_transport_t *transport, const char *uhid)` — Désenregistrer un nœud
...
- Paquet maximal : 65 Ko (configurable via `AETHER_MAX_PAYLOAD_LEN`)
...
- **AetherAPI** (C#) — relais maillé côté serveur et analytique
- **Aether.Core** (C#) — implémentation de référence (format filaire interopérable)
...
- Référence C# : `/Users/admin/Code/Dev/aether-protocol/src/Aether.Core/`
