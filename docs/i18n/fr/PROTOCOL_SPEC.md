...
> - Sérialiseur de référence : `src/Aether.Core/Protocol/PacketSerializer.cs`
> - Pile Signal de référence : `src/Aether.Security/Services/SignalProtocolService.cs`
> - Routage de référence : `src/Aether.Core/Routing/RoutingService.cs`
> - DTN de référence : `src/Aether.Core/Dtn/DtnService.cs`
...
> Réconcilié le 2026-05-05 avec `src/Aether.Core/Protocol/PacketSerializer.cs`
...
L'implémentation de référence se trouve à `src/Aether.Security/Services/
...
> `src/Aether.Security/Services/SignalProtocolService.cs` et le corpus de
...
Référence : `Aether.Security.Models.PreKeyBundle`. La forme de contrat de fil est
...
de la réserve est exercée par `tests/Aether.Core.Tests/PreKeyPoolTests.cs`.
...
Référence : `Aether.Security.Models.EncryptedPayload` (lignes 55–66 de
...
| C           | primitives uniquement — `aether_x25519_*`, `aether_signal_kdf_rk` | non implémenté | — | kdf_rk_basic uniquement |
...
5. **Repli :** Si aucun transport local ne peut atteindre la cible, le paquet est mis en file d'attente pour relais serveur via AetherAPI.
...
3. **Envoi en double chemin :** Le SOS est envoyé simultanément via :
   ...
   - **Appel API :** Envoyé à AetherAPI pour la distribution côté serveur et la liaison à PanikAPI (envoi de SMS/e-mail).
...
3. **Tentative de relais serveur :** Si le routage maillé échoue, l'expéditeur tente de relayer via AetherAPI. Si le serveur peut atteindre le destinataire (ou mettre le message en file), la livraison réussit.
...
4. Le reçu est également synchronisé vers AetherAPI à des fins d'analyse.
...
> Le module C# `Aether.Streaming` embarque des interfaces, des modèles et des services
...
> testés par fixture. `Aether.Streaming.WatchTogetherService` fournit le
...
|------------|--------|-------------|
| AETHER_VIDEO_CALL | AETHER_VOICE | Appels vidéo P2P et de groupe |
| AETHER_VIDEO_GROUP | AETHER_VIDEO_CALL | Sessions vidéo multi-parties |
| AETHER_SCREEN_SHARE | AETHER_VIDEO_CALL | Partage d'écran dans les appels vidéo |
| AETHER_WATCH_TOGETHER | AETHER_CONTENT_P2P | Lecture multimédia synchronisée |
| AETHER_WATCH_REACTIONS | AETHER_WATCH_TOGETHER | Réactions emoji et vocales |
| AETHER_TORRENT_INGEST | AETHER_CONTENT_P2P | Acceptation de fichiers BitTorrent pour la distribution maillée |
