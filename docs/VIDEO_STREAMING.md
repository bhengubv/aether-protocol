...
All packet types are defined in `AetherNet.Protocol.PacketType` (open-source) and `TheGeekNetwork.Shared.AetherNet.Protocol.Models.PacketType` (private).
...
|----------|-------------|-------------------|---------------|
| `AETHERNET_VIDEO_CALL` | Video Calling | `AETHERNET_VOICE` | P2P video calls, codec negotiation, capability detection |
| `AETHERNET_VIDEO_GROUP` | Group Video | `AETHERNET_VIDEO_CALL` | Multi-party video sessions, FullMesh/SFU topology |
| `AETHERNET_SCREEN_SHARE` | Screen Sharing | `AETHERNET_VIDEO_CALL` | Screen capture frame sharing in calls |
| `AETHERNET_WATCH_TOGETHER` | Watch Together | `AETHERNET_CONTENT_P2P` | Synchronized media playback sessions |
| `AETHERNET_WATCH_REACTIONS` | Watch Reactions | `AETHERNET_WATCH_TOGETHER` | Emoji and voice reactions during sessions |
| `AETHERNET_TORRENT_INGEST` | BitTorrent Ingest | `AETHERNET_CONTENT_P2P` | Accept torrent files for watch-together |

**Parent dependency** means the parent flag must be enabled before the child flag can be activated. For example, `AETHERNET_VIDEO_GROUP` requires `AETHERNET_VIDEO_CALL` which in turn requires `AETHERNET_VOICE`.

---

## Database Schema (AetherNetAPI)
...
AetherNetAPI exposes 17 endpoints across two endpoint groups, all requiring `NodeAuth` authorization.
...
// aether-protocol/src/AetherNet.Streaming/Models/VideoModels.cs
...
All video services are registered in `AetherNetExtensions.AddAetherNetLink()`:
...
4. If the issue persists, inspect the `chip_in_pools` and `chip_in_contributions` tables in the `aethernet_db` database
...
| `010_VideoWatchTogether.sql` | `AetherNetAPI/Migrations/` | 8 PostgreSQL tables + 6 feature flags |
| `VideoEndpoints.cs` | `AetherNetAPI/Endpoints/` | 8 video API endpoints |
| `WatchEndpoints.cs` | `AetherNetAPI/Endpoints/` | 9 watch-together API endpoints |
| `AetherNetExtensions.cs` | `Shared.Aether/DependencyInjection/` | DI registration for all video services |
