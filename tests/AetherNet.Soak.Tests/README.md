...
The standard unit suite (`tests/AetherNet.Core.Tests`) is unaffected — soak
...
```sh
dotnet test tests/AetherNet.Soak.Tests/AetherNet.Soak.Tests.csproj -c Release --nologo
```

Long-form soak (CI / nightly):

```sh
AETHERNET_SOAK_ITERATIONS=100000 dotnet test tests/AetherNet.Soak.Tests/... -c Release --nologo
```

On Windows PowerShell:

```powershell
$env:AETHERNET_SOAK_ITERATIONS = 100000
dotnet test tests/AetherNet.Soak.Tests/AetherNet.Soak.Tests.csproj -c Release --nologo
```
...
```sh
dotnet test tests/AetherNet.Soak.Tests/... --filter "Category=Soak"
```
...
`AETHERNET_SOAK_ITERATIONS` overrides the per-test default. Some tests cap at
