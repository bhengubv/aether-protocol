...
/// by <see cref="AetherMeshTelemetry"/>:
///   <list type="bullet">
...
/// global <see cref="AetherMeshTelemetry.Meter"/> and
/// <see cref="AetherMeshTelemetry.ActivitySource"/> remain in their unsubscribed
/// (zero-overhead) state for subsequent tests.
/// </summary>
public class AetherMeshTelemetryTests
{
    ...
    public async Task Counters_Increment_On_Encrypt_And_Decrypt()
    {
        ...
        using var listener = new MeterListener
        {
            ...
            {
                if (instrument.Meter.Name == AetherMeshTelemetry.MeterName
        ...
        Assert.True(observed.GetValueOrDefault("aethermesh.messages.encrypted") >= 1,
            "MessagesEncrypted counter did not increment");
        Assert.True(observed.GetValueOrDefault("aethermesh.messages.decrypted") >= 1,
            "MessagesDecrypted counter did not increment");
        Assert.True(observed.GetValueOrDefault("aethermesh.sessions.established") >= 1,
    ...
    public async Task Histograms_Record_NonZero_Latency()
    {
        ...
        var encryptSamples = samples.Where(s => s.Name == "aethermesh.encrypt.latency").ToArray();
        var decryptSamples = samples.Where(s => s.Name == "aethermesh.decrypt.latency").ToArray();
    ...
    public async Task ActivitySource_Produces_Expected_Operation_Names()
    {
        ...
        {
            ShouldListenTo = source => source.Name == AetherMeshTelemetry.ActivitySourceName,
        ...
        Assert.Contains("AetherMesh.X3DH.Initiator", names);
        Assert.Contains("AetherMesh.X3DH.Responder", names);
        Assert.Contains("AetherMesh.Encrypt", names);
        Assert.Contains("AetherMesh.Decrypt", names);
    }
    ...
    public async Task Activity_Tags_Are_Sanitised_Never_Raw_UHID()
    {
        ...
        var sanitised = AetherMeshTelemetry.SanitizeUhid(AliceUhid);
    ...
    public async Task NoListener_HotPath_Does_Not_Throw_Or_Allocate_Activity()
    {
        ...
        Assert.False(AetherMeshTelemetry.ActivitySource.HasListeners(),
    ...
    {
        Assert.Equal("AetherMesh.Protocol", AetherMeshTelemetry.MeterName);
        Assert.Equal("AetherMesh.Protocol", AetherMeshTelemetry.ActivitySourceName);
        Assert.Equal("AetherMesh.Protocol", AetherMeshTelemetry.Meter.Name);
        Assert.Equal("AetherMesh.Protocol", AetherMeshTelemetry.ActivitySource.Name);
    }
