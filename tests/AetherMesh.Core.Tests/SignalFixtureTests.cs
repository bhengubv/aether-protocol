...
// set the env var AETHERMESH_SIGNAL_FIXTURES_REGEN=1 — the test prints expected
...
public class SignalFixtureTests
{
    ...
    private static bool RegenerateMode =>
        Environment.GetEnvironmentVariable("AETHERMESH_SIGNAL_FIXTURES_REGEN") == "1";
    ...
    private static string ResolveFixturePath(string relativePath)
    {
        ...
        while (dir != null && !File.Exists(Path.Combine(dir.FullName, "AetherMeshProtocol.slnx")))
            dir = dir.Parent;
        if (dir == null)
