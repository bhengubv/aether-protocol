...
/// <list type="bullet">
///   <item>Iteration count resolution: env var <c>AETHERMESH_SOAK_ITERATIONS</c>
...
/// <c>dotnet test tests/AetherMesh.Soak.Tests/...</c> picks them up.
/// </summary>
public abstract class SoakTestBase
{
    /// <summary>
    /// Default iteration budget. Aimed at &lt; 30 s on a developer laptop;
    /// CI bumps via <c>AETHERMESH_SOAK_ITERATIONS</c> for true soak runs.
    /// </summary>
    ...
    {
        var raw = Environment.GetEnvironmentVariable("AETHERMESH_SOAK_ITERATIONS");
