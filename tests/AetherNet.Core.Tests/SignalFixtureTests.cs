...
// set the env var AETHERNET_SIGNAL_FIXTURES_REGEN=1 — the test prints expected
...
public class SignalFixtureTests
{
    ...
    private static bool RegenerateMode =>
        Environment.GetEnvironmentVariable("AETHERNET_SIGNAL_FIXTURES_REGEN") == "1";
    ...
    private static byte[] X25519AgreeViaService(byte[] priv, byte[] pub)
    {
        var t = typeof(SignalProtocolService).Assembly
            .GetType("AetherNet.Security.Services.X25519Service")
    ...
    private static string ResolveFixturePath(string relativePath)
    {
        ...
        while (dir != null && !File.Exists(Path.Combine(dir.FullName, "AetherNetProtocol.slnx")))
            dir = dir.Parent;
        if (dir == null)
