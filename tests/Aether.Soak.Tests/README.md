...
The standard unit suite (`tests/Aether.Core.Tests`) is unaffected — soak
...
```sh
dotnet test tests/Aether.Soak.Tests/Aether.Soak.Tests.csproj -c Release --nologo
```

Long-form soak (CI / nightly):

```sh
AETHER_SOAK_ITERATIONS=100000 dotnet test tests/Aether.Soak.Tests/... -c Release --nologo
```

On Windows PowerShell:

```powershell
$env:AETHER_SOAK_ITERATIONS = 100000
dotnet test tests/Aether.Soak.Tests/Aether.Soak.Tests.csproj -c Release --nologo
```
...
```sh
dotnet test tests/Aether.Soak.Tests/... --filter "Category=Soak"
```
...
`AETHER_SOAK_ITERATIONS` overrides the per-test default. Some tests cap at
