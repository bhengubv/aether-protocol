...
/// global <see cref="AetherTelemetry.Meter"/> and
/// <see cref="AetherTelemetry.ActivitySource"/> remain in their unsubscribed
/// (zero-overhead) state for subsequent tests.
/// </summary>
public class AetherTelemetryTests
{
    ...
    public async Task Counters_Increment_On_Encrypt_And_Decrypt()
    {
        ...
        using var listener = new MeterListener
        {
            ...
            {
                if (instrument.Meter.Name == AetherTelemetry.MeterName
        ...
        Assert.True(observed.GetValueOrDefault("aether.messages.encrypted") >= 1,
            "MessagesEncrypted counter did not increment");
        Assert.True(observed.GetValueOrDefault("aether.messages.decrypted") >= 1,
            "MessagesDecrypted counter did not increment");
        Assert.True(observed.GetValueOrDefault("aether.sessions.established") >= 1,
    ...
    public async Task Histograms_Record_NonZero_Latency()
    {
        ...
        var encryptSamples = samples.Where(s => s.Name == "aether.encrypt.latency").ToArray();
        var decryptSamples = samples.Where(s => s.Name == "aether.decrypt.latency").ToArray();
    ...
    public async Task ActivitySource_Produces_Expected_Operation_Names()
    {
        ...
        {
            ShouldListenTo = source => source.Name == AetherTelemetry.ActivitySourceName,
        ...
        Assert.Contains("Aether.X3DH.Initiator", names);
        Assert.Contains("Aether.X3DH.Responder", names);
        Assert.Contains("Aether.Encrypt", names);
        Assert.Contains("Aether.Decrypt", names);
    }
    ...
    public async Task Activity_Tags_Are_Sanitised_Never_Raw_UHID()
    {
        ...
        var sanitised = AetherTelemetry.SanitizeUhid(AliceUhid);
    ...
    public async Task NoListener_HotPath_Does_Not_Throw_Or_Allocate_Activity()
    {
        ...
        Assert.False(AetherTelemetry.ActivitySource.HasListeners(),
    ...
    {
        Assert.Equal("Aether.Protocol", AetherTelemetry.MeterName);
        Assert.Equal("Aether.Protocol", AetherTelemetry.ActivitySourceName);
        Assert.Equal("Aether.Protocol", AetherTelemetry.Meter.Name);
        Assert.Equal("Aether.Protocol", AetherTelemetry.ActivitySource.Name);
    }
