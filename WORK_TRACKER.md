...
- [x] **E2 · stable UHID off the wire, rotating ERID on** — `EridHeaderCodec` (Core, 5 tests) is the mechanism; the live wiring is in `ChatService.Wrap` (outbound swap) / `OnPacket` (inbound resolve) via `CircleDirectory` (`MyAddress`/`AddressFor`/`Recognise`), gated on holding the peer's routing key (the de-facto `erid-routing` capability — the key only arrives sealed inside the session, so an un-upgraded peer never triggers it and the key-share itself stays on the stable tag). **Verified on P30↔merlin over Wi-Fi Direct**: the wire source read `PV3ZVV012H2ANFS8` (a 16-char rotating ERID, not the tag), resolved back to the stable tag on receipt, delivery intact both ways. **All lanes:** the same swap is also at the Android transport chokepoint (`AndroidRadioMesh.HideIdentity`/`RevealIdentity`, gated + bootstrap-exempt), so voice/attachments/group calls hide the UHID too (chat regression re-verified on hardware after). The web head's transport would need the equivalent (a follow-on). · **M · DONE (verified on hardware, all Android lanes)**
...
- [~] **H1 · Local-account store (SQLite)** — single-row SQLite `identity` store + `IdentityService`, now with a persisted **account object**: display-name + avatar + recovery-backed-up state as migration-safe columns on the identity row (`Account` / `GetAccount` / `SaveAccount` / `SetRecoveryBackedUp` in `AetherStore.cs`), surfaced as an editable display name in Settings — was a hard-coded "You" held only in the page. 3 store tests (incl. survives-a-reopen); verified on the web head (typed a name, reloaded, persisted). Remaining gap: no **online** account — the **DECIDE · email/mobile placement** gate. `samples/…/AetherStore.cs`, `Pages/Settings.razor`. · **M · PARTIAL (local account object DONE; online acct DECIDE-gated)**
...
- [x] **H4 · Device revocation + PanicWipe pairing** — device revocation **built this session**: `AetherNet.Security.Sync.DeviceRevocation` + `DeviceRevocationCodec` (signed inverse of `DeviceLink`, domain-separated so a link can't be replayed as a revocation) + `RevocationSet` (admits only validly-signed revocations). Pairs with the existing `PanicWipe`: PanicWipe erases the **local** device under duress, `DeviceRevocation` (gossiped via `SyncRecord`) invalidates a device you **no longer hold**. 7 tests. `PanicWipe`'s app-level trigger is **now wired**: `PanicWipeService` (arm a duress PIN → `TrySubmit(pin)` triggers, or `Wipe()` for a panic button) destroys the identity key via a new `ISecretVault.Remove` — targeting the app's real key name `aether.node.identity`, not only the protocol's canonical list — and the whole local DB via `AetherStore.WipeAll` (+VACUUM). 8 tests. No lock screen needed: the trigger is a seam any surface calls; a PIN *UI* stays the H2 product decision. `samples/…/Services/PanicWipeService.cs`. · **S · DONE**
