...
> **External contributions are not open yet.** The protocol is still under active development and external pull requests are not being accepted at this time. This guide documents the workflow and the areas where help will be wanted once a public contribution window is announced -- check back then, or see the README's *Contributing* section. Security issues are the exception: report them privately per [SECURITY.md](SECURITY.md) at any time.
...
- **NAT traversal beyond host candidates** -- The WebRTC transport is serverless by default (host-candidate-only ICE); STUN/TURN are optional. Harden and test NAT-traversal paths for networks where host candidates alone can't connect.

The Signal Protocol core -- X3DH, the full Double Ratchet, and the one-time pre-key pool -- is already complete and fixture-verified in all 8 languages; it is not an open item. The remaining open work is physical-radio (RF) bring-up, tracked in [`OPEN_ISSUES.md`](OPEN_ISSUES.md).
