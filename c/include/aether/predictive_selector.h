...
//
// Extends aether_transport_rank() by replacing the EWMA RTT term with a
...
//
// Thread safety: spin-lock inside aether_predictive_selector_t.

#ifndef AETHER_PREDICTIVE_SELECTOR_H
#define AETHER_PREDICTIVE_SELECTOR_H
...
#define AETHER_PREDICTIVE_MAX_TRANSPORTS 32
 ...
 */
typedef struct {
    aether_transport_t           *transport; /**< Non-owning pointer. NULL = empty slot. */
    aether_kalman_rtt_filter_t    filter;
} aether_predictive_entry_t;

/**
 * Predictive transport selector.
 *
 * Fixed-size (AETHER_PREDICTIVE_MAX_TRANSPORTS slots).  All mutation is
 * protected by an embedded spin-lock so the selector is safe to share
 * across threads.
 *
 * Initialise with aether_predictive_selector_init() before use.
 */
typedef struct {
    volatile int              lock;    /**< Spin-lock: 0 = unlocked, 1 = locked. */
    size_t                    count;   /**< Number of registered (non-NULL) entries. */
    aether_predictive_entry_t entries[AETHER_PREDICTIVE_MAX_TRANSPORTS];
} aether_predictive_selector_t;

/**
 * One entry returned by aether_predictive_selector_rank().
 */
typedef struct {
    aether_transport_t *transport;       /**< Non-owning pointer to the transport.  */
...
} aether_predictive_rank_entry_t;

// ── Lifecycle ─────────────────────────────────────────────────────────────────

/**
 * Initialise a selector struct.  Must be called before any other
 * aether_predictive_selector_* functions.
 *
 ...
 * @param out_ranked   Caller-allocated array of at least AETHER_PREDICTIVE_MAX_TRANSPORTS
...
#endif /* AETHER_PREDICTIVE_SELECTOR_H */
