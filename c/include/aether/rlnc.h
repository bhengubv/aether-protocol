...
// Components
// ──────────
//   aether_gf256_*          — GF(2⁸) log/exp tables and arithmetic helpers.
//   aether_rlnc_encoder_t   — systematic + random-repair packet generation.
//   aether_rlnc_decoder_t   — incremental Gauss-Jordan elimination.
//   aether_rlnc_codec_t     — aether_fec_codec_t vtable adapter.
//
// Wire format per packet:
//   [ K coefficient bytes ][ symbolSize data bytes ]
//
// Thread safety:
//   aether_gf256_init() is idempotent and thread-safe via a spin-lock once-flag.
...
//   aether_rlnc_codec_t (encode/try_decode) IS thread-safe: the codec itself
//   is stateless; all mutable state lives in per-call encoder/decoder locals.

#ifndef AETHER_RLNC_H
#define AETHER_RLNC_H

#include <stdint.h>
#include <stdbool.h>
#include <stddef.h>

#include "transport.h"  /* aether_fec_codec_t */
 ...
 * aether_rlnc_encoder_new(), aether_rlnc_decoder_new(), and
 * aether_rlnc_codec_new().  Only call explicitly if you need the arithmetic
 ...
 *
 * @pre aether_gf256_init() must have been called.
 */
 ...
 *
 * @pre a != 0; aether_gf256_init() must have been called.
 ...
 */
static inline uint8_t aether_gf256_add(uint8_t a, uint8_t b) { return (uint8_t)(a ^ b); }
 ...
 *
 * Each call to aether_rlnc_encoder_next_packet() advances the internal
 * counter by one.
 *
 * Ownership: caller owns the struct; free with aether_rlnc_encoder_free().
 ...
 *
 * Ownership: caller owns the struct; free with aether_rlnc_decoder_free().
 */
typedef struct {
...
// ── RlncCodec : aether_fec_codec_t ───────────────────────────────────────────

/**
 * RLNC FEC codec struct.
 *
 * The embedded aether_fec_codec_t is the first member so aether_rlnc_codec_t *
 * can be freely cast to aether_fec_codec_t * and passed to the generic
 * transport layer.
 *
 * Each encoded packet = [ K coefficient bytes ][ symbolSize data bytes ].
 */
typedef struct {
    aether_fec_codec_t base;  /**< Must be first — vtable + metadata.      */
