...
#ifndef AETHER_SIGNAL_PROTOCOL_H
#define AETHER_SIGNAL_PROTOCOL_H
 ...
 *
 * Stored in a flat array bounded by AETHER_SIGNAL_MAX_SKIPPED; the oldest
...
typedef struct {
    ...
    char peer_uhid[AETHER_MAX_UHID_LEN + 1];
    ...
    aether_skipped_key_entry_t skipped[AETHER_SIGNAL_MAX_SKIPPED];
    int skipped_count;
} aether_signal_session_t;
 ...
 * AETHER_SIGNAL_MAX_SESSIONS concurrent peer sessions.
 *
...
typedef struct {
    ...
    char local_uhid[AETHER_MAX_UHID_LEN + 1];
    ...
    aether_opk_entry_t opks[AETHER_SIGNAL_OPK_POOL_SIZE];
    /** Next OPK id to assign (auto-incremented). */
    int32_t opk_next_id;

    // ── Peer sessions ────────────────────────────────────────────────────
    aether_signal_session_t sessions[AETHER_SIGNAL_MAX_SESSIONS];
    int session_count;
} aether_signal_service_t;
...
typedef struct {
    ...
    char uhid[AETHER_MAX_UHID_LEN + 1];
 ...
 *
 * ciphertext is heap-allocated; call aether_signal_message_free() when done.
 */
typedef struct {
    /** AETHER_SIGNAL_MSG_TYPE_NORMAL or AETHER_SIGNAL_MSG_TYPE_PRE_KEY. */
    ...
    // ── PreKey header (message_type == AETHER_SIGNAL_MSG_TYPE_PRE_KEY) ───
 ...
 * the Ed25519 key), and fills the OPK pool with AETHER_SIGNAL_OPK_POOL_SIZE
 * fresh one-time pre-keys with IDs 1..AETHER_SIGNAL_OPK_POOL_SIZE.
 *
 ...
 *                   aether_signal_message_free when done).
 * @returns          true on success.
 */
...
#endif /* AETHER_SIGNAL_PROTOCOL_H */
