...
//
// Nodes broadcast signed AetherNetReputationUpdatePayload packets (type 52)
...
#define AETHERNET_PACKET_TYPE_REPUTATION_UPDATE 52

/** Maximum age of an accepted gossip packet (5 minutes). */
#define AETHERNET_GOSSIP_FRESHNESS_MS (5 * 60 * 1000)
...
typedef struct {
    ...
    char    reason[128];
} AetherNetReputationUpdatePayload;

/**
 * Callbacks the gossip service needs from the host application.
 * All function pointers must be non-NULL when passed to aethernet_gossip_create.
 */
typedef struct {
    ...
    bool (*verify_packet)(const char *json_packet,
                          ...
                          void *ctx);
    void *verify_ctx;
} AetherNetGossipCallbacks;

/** Opaque gossip service handle. */
typedef struct AetherNetGossipService AetherNetGossipService;
