...
// Components
// ──────────
//   aethermesh_gf256_*          — GF(2⁸) log/exp tables and arithmetic helpers.
//   aethermesh_rlnc_encoder_t   — systematic + random-repair packet generation.
//   aethermesh_rlnc_decoder_t   — incremental Gauss-Jordan elimination.
//   aethermesh_rlnc_codec_t     — aethermesh_fec_codec_t vtable adapter.
//
// Wire format per packet:
//   [ K coefficient bytes ][ symbolSize data bytes ]
//
// Thread safety:
//   aethermesh_gf256_init() is idempotent and thread-safe via a spin-lock once-flag.
...
//   aethermesh_rlnc_codec_t (encode/try_decode) IS thread-safe: the codec itself
//   is stateless; all mutable state lives in per-call encoder/decoder locals.

#ifndef AETHERMESH_RLNC_H
#define AETHERMESH_RLNC_H

#include <stdint.h>
#include <stdbool.h>
#include <stddef.h>

#include "transport.h"  /* aethermesh_fec_codec_t */
 ...
 * aethermesh_rlnc_encoder_new(), aethermesh_rlnc_decoder_new(), and
 * aethermesh_rlnc_codec_new().  Only call explicitly if you need the arithmetic
 ...
 *
 * @pre aethermesh_gf256_init() must have been called.
 */
 ...
 *
 * @pre a != 0; aethermesh_gf256_init() must have been called.
 ...
 */
static inline uint8_t aethermesh_gf256_add(uint8_t a, uint8_t b) { return (uint8_t)(a ^ b); }
 ...
 *
 * Each call to aethermesh_rlnc_encoder_next_packet() advances the internal
 * counter by one.
 *
 * Ownership: caller owns the struct; free with aethermesh_rlnc_encoder_free().
 ...
 *
 * Ownership: caller owns the struct; free with aethermesh_rlnc_decoder_free().
 */
typedef struct {
...
// ── RlncCodec : aethermesh_fec_codec_t ───────────────────────────────────────────

/**
 * RLNC FEC codec struct.
 *
 * The embedded aethermesh_fec_codec_t is the first member so aethermesh_rlnc_codec_t *
 * can be freely cast to aethermesh_fec_codec_t * and passed to the generic
 * transport layer.
 *
 * Each encoded packet = [ K coefficient bytes ][ symbolSize data bytes ].
 */
typedef struct {
    aethermesh_fec_codec_t base;  /**< Must be first — vtable + metadata.      */
    int                k;     /**< Generation size K.  Range: [1, 255].    */
} aethermesh_rlnc_codec_t;
