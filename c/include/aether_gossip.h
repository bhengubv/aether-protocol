...
//
// Nodes broadcast signed AetherReputationUpdatePayload packets (type 52)
...
#ifndef AETHER_GOSSIP_H
#define AETHER_GOSSIP_H
...
#define AETHER_PACKET_TYPE_REPUTATION_UPDATE 52

/** Maximum age of an accepted gossip packet (5 minutes). */
#define AETHER_GOSSIP_FRESHNESS_MS (5 * 60 * 1000)
...
typedef struct {
    ...
    char    reason[128];
} AetherReputationUpdatePayload;

/**
 * Callbacks the gossip service needs from the host application.
 * All function pointers must be non-NULL when passed to aether_gossip_create.
 */
typedef struct {
    ...
    bool (*verify_packet)(const char *json_packet,
                          ...
                          void *ctx);
    void *verify_ctx;
} AetherGossipCallbacks;

/** Opaque gossip service handle. */
typedef struct AetherGossipService AetherGossipService;
