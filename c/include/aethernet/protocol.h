...
typedef enum {
    ...
    // EridAnnounce -- directed transport of an already-Signal-encrypted ERID announcement (the
    // plaintext is an EridAnnouncementCodec frame sharing the sender's routing key). Wire byte 56
    // matches the C# PacketType.EridAnnounce so an announcement hop is byte-identical across
    // languages; the body is opaque (this layer never frames or encrypts). An un-upgraded node
    // drops the unknown type. Carried directly by aethernet_erid_announce_service (c/src/erid_announce.c).
    AETHERNET_PACKET_TYPE_ERID_ANNOUNCE         = 56,
