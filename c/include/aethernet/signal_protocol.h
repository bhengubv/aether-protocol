...
#ifndef AETHERNET_SIGNAL_PROTOCOL_H
#define AETHERNET_SIGNAL_PROTOCOL_H
 ...
 *
 * Stored in a flat array bounded by AETHERNET_SIGNAL_MAX_SKIPPED; the oldest
...
typedef struct {
    ...
    char peer_uhid[AETHERNET_MAX_UHID_LEN + 1];
    ...
    aethernet_skipped_key_entry_t skipped[AETHERNET_SIGNAL_MAX_SKIPPED];
    int skipped_count;
} aethernet_signal_session_t;
 ...
 * AETHERNET_SIGNAL_MAX_SESSIONS concurrent peer sessions.
 *
...
typedef struct {
    ...
    char local_uhid[AETHERNET_MAX_UHID_LEN + 1];
    ...
    aethernet_opk_entry_t opks[AETHERNET_SIGNAL_OPK_POOL_SIZE];
    /** Next OPK id to assign (auto-incremented). */
    int32_t opk_next_id;

    // ── Peer sessions ────────────────────────────────────────────────────
    aethernet_signal_session_t sessions[AETHERNET_SIGNAL_MAX_SESSIONS];
    int session_count;
} aethernet_signal_service_t;
...
typedef struct {
    ...
    char uhid[AETHERNET_MAX_UHID_LEN + 1];
 ...
 *
 * ciphertext is heap-allocated; call aethernet_signal_message_free() when done.
 */
typedef struct {
    /** AETHERNET_SIGNAL_MSG_TYPE_NORMAL or AETHERNET_SIGNAL_MSG_TYPE_PRE_KEY. */
    ...
    // ── PreKey header (message_type == AETHERNET_SIGNAL_MSG_TYPE_PRE_KEY) ───
 ...
 * the Ed25519 key), and fills the OPK pool with AETHERNET_SIGNAL_OPK_POOL_SIZE
 * fresh one-time pre-keys with IDs 1..AETHERNET_SIGNAL_OPK_POOL_SIZE.
 *
 ...
 *                   aethernet_signal_message_free when done).
 * @returns          true on success.
 */
...
#endif /* AETHERNET_SIGNAL_PROTOCOL_H */
