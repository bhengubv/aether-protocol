...
#ifndef AETHERNET_SECURITY_H
#define AETHERNET_SECURITY_H

#include <stdint.h>
#include <stdbool.h>
#include <stddef.h>
#include "constants.h"
#include "aethernet/protocol.h"         /* aethernet_mesh_packet_t / aethernet_packet_t */
#include "aethernet_reputation.h"       /* AetherNetNodeReputationService            */
 ...
 * Outputs:
 *   out_private: AETHERNET_ED25519_PRIVATE_KEY_SIZE bytes
 *   out_public:  AETHERNET_ED25519_PUBLIC_KEY_SIZE bytes
 *
 ...
 * Returns: true on success.
 * Output:
 *   out_signature: AETHERNET_ED25519_SIGNATURE_SIZE bytes (caller allocates)
 */
 ...
 *   out_tag:        AETHERNET_AES_GCM_TAG_SIZE bytes
 *   out_nonce:      AETHERNET_AES_GCM_NONCE_SIZE bytes (if nonce is NULL, randomly generated)
 *
 * Caller must allocate output buffers.
...
bool aethernet_aes256_gcm_encrypt(const uint8_t *plaintext,
                               ...
                               const uint8_t *nonce, // NULL to generate, or AETHERNET_AES_GCM_NONCE_SIZE bytes
...
bool aethernet_aes256_gcm_decrypt(const uint8_t *ciphertext,
                               ...
                               const uint8_t *key,     // 32 bytes for AES-256
                               const uint8_t *nonce,   // AETHERNET_AES_GCM_NONCE_SIZE bytes
                               const uint8_t *tag,     // AETHERNET_AES_GCM_TAG_SIZE bytes
                               const uint8_t *aad,     // Additional authenticated data (can be NULL)
                               size_t aad_len,
 ...
 * Returns: true on success.
 * Output:
 *   out_hash: AETHERNET_HMAC_SHA256_SIZE bytes (caller allocates)
 */
 ...
 * Returns: true on success.
 * Output:
 *   out_hash: AETHERNET_SHA256_SIZE bytes (caller allocates)
 */
...
#define AETHERNET_X25519_PUBLIC_KEY_SIZE 32U
/** X25519 private key size in bytes. */
#define AETHERNET_X25519_PRIVATE_KEY_SIZE 32U
/** X25519 shared-secret size in bytes (output of one DH op). */
#define AETHERNET_X25519_SHARED_SECRET_SIZE 32U

/**
 * Generate a fresh X25519 keypair.
 *
 * Output buffers (caller-allocated):
 *   out_private: AETHERNET_X25519_PRIVATE_KEY_SIZE bytes
 *   out_public:  AETHERNET_X25519_PUBLIC_KEY_SIZE bytes
 *
 * Returns: true on success.
 */
 ...
 *   out_shared: AETHERNET_X25519_SHARED_SECRET_SIZE bytes
 *
...
/** Opaque nonce-store handle.  Create with aethernet_nonce_store_new(). */
typedef struct aethernet_nonce_store aethernet_nonce_store_t;

/**
 * Allocate and initialise a new nonce store.
 * Caller must free with aethernet_nonce_store_free().
 *
...
/**
 * Free a nonce store previously created by aethernet_nonce_store_new().
 ...
 *
 *   • Nonce replay  → aethernet_reputation_record_replay(rep, source_uhid)
 *   • Sig failure   → aethernet_reputation_record_sig_failure(rep, source_uhid)
 *
 * The service owns NO heap allocation — embed AetherNetPacketSigningService
 * by value or as a static.  Call aethernet_packet_signing_init() before use.
 * ──────────────────────────────────────────────────────────────────────── */
...
} AetherNetPacketSigningService;

/**
 * Initialise an AetherNetPacketSigningService.
 *
 * Parameters:
 ...
 * aethernet_packet_signing_set_reputation() to attach one.
 */
 ...
 *
 * Workflow:
 *   1. If svc->nonce_store != NULL: call aethernet_nonce_store_check_and_record().
 *      On replay (returns false): fire reputation hook + return false.
 *   2. Rebuild signable data via aethernet_packet_get_signable_data().
 *   3. aethernet_ed25519_verify() against sender_public_key.
 ...
 *
 * Note: aethernet_packet_get_signable_data() is declared in aether/protocol.h.
