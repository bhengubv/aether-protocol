...
// Storage model
// ─────────────
// All per-UHID state lives in two fixed-size arrays (AETHERNET_ANOMALY_MAX_NODES
// entries each) that are heap-allocated once in aethernet_anomaly_create and never
...
// used by aethernet_reputation.c.
//
...
#define AETHERNET_ANOMALY_MAX_NODES   1024

/** Ring-buffer capacity for the destination-scatter detector (per source). */
#define AETHERNET_SCATTER_BUF_SIZE    128

// ─── Per-node scatter ring-buffer ─────────────────────────────────────────────

typedef struct {
    char    dest[AETHERNET_UHID_MAX_LEN];
    int64_t timestamp_ms;
} ScatterEntry;

typedef struct {
    ScatterEntry buf[AETHERNET_SCATTER_BUF_SIZE];
...
struct AetherNetBehavioralAnomalyDetector {
    AetherNetNodeReputationService *reputation;
    AetherNetAnomalyOptions         opts;

    AnomalyNode nodes[AETHERNET_ANOMALY_MAX_NODES];
    int         node_count;
};
...
AetherNetBehavioralAnomalyDetector *aethernet_anomaly_create(
    AetherNetNodeReputationService *reputation,
    const AetherNetAnomalyOptions  *opts)
{
    AetherNetBehavioralAnomalyDetector *det =
        (AetherNetBehavioralAnomalyDetector *)calloc(1, sizeof(*det));
    if (det == NULL) {
        return NULL;
    }
    ...
    if (opts != NULL) {
        ...
        aethernet_anomaly_options_default(&det->opts);
    }
 ...
 */
static AnomalyNode *find_node(AetherNetBehavioralAnomalyDetector *det,
                               const char *uhid)
{
    for (int i = 0; i < det->node_count; i++) {
        if (strncmp(det->nodes[i].uhid, uhid, AETHERNET_UHID_MAX_LEN - 1) == 0) {
            return &det->nodes[i];
        }
    }
    return NULL;
 ...
 */
static AnomalyNode *get_or_create_node(AetherNetBehavioralAnomalyDetector *det,
                                        const char *uhid)
{
    ...
    if (det->node_count < AETHERNET_ANOMALY_MAX_NODES) {
        n = &det->nodes[det->node_count++];
    } else {
        /* Evict oldest, shift everyone down. */
        memmove(&det->nodes[0], &det->nodes[1],
                sizeof(AnomalyNode) * (AETHERNET_ANOMALY_MAX_NODES - 1));
        n = &det->nodes[AETHERNET_ANOMALY_MAX_NODES - 1];
    }

    /* Initialise fresh record. */
    memset(n, 0, sizeof(*n));
    strncpy(n->uhid, uhid, AETHERNET_UHID_MAX_LEN - 1);
    n->uhid[AETHERNET_UHID_MAX_LEN - 1] = '\0';
...
static void volume_observe(AetherNetBehavioralAnomalyDetector *det,
                           AnomalyNode *n,
                           int64_t     timestamp_ms)
{
    ...
    if (timestamp_ms - n->volume.window_start_ms >= o->volume_window_ms) {
        ...
        if (ewma == 0.0) {
            ...
            if ((double)wc > o->volume_spike_multiplier * ewma) {
                aethernet_reputation_record_rreq_flood(det->reputation, n->uhid);
            }
...
static void scatter_append(ScatterRing *ring,
                            ...
                            int64_t     timestamp_ms)
{
    int slot = ring->head % AETHERNET_SCATTER_BUF_SIZE;
    strncpy(ring->buf[slot].dest, dest, AETHERNET_UHID_MAX_LEN - 1);
    ring->buf[slot].dest[AETHERNET_UHID_MAX_LEN - 1] = '\0';
    ring->buf[slot].timestamp_ms = timestamp_ms;
    ring->head = (ring->head + 1) % AETHERNET_SCATTER_BUF_SIZE;
    if (ring->count < AETHERNET_SCATTER_BUF_SIZE) {
        ring->count++;
    }
}
                                 ...
                                 int64_t           window_ms)
{
    ...
    /* Iterate live entries.  The ring's logical entries occupy slots
       [(head - count) .. (head - 1)] mod AETHERNET_SCATTER_BUF_SIZE. */
    for (int i = 0; i < ring->count; i++) {
        int slot = ((ring->head - ring->count + i) + AETHERNET_SCATTER_BUF_SIZE * 2)
                   % AETHERNET_SCATTER_BUF_SIZE;
        ...
        for (int j = 0; j < i && !seen; j++) {
            int slot2 = ((ring->head - ring->count + j) + AETHERNET_SCATTER_BUF_SIZE * 2)
                        % AETHERNET_SCATTER_BUF_SIZE;
            const ScatterEntry *e2 = &ring->buf[slot2];
            if (now_ms - e2->timestamp_ms > window_ms) {
                continue;
            }
            if (strncmp(e->dest, e2->dest, AETHERNET_UHID_MAX_LEN - 1) == 0) {
                seen = 1;
            }
        }
        if (!seen) {
...
static void scatter_observe(AetherNetBehavioralAnomalyDetector *det,
                            AnomalyNode *n,
                            const char  *dest,
                            int64_t      timestamp_ms)
{
