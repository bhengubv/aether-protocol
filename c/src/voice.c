...
struct aethernet_voice_service {
    ...
    aethernet_voice_incoming_cb      incoming_cb;
    void                         *incoming_cb_ud;
    aethernet_voice_state_changed_cb state_changed_cb;
    void                         *state_changed_cb_ud;
    aethernet_voice_frame_cb         frame_cb;
    void                         *frame_cb_ud;
};

// ─── Helpers ──────────────────────────────────────────────

static voice_call_record_t *find_call(aethernet_voice_service_t *svc, const uint8_t call_id[16]) {
...
static voice_call_record_t *alloc_call(aethernet_voice_service_t *svc) {
    for (int i = 0; i < VOICE_MAX_CALLS; i++) {
...
aethernet_voice_service_t *aethernet_voice_service_create(
    ...
    aethernet_voice_service_t *svc = (aethernet_voice_service_t *)calloc(1, sizeof(aethernet_voice_service_t));
    if (!svc) return NULL;
...
void aethernet_voice_set_incoming_cb(aethernet_voice_service_t *svc, aethernet_voice_incoming_cb cb, void *ud) {
    if (!svc) return;
    svc->incoming_cb    = cb;
    svc->incoming_cb_ud = ud;
}

void aethernet_voice_set_state_changed_cb(aethernet_voice_service_t *svc, aethernet_voice_state_changed_cb cb, void *ud) {
    if (!svc) return;
    svc->state_changed_cb    = cb;
    svc->state_changed_cb_ud = ud;
}

void aethernet_voice_set_frame_cb(aethernet_voice_service_t *svc, aethernet_voice_frame_cb cb, void *ud) {
    if (!svc) return;
    svc->frame_cb    = cb;
    svc->frame_cb_ud = ud;
}
...
int aethernet_voice_send_offer(
    ...
    send_signal_json(svc, obj, to_uhid, AETHERNET_PACKET_TYPE_VOICE_SIGNALING);
    return 0;
}
...
int aethernet_voice_accept_call(aethernet_voice_service_t *svc, const uint8_t call_id[16]) {
    ...
    rec->state    = AETHERNET_VOICE_STATE_CONNECTED;
    rec->next_seq = 0;
    ...
    send_signal_json(svc, obj, rec->remote_uhid, AETHERNET_PACKET_TYPE_VOICE_SIGNALING);

    if (svc->state_changed_cb)
        svc->state_changed_cb(rec->call_id, AETHERNET_VOICE_STATE_CONNECTED, svc->state_changed_cb_ud);
    return 0;
}
...
int aethernet_voice_hang_up(aethernet_voice_service_t *svc, const uint8_t call_id[16]) {
    ...
    send_signal_json(svc, obj, remote, AETHERNET_PACKET_TYPE_VOICE_SIGNALING);

    if (svc->state_changed_cb)
        svc->state_changed_cb(rec->call_id, AETHERNET_VOICE_STATE_ENDED, svc->state_changed_cb_ud);
    free_call(rec);
    return 0;
}
    ...
    if (packet->type == AETHERNET_PACKET_TYPE_VOICE_SIGNALING) {
        ...
        if (!sig) {
            ...
            if (rec) {
                ...
                rec->state  = AETHERNET_VOICE_STATE_INCOMING;
                rec->active = true;
            ...
            if (rec) {
                rec->state = AETHERNET_VOICE_STATE_CONNECTED;
                if (svc->state_changed_cb)
                    svc->state_changed_cb(call_id, AETHERNET_VOICE_STATE_CONNECTED, svc->state_changed_cb_ud);
            }
        } else if (strcmp(sig, "hangup") == 0) {
            voice_call_record_t *rec = find_call(svc, call_id);
            if (rec) {
                if (svc->state_changed_cb)
                    svc->state_changed_cb(call_id, AETHERNET_VOICE_STATE_ENDED, svc->state_changed_cb_ud);
                free_call(rec);
            }
        }
