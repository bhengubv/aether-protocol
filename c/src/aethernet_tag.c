...
// AetherNetTag — human-readable identity address derived from an Ed25519 public key.
...
#include "aethernet/security.h"   /* aethernet_sha256()                          */
#include "aethernet/constants.h"  /* AETHERNET_SHA256_SIZE, AETHERNET_ED25519_PUBLIC_KEY_SIZE */
 ...
 */
static void encode_tag(const uint8_t hash[32], aethernet_tag_t *out)
{
    /*
...
int aethernet_tag_from_public_key(const uint8_t *public_key,
                               size_t key_len,
                               aethernet_tag_t *out)
{
    if (!public_key || !out || key_len != AETHERNET_ED25519_PUBLIC_KEY_SIZE)
        return -1;

    uint8_t hash[AETHERNET_SHA256_SIZE];
    if (!aethernet_sha256(public_key, key_len, hash))
        return -1;
...
int aethernet_tag_parse(const char *tag, aethernet_tag_t *out)
{
    if (!tag || !out)
        return -1;
...
int aethernet_tag_verify(const char *tag,
                      ...
                      size_t key_len)
{
    ...
    aethernet_tag_t expected;
    if (aethernet_tag_from_public_key(public_key, key_len, &expected) != 0)
        return 0;

    aethernet_tag_t parsed;
    if (aethernet_tag_parse(tag, &parsed) != 0)
        return 0;

    return (memcmp(expected.value, parsed.value, AETHERNET_TAG_LENGTH) == 0) ? 1 : 0;
}

int aethernet_tag_is_valid(const aethernet_tag_t *tag)
{
    if (!tag)
        return 0;
    if (tag->value[0] == '\0')
