 ...
 */
static AetherNetReputationEntry *find_entry(AetherNetNodeReputationService *svc, const char *uhid)
{
    for (int i = 0; i < svc->count; i++) {
        if (strncmp(svc->entries[i].uhid, uhid, AETHERNET_UHID_MAX_LEN - 1) == 0) {
 ...
 */
static AetherNetReputationEntry *get_or_create(AetherNetNodeReputationService *svc, const char *uhid)
{
    AetherNetReputationEntry *e = find_entry(svc, uhid);
    if (e != NULL) {
        return e;
    }

    if (svc->count < AETHERNET_REPUTATION_MAX_ENTRIES) {
        e = &svc->entries[svc->count++];
    } else {
        /* Evict oldest (slot 0), shift everyone down. */
        memmove(&svc->entries[0], &svc->entries[1],
                sizeof(AetherNetReputationEntry) * (AETHERNET_REPUTATION_MAX_ENTRIES - 1));
        e = &svc->entries[AETHERNET_REPUTATION_MAX_ENTRIES - 1];
    }

    strncpy(e->uhid, uhid, AETHERNET_UHID_MAX_LEN - 1);
    e->uhid[AETHERNET_UHID_MAX_LEN - 1] = '\0';
    e->score = 1.0;
    return e;
}

/* Apply a delta to `uhid`'s score, creating the entry if needed. */
static void apply_delta(AetherNetNodeReputationService *svc, const char *uhid, double delta)
{
    AetherNetReputationEntry *e = get_or_create(svc, uhid);
