...
#include "aether/security.h"   /* aether_sha256()                          */
#include "aether/constants.h"  /* AETHER_SHA256_SIZE, AETHER_ED25519_PUBLIC_KEY_SIZE */
 ...
 */
static void encode_tag(const uint8_t hash[32], aether_tag_t *out)
{
    /*
...
int aether_tag_from_public_key(const uint8_t *public_key,
                               ...
                               aether_tag_t *out)
{
    if (!public_key || !out || key_len != AETHER_ED25519_PUBLIC_KEY_SIZE)
        return -1;

    uint8_t hash[AETHER_SHA256_SIZE];
    if (!aether_sha256(public_key, key_len, hash))
        return -1;
                      ...
                      size_t key_len)
{
    ...
    aether_tag_t expected;
    if (aether_tag_from_public_key(public_key, key_len, &expected) != 0)
        return 0;

    aether_tag_t parsed;
    if (aether_tag_parse(tag, &parsed) != 0)
        return 0;

    return (memcmp(expected.value, parsed.value, AETHER_TAG_LENGTH) == 0) ? 1 : 0;
}
