...
#define FRESHNESS_WINDOW_MS  AETHERNET_GOSSIP_FRESHNESS_MS

// ─── Opaque struct definition ─────────────────────────────────────────────────

struct AetherNetGossipService {
    AetherNetNodeReputationService *reputation;
    AetherNetGossipCallbacks        callbacks;  /* copy of caller's callbacks     */
};
...
AetherNetGossipService *aethernet_gossip_create(
    AetherNetNodeReputationService *reputation,
    const AetherNetGossipCallbacks *callbacks)
{
    ...
    AetherNetGossipService *svc =
        (AetherNetGossipService *)calloc(1, sizeof(*svc));
    if (!svc) return NULL;
...
void aethernet_gossip_destroy(AetherNetGossipService *svc)
{
    free(svc);
}
...
int aethernet_gossip_broadcast(
    AetherNetGossipService *svc,
    ...
    const char *reason)
{
    ...
    const AetherNetGossipCallbacks *cb = &svc->callbacks;
    ...
    int pkt_len = snprintf(pkt_buf, sizeof(pkt_buf),
        ...
        AETHERNET_PACKET_TYPE_REPUTATION_UPDATE,
        cb->local_uhid,
        payload_buf,
        (long long)now_ms);
    ...
    size_t key_len)
{
    ...
    if (pkt_type != AETHERNET_PACKET_TYPE_REPUTATION_UPDATE) return false;
    ...
    double R = aethernet_reputation_get_score(svc->reputation, reporter_uhid);

    /* 8. Compute the effective (weighted) delta and apply it. */
    double clamped   = clamp_delta(score_delta);
    double effective = clamped * R;
    aethernet_reputation_apply_weighted_delta(svc->reputation, target_uhid, effective);
