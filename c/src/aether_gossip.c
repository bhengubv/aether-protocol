...
#define FRESHNESS_WINDOW_MS  AETHER_GOSSIP_FRESHNESS_MS

// ─── Opaque struct definition ─────────────────────────────────────────────────

struct AetherGossipService {
    AetherNodeReputationService *reputation;
    AetherGossipCallbacks        callbacks;  /* copy of caller's callbacks     */
};
...
AetherGossipService *aether_gossip_create(
    AetherNodeReputationService *reputation,
    const AetherGossipCallbacks *callbacks)
{
    ...
    AetherGossipService *svc =
        (AetherGossipService *)calloc(1, sizeof(*svc));
    if (!svc) return NULL;
...
void aether_gossip_destroy(AetherGossipService *svc)
{
    free(svc);
}
    ...
    const char *reason)
{
    ...
    const AetherGossipCallbacks *cb = &svc->callbacks;
    ...
    int pkt_len = snprintf(pkt_buf, sizeof(pkt_buf),
        ...
        AETHER_PACKET_TYPE_REPUTATION_UPDATE,
        cb->local_uhid,
        payload_buf,
        (long long)now_ms);
    ...
    size_t key_len)
{
    ...
    if (pkt_type != AETHER_PACKET_TYPE_REPUTATION_UPDATE) return false;
    ...
    double R = aether_reputation_get_score(svc->reputation, reporter_uhid);

    /* 8. Compute the effective (weighted) delta and apply it. */
    double clamped   = clamp_delta(score_delta);
    double effective = clamped * R;
    aether_reputation_apply_weighted_delta(svc->reputation, target_uhid, effective);
