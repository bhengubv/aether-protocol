...
//
//   - aethernet_x25519_derive_public  / aethernet_x25519_agree
//   - aethernet_hkdf_sha256
//   - aethernet_hmac_sha256
//
...
// via env AETHERNET_FUZZ_ITERATIONS for adversarial local runs (100k+ takes
...
// AETHERNET_FUZZ_SEED. We do NOT need cryptographic strength here; we need
...
static int fuzz_x25519(const uint8_t *buf, size_t len) {
    ...
    if (!aethernet_x25519_derive_public(alice_priv, alice_pub)) {
    ...
    uint8_t bob_pub[32];
    if (!aethernet_x25519_derive_public(bob_priv, bob_pub)) return 1;

    uint8_t shared[32];
    bool ok = aethernet_x25519_agree(alice_priv, bob_pub, shared);
    if (ok) {
    ...
    (void)aethernet_x25519_agree(alice_priv, alice_pub, shared);
...
static int fuzz_hkdf(const uint8_t *buf, size_t len) {
    ...
    bool ok = aethernet_hkdf_sha256(salt_len ? salt : NULL, salt_len,
                                 ikm, ikm_len,
...
static int fuzz_hmac(const uint8_t *buf, size_t len) {
    ...
    bool ok = aethernet_hmac_sha256(key, key_len, data, data_len, out_with_guard);
    if (!ok) {
...
int main(void) {
    ...
    // via AETHERNET_FUZZ_ITERATIONS for local adversarial runs.
    size_t iterations = 10000;
    splitmix_state = 0xA37E1B23DEADBEEFULL;

    const char *iter_env = getenv("AETHERNET_FUZZ_ITERATIONS");
    ...
    const char *seed_env = getenv("AETHERNET_FUZZ_SEED");
