...
static int hmac_one(const uint8_t *key, size_t key_len, uint8_t b, uint8_t out[32]) {
    uint8_t msg[1] = { b };
    return aethernet_hmac_sha256(key, key_len, msg, sizeof(msg), out) ? 1 : 0;
}
...
static int test_x3dh_basic(void) {
    ...
    aethernet_x25519_derive_public(alice_ik, alice_ik_pub);
    aethernet_x25519_derive_public(alice_ek, alice_ek_pub);
    aethernet_x25519_derive_public(bob_ik, bob_ik_pub);
    aethernet_x25519_derive_public(bob_spk, bob_spk_pub);
    aethernet_x25519_derive_public(bob_opk, bob_opk_pub);

    uint8_t dh1[32], dh2[32], dh3[32], dh4[32];
    aethernet_x25519_agree(alice_ik, bob_spk_pub, dh1);
    aethernet_x25519_agree(alice_ek, bob_ik_pub, dh2);
    aethernet_x25519_agree(alice_ek, bob_spk_pub, dh3);
    aethernet_x25519_agree(alice_ek, bob_opk_pub, dh4);
    ...
    aethernet_hkdf_sha256(NULL, 0, shared, sizeof(shared),
                      (const uint8_t *)root_info, strlen(root_info), 32, root_key);
    aethernet_hkdf_sha256(NULL, 0, root_key, 32,
...
static int test_kdf_rk_basic(void) {
    ...
    if (!aethernet_signal_kdf_rk(root_key, dh_output, new_root, new_chain)) {
        fprintf(stderr, "  aethernet_signal_kdf_rk returned false\n");
        free(inputs_json); free(case_obj); free(expected_json);
        return 0;
    }
