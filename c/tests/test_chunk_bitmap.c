...
static void test_encode_produces_correct_bitset(void)
{
    ...
    for (int v = 0; v < VECTOR_COUNT; v++) {
        ...
        aethernet_bitset_t bs = aethernet_bitset_encode(
        ...
        // Verify base64.
        char *b64 = aethernet_chunk_bitmap_marshal_json(
...
static void test_decode_recovers_correct_indices(void)
{
    ...
    for (int v = 0; v < VECTOR_COUNT; v++) {
        ...
        int out_count = 0;
        int *indices = aethernet_bitset_decode(bitset, blen, vec->chunk_count, &out_count);
        free(bitset);

        // Compare sorted results against expected.
        assert(out_count == vec->index_count);
        // aethernet_bitset_decode returns indices in ascending order already.
...
static void test_json_serialization_matches_expected(void)
{
    ...
    for (int v = 0; v < VECTOR_COUNT; v++) {
        ...
        char *json = aethernet_chunk_bitmap_marshal_json(
