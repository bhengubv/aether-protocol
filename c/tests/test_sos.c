...
static aether_mesh_packet_t *new_sos_packet(const char *src, int32_t ttl) {
    aether_mesh_packet_t *p = aether_packet_new();
    p->type = AETHER_PACKET_TYPE_SOS_BROADCAST;
    aether_packet_set_source_uhid(p, src);
    aether_packet_set_destination_uhid(p, "");
    p->ttl = ttl;
    p->priority = AETHER_SOS_PRIORITY;
    ...
    aether_packet_set_payload(p, (const uint8_t *)body, strlen(body));
    return p;
}
...
static void on_sos(const aether_sos_alert_t *alert, void *ud) {
    (void)alert; (void)ud;
    received_count++;
}
...
static void broadcast_floods_and_stores_alert(void) {
    ...
    aether_sos_service_t *svc = aether_sos_service_new(&sender);

    int rc = aether_sos_broadcast(svc, "sos", "help", -33.9, 18.4, NULL);
    assert(rc == 0);
    assert(s.broadcasts_len == 1);
    assert(s.broadcasts[0]->type == AETHER_PACKET_TYPE_SOS_BROADCAST);
    assert(s.broadcasts[0]->ttl == AETHER_SOS_TTL);
    assert(s.broadcasts[0]->priority == AETHER_SOS_PRIORITY);

    aether_sos_service_free(svc);
    fake_clear(&s);
}
...
static void broadcast_rate_limited_after_max(void) {
    ...
    for (int i = 0; i < AETHER_MAX_SOS_BROADCASTS_PER_HOUR; i++) {
        int rc = aether_sos_broadcast(svc, "sos", "h", 0, 0, NULL);
        assert(rc == 0);
    }
    int rc = aether_sos_broadcast(svc, "sos", "h", 0, 0, NULL);
...
static void broadcast_rejects_null_type(void) {
    ...
    int rc = aether_sos_broadcast(svc, NULL, "help", 0, 0, NULL);
    assert(rc == -1);
...
static void handle_drops_duplicate_packet_id(void) {
    ...
    aether_mesh_packet_t *pkt = new_sos_packet("alice", AETHER_SOS_TTL);
    aether_sos_handle_packet(svc, pkt);
    int after_first = s.broadcasts_len;

    // Re-feed the same packet id
    aether_mesh_packet_t *pkt2 = aether_packet_clone(pkt);
    aether_sos_handle_packet(svc, pkt2);
    assert(s.broadcasts_len == after_first);

    aether_packet_free(pkt);
    aether_packet_free(pkt2);
    aether_sos_service_free(svc);
    fake_clear(&s);
}
...
static void handle_ignores_self_originated(void) {
    ...
    aether_mesh_packet_t *pkt = new_sos_packet(LOCAL_UHID, AETHER_SOS_TTL);
    aether_sos_handle_packet(svc, pkt);
...
static void handle_rebroadcasts_when_ttl_allows(void) {
    ...
    aether_mesh_packet_t *pkt = new_sos_packet("alice", 5);
    aether_sos_handle_packet(svc, pkt);
    assert(s.broadcasts_len == 1);
...
static void handle_does_not_rebroadcast_when_ttl_exhausted(void) {
    ...
    aether_mesh_packet_t *pkt = new_sos_packet("alice", 1);
    aether_sos_handle_packet(svc, pkt);
    assert(s.broadcasts_len == 0);
...
static void handle_invokes_received_callback(void) {
    ...
    aether_sos_service_t *svc = aether_sos_service_new(&sender);
    received_count = 0;
    aether_sos_set_received_cb(svc, on_sos, NULL);
...
static void resolve_with_unknown_id_is_safe(void) {
    ...
    uint8_t id[AETHER_PACKET_ID_SIZE] = {0};
    aether_sos_resolve(svc, id);
