...
// Unit tests for aether_gossip.c (ReputationGossipService).
//
...
static AetherGossipCallbacks make_callbacks(const char *local_uhid)
{
    AetherGossipCallbacks cb;
...
// Used to inject inbound packets into aether_gossip_handle without going
// through aether_gossip_broadcast (which would stamp timestamp_ms = 0 from the
...
static void broadcast_returns_delivered_count(void)
{
    ...
    AetherGossipCallbacks cb = make_callbacks("local-node");
    AetherGossipService *svc = aether_gossip_create(&rep, &cb);
    assert(svc != NULL);

    int delivered = aether_gossip_broadcast(svc, "peer-a", -0.10, "bad-behaviour");
    /* fake_broadcast always returns 3 */
    assert(delivered == 3);
    assert(g_broadcast_count == 1);

    aether_gossip_destroy(svc);
}
...
static void broadcast_payload_has_correct_fields(void)
{
    ...
    aether_gossip_broadcast(svc, "peer-b", -0.05, "flood");
...
static void broadcast_clamps_delta_above_1(void)
{
    ...
    aether_gossip_broadcast(svc, "peer-c", 5.0, "bonus");
...
static void broadcast_clamps_delta_below_minus_1(void)
{
    ...
    aether_gossip_broadcast(svc, "peer-d", -9.9, "punish");
...
static void handle_wrong_type_returns_false(void)
{
    ...
    bool accepted = aether_gossip_handle(svc, pkt, NULL, 0);
    assert(accepted == false);

    /* Victim's score must be untouched (1.0). */
    double score = aether_reputation_get_score(&rep, "victim");
...
static void handle_invalid_signature_returns_false(void)
{
    ...
    double score = aether_reputation_get_score(&rep, "target-a");
    assert(fabs(score - 1.0) < 1e-9);
...
static void handle_stale_timestamp_returns_false(void)
{
    ...
    long long stale_ts = -((long long)AETHER_GOSSIP_FRESHNESS_MS + 1LL);
    char pkt[2048];
    ...
    double score = aether_reputation_get_score(&rep, "target-b");
    assert(fabs(score - 1.0) < 1e-9);
...
static void handle_own_gossip_returns_false(void)
{
    ...
    double score = aether_reputation_get_score(&rep, "target-c");
    assert(fabs(score - 1.0) < 1e-9);
...
static void handle_unknown_reporter_full_delta(void)
{
    ...
    double score = aether_reputation_get_score(&rep, "target-d");
...
static void handle_degraded_reporter_weighted_delta(void)
{
    ...
    aether_reputation_record_rreq_flood(&rep, "degraded-reporter"); /* -0.05 → 0.95 */
    aether_reputation_record_rreq_flood(&rep, "degraded-reporter"); /* -0.05 → 0.90 */
    aether_reputation_record_rreq_flood(&rep, "degraded-reporter"); /* -0.05 → 0.85 */
    aether_reputation_record_rreq_flood(&rep, "degraded-reporter"); /* -0.05 → 0.80 */
    aether_reputation_record_rreq_flood(&rep, "degraded-reporter"); /* -0.05 → 0.75 */
    aether_reputation_record_rreq_flood(&rep, "degraded-reporter"); /* -0.05 → 0.70 */
    aether_reputation_record_rreq_flood(&rep, "degraded-reporter"); /* -0.05 → 0.65 */
    aether_reputation_record_rreq_flood(&rep, "degraded-reporter"); /* -0.05 → 0.60 */
    aether_reputation_record_rreq_flood(&rep, "degraded-reporter"); /* -0.05 → 0.55 */
    aether_reputation_record_rreq_flood(&rep, "degraded-reporter"); /* -0.05 → 0.50 */

    double reporter_score = aether_reputation_get_score(&rep, "degraded-reporter");
    ...
    double score = aether_reputation_get_score(&rep, "target-e");
