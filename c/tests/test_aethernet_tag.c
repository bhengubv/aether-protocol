...
// Unit tests for aethernet_tag.c
...
{
    aethernet_tag_t tag;
    int rc = aethernet_tag_from_public_key(FIXED_KEY, 32, &tag);
    assert(rc == 0);
...
{
    aethernet_tag_t tag1, tag2;

    int rc1 = aethernet_tag_from_public_key(FIXED_KEY, 32, &tag1);
    assert(rc1 == 0);

    int rc2 = aethernet_tag_parse(tag1.value, &tag2);
    assert(rc2 == 0);

    assert(memcmp(tag1.value, tag2.value, AETHERNET_TAG_LENGTH) == 0);
}
...
{
    aethernet_tag_t tag;
    aethernet_tag_from_public_key(FIXED_KEY, 32, &tag);

    int result = aethernet_tag_verify(tag.value, FIXED_KEY, 32);
    assert(result == 1);
}
...
static void verify_wrong_key_returns_0(void)
{
    ...
    int result = aethernet_tag_verify(tag.value, OTHER_KEY, 32);
    assert(result == 0);
}
...
{
    aethernet_tag_t ref, parsed;
    aethernet_tag_from_public_key(FIXED_KEY, 32, &ref);

    int rc = aethernet_tag_parse(ref.value, &parsed);
    assert(rc == 0);
    assert(memcmp(ref.value, parsed.value, AETHERNET_TAG_LENGTH) == 0);
}
...
static void parse_accepts_no_separator(void)
{
    ...
    int rc = aethernet_tag_parse(no_sep, &parsed);
    assert(rc == 0);
    assert(memcmp(ref.value, parsed.value, AETHERNET_TAG_LENGTH) == 0);
}
...
static void parse_accepts_lowercase_with_separator(void)
{
    ...
    char lower[AETHERNET_TAG_LENGTH];
    for (int i = 0; i < 11; i++) {
        char c = ref.value[i];
    ...
    int rc = aethernet_tag_parse(lower, &parsed);
    assert(rc == 0);
    assert(memcmp(ref.value, parsed.value, AETHERNET_TAG_LENGTH) == 0);
}
...
static void parse_accepts_mixed_case(void)
{
    ...
    char mixed[AETHERNET_TAG_LENGTH];
    for (int i = 0; i < 11; i++) {
        char c = ref.value[i];
        if (i % 2 == 0 && c >= 'A' && c <= 'Z')
            c = (char)(c - 'A' + 'a');
    ...
    int rc = aethernet_tag_parse(mixed, &parsed);
    assert(rc == 0);
    assert(memcmp(ref.value, parsed.value, AETHERNET_TAG_LENGTH) == 0);
}
...
{
    aethernet_tag_t out;
    int rc = aethernet_tag_parse(NULL, &out);
    assert(rc == -1);
}

static void parse_rejects_null_out(void)
{
    int rc = aethernet_tag_parse("ABCDE-FGHJK", NULL);
    assert(rc == -1);
}
...
{
    aethernet_tag_t out;
    int rc = aethernet_tag_parse("ABCD", &out);
    assert(rc == -1);
}
...
{
    aethernet_tag_t out;
    int rc = aethernet_tag_parse("ABCDE-FGHJK0", &out); /* 12 chars */
    assert(rc == -1);
}
...
static void parse_rejects_invalid_chars(void)
{
    ...
    int rc = aethernet_tag_parse("IABCD-EFGHJ", &out);
    assert(rc == -1);
}
...
static void parse_rejects_invalid_chars_no_sep(void)
{
    ...
    int rc = aethernet_tag_parse("OABCDEFGHJ", &out);
    assert(rc == -1);
}
...
static void parse_rejects_wrong_separator_position(void)
{
    ...
    int rc = aethernet_tag_parse("ABCD-EFGHJ0", &out);
    assert(rc == -1);
}
...
{
    aethernet_tag_t tag1, tag2;
    aethernet_tag_from_public_key(FIXED_KEY, 32, &tag1);
    aethernet_tag_from_public_key(OTHER_KEY, 32, &tag2);
...
/* ── 7. aethernet_tag_is_valid() ────────────────────────────────────────────── */

static void is_valid_returns_1_for_derived_tag(void)
{
    aethernet_tag_t tag;
    aethernet_tag_from_public_key(FIXED_KEY, 32, &tag);
    assert(aethernet_tag_is_valid(&tag) == 1);
}
...
static void is_valid_returns_0_for_empty(void)
{
    ...
    assert(aethernet_tag_is_valid(&tag) == 0);
}

static void is_valid_returns_0_for_null(void)
{
    assert(aethernet_tag_is_valid(NULL) == 0);
}
...
{
    aethernet_tag_t out;
    int rc = aethernet_tag_from_public_key(NULL, 32, &out);
    assert(rc == -1);
}
...
{
    aethernet_tag_t out;
    int rc = aethernet_tag_from_public_key(FIXED_KEY, 16, &out);
    assert(rc == -1);
}

static void from_public_key_rejects_null_out(void)
{
    int rc = aethernet_tag_from_public_key(FIXED_KEY, 32, NULL);
    assert(rc == -1);
}
...
{
    aethernet_tag_t a, b, c;
    aethernet_tag_from_public_key(FIXED_KEY, 32, &a);
    aethernet_tag_from_public_key(FIXED_KEY, 32, &b);
    aethernet_tag_from_public_key(FIXED_KEY, 32, &c);

    assert(memcmp(a.value, b.value, AETHERNET_TAG_LENGTH) == 0);
    assert(memcmp(b.value, c.value, AETHERNET_TAG_LENGTH) == 0);
}
