...
static void ed25519_generate_keypair_produces_nonzero_keys(void) {
    uint8_t priv[AETHERNET_ED25519_PRIVATE_KEY_SIZE];
    uint8_t pub[AETHERNET_ED25519_PUBLIC_KEY_SIZE];
    bool ok = aethernet_ed25519_generate_keypair(priv, pub);
    assert(ok);
...
static void ed25519_sign_and_verify_roundtrip(void) {
    ...
    bool signed_ok = aethernet_ed25519_sign(priv, msg, sizeof(msg), sig);
    assert(signed_ok);
    assert(!all_zero(sig, sizeof(sig)));

    bool verified = aethernet_ed25519_verify(pub, msg, sizeof(msg), sig);
    assert(verified);
}
...
static void ed25519_verify_tampered_signature_returns_false(void) {
    ...
    sig[0] ^= 0xFF;
    bool bad = aethernet_ed25519_verify(pub, msg, sizeof(msg), sig);
    assert(!bad);
}
...
static void ed25519_verify_wrong_key_returns_false(void) {
    ...
    uint8_t priv2[AETHERNET_ED25519_PRIVATE_KEY_SIZE];
    uint8_t pub2[AETHERNET_ED25519_PUBLIC_KEY_SIZE];
    aethernet_ed25519_generate_keypair(priv2, pub2);
    ...
    bool bad = aethernet_ed25519_verify(pub2, msg, sizeof(msg), sig);
    assert(!bad);
}
...
static void aes_gcm_encrypt_decrypt_roundtrip(void) {
    ...
    bool enc_ok = aethernet_aes256_gcm_encrypt(
    ...
    bool dec_ok = aethernet_aes256_gcm_decrypt(
...
static void aes_gcm_decrypt_tampered_tag_returns_false(void) {
    ...
    aethernet_aes256_gcm_encrypt(plain, sizeof(plain), key, NULL, NULL, 0, cipher, tag, nonce);

    // Flip a tag byte
    tag[0] ^= 0x01;
    uint8_t out[sizeof(plain)];
    bool bad = aethernet_aes256_gcm_decrypt(
...
static void hmac_sha256_is_deterministic(void) {
    ...
    uint8_t h1[AETHERNET_HMAC_SHA256_SIZE];
    uint8_t h2[AETHERNET_HMAC_SHA256_SIZE];

    bool ok1 = aethernet_hmac_sha256(key, sizeof(key), data, sizeof(data), h1);
    bool ok2 = aethernet_hmac_sha256(key, sizeof(key), data, sizeof(data), h2);
    assert(ok1 && ok2);
...
static void hmac_sha256_different_keys_produce_different_hashes(void) {
    ...
    aethernet_hmac_sha256(key1, sizeof(key1), data, sizeof(data), h1);
    aethernet_hmac_sha256(key2, sizeof(key2), data, sizeof(data), h2);
    assert(memcmp(h1, h2, sizeof(h1)) != 0);
}
...
static void sha256_is_deterministic(void) {
    ...
    uint8_t h1[AETHERNET_SHA256_SIZE];
    uint8_t h2[AETHERNET_SHA256_SIZE];

    bool ok1 = aethernet_sha256(data, sizeof(data), h1);
    bool ok2 = aethernet_sha256(data, sizeof(data), h2);
    assert(ok1 && ok2);
    assert(memcmp(h1, h2, sizeof(h1)) == 0);
    assert(!all_zero(h1, sizeof(h1)));
}
...
static void sha256_different_inputs_produce_different_hashes(void) {
    const uint8_t a[] = { 'a' };
    const uint8_t b[] = { 'b' };
    uint8_t ha[AETHERNET_SHA256_SIZE], hb[AETHERNET_SHA256_SIZE];
    aethernet_sha256(a, sizeof(a), ha);
    aethernet_sha256(b, sizeof(b), hb);
...
static void blake3_is_deterministic(void) {
    ...
    uint8_t h1[AETHERNET_BLAKE3_SIZE];
    uint8_t h2[AETHERNET_BLAKE3_SIZE];

    bool ok1 = aethernet_blake3(data, sizeof(data), h1);
    bool ok2 = aethernet_blake3(data, sizeof(data), h2);
    assert(ok1 && ok2);
    assert(memcmp(h1, h2, sizeof(h1)) == 0);
    assert(!all_zero(h1, sizeof(h1)));
}
...
static void blake3_and_sha256_produce_different_outputs(void) {
    ...
    uint8_t hb3[AETHERNET_BLAKE3_SIZE];
    uint8_t hsha[AETHERNET_SHA256_SIZE];
    aethernet_blake3(data, sizeof(data), hb3);
    aethernet_sha256(data, sizeof(data), hsha);
    assert(memcmp(hb3, hsha, AETHERNET_BLAKE3_SIZE) != 0);
}
...
static void hkdf_is_deterministic(void) {
    ...
    aethernet_hkdf_sha256(salt, sizeof(salt), ikm, sizeof(ikm), info, sizeof(info), sizeof(okm1), okm1);
    aethernet_hkdf_sha256(salt, sizeof(salt), ikm, sizeof(ikm), info, sizeof(info), sizeof(okm2), okm2);
...
static void signal_kdf_rk_produces_nonzero_keys(void) {
    ...
    aethernet_random_bytes(root_key, sizeof(root_key));
    aethernet_random_bytes(dh_output, sizeof(dh_output));

    uint8_t new_rk[32];
    uint8_t new_ck[32];
    bool ok = aethernet_signal_kdf_rk(root_key, dh_output, new_rk, new_ck);
    assert(ok);
...
static void signal_kdf_rk_is_deterministic(void) {
    ...
    aethernet_signal_kdf_rk(rk, dh, new_rk1, new_ck1);
    aethernet_signal_kdf_rk(rk, dh, new_rk2, new_ck2);
...
static void x25519_generate_keypair_produces_nonzero_keys(void) {
    uint8_t priv[AETHERNET_X25519_PRIVATE_KEY_SIZE];
    uint8_t pub[AETHERNET_X25519_PUBLIC_KEY_SIZE];
    bool ok = aethernet_x25519_generate_keypair(priv, pub);
    assert(ok);
    assert(!all_zero(priv, sizeof(priv)));
    assert(!all_zero(pub, sizeof(pub)));
}
...
static void x25519_derive_public_matches_generated(void) {
    uint8_t priv[AETHERNET_X25519_PRIVATE_KEY_SIZE];
    uint8_t pub_generated[AETHERNET_X25519_PUBLIC_KEY_SIZE];
    uint8_t pub_derived[AETHERNET_X25519_PUBLIC_KEY_SIZE];

    aethernet_x25519_generate_keypair(priv, pub_generated);
    bool ok = aethernet_x25519_derive_public(priv, pub_derived);
    assert(ok);
    assert(memcmp(pub_generated, pub_derived, AETHERNET_X25519_PUBLIC_KEY_SIZE) == 0);
}

static void x25519_ecdh_is_commutative(void) {
    // Alice and Bob should arrive at the same shared secret.
    uint8_t alice_priv[AETHERNET_X25519_PRIVATE_KEY_SIZE];
    uint8_t alice_pub[AETHERNET_X25519_PUBLIC_KEY_SIZE];
    uint8_t bob_priv[AETHERNET_X25519_PRIVATE_KEY_SIZE];
    uint8_t bob_pub[AETHERNET_X25519_PUBLIC_KEY_SIZE];

    aethernet_x25519_generate_keypair(alice_priv, alice_pub);
    aethernet_x25519_generate_keypair(bob_priv, bob_pub);

    uint8_t alice_shared[AETHERNET_X25519_SHARED_SECRET_SIZE];
    uint8_t bob_shared[AETHERNET_X25519_SHARED_SECRET_SIZE];

    bool ok_a = aethernet_x25519_agree(alice_priv, bob_pub, alice_shared);
    bool ok_b = aethernet_x25519_agree(bob_priv, alice_pub, bob_shared);
    assert(ok_a && ok_b);
    assert(memcmp(alice_shared, bob_shared, AETHERNET_X25519_SHARED_SECRET_SIZE) == 0);
    assert(!all_zero(alice_shared, AETHERNET_X25519_SHARED_SECRET_SIZE));
}
...
static void x25519_agree_rejects_low_order_point(void) {
    ...
    uint8_t priv[AETHERNET_X25519_PRIVATE_KEY_SIZE];
    uint8_t pub_discard[AETHERNET_X25519_PUBLIC_KEY_SIZE];
    aethernet_x25519_generate_keypair(priv, pub_discard);  // need a valid private key

    uint8_t pub_lo[AETHERNET_X25519_PUBLIC_KEY_SIZE];
    memset(pub_lo, 0, sizeof(pub_lo));  // all-zero = low-order point

    uint8_t shared[AETHERNET_X25519_SHARED_SECRET_SIZE];
    bool ok = aethernet_x25519_agree(priv, pub_lo, shared);
...
static void zeroize_clears_buffer(void) {
    uint8_t buf[64];
    aethernet_random_bytes(buf, sizeof(buf));
    // Likely non-zero after random fill
    aethernet_zeroize(buf, sizeof(buf));
...
static void random_bytes_produces_nonzero_output(void) {
    ...
    uint8_t buf[32];
    bool ok = aethernet_random_bytes(buf, sizeof(buf));
    assert(ok);
    assert(!all_zero(buf, sizeof(buf)));
}
...
static void random_bytes_two_calls_differ(void) {
    ...
    uint8_t a[32], b[32];
    aethernet_random_bytes(a, sizeof(a));
    aethernet_random_bytes(b, sizeof(b));
...
static void nonce_store_new_returns_non_null(void) {
    aethernet_nonce_store_t *s = aethernet_nonce_store_new();
    assert(s != NULL);
    aethernet_nonce_store_free(s);
}

static void nonce_store_fresh_nonce_returns_true(void) {
    aethernet_nonce_store_t *s = aethernet_nonce_store_new();
    uint8_t nonce[8] = {1, 2, 3, 4, 5, 6, 7, 8};
    bool ok = aethernet_nonce_store_check_and_record(s, "alice", nonce, 8, 300);
    assert(ok);
    aethernet_nonce_store_free(s);
}
...
static void nonce_store_same_source_same_nonce_is_replay(void) {
    ...
    bool first  = aethernet_nonce_store_check_and_record(s, "bob", nonce, 8, 300);
    bool second = aethernet_nonce_store_check_and_record(s, "bob", nonce, 8, 300);
...
static void nonce_store_different_source_same_nonce_bytes_not_replay(void) {
    ...
    bool alice = aethernet_nonce_store_check_and_record(s, "alice", nonce, 8, 300);
    bool bob   = aethernet_nonce_store_check_and_record(s, "bob",   nonce, 8, 300);
    assert(alice == true);
...
static void nonce_store_same_source_different_nonce_not_replay(void) {
    ...
    bool first  = aethernet_nonce_store_check_and_record(s, "dave", n1, 8, 300);
    bool second = aethernet_nonce_store_check_and_record(s, "dave", n2, 8, 300);
    assert(first  == true);
...
static void nonce_store_null_params_return_false(void) {
    aethernet_nonce_store_t *s = aethernet_nonce_store_new();
    uint8_t nonce[8] = {0};
    assert(aethernet_nonce_store_check_and_record(NULL, "alice", nonce, 8, 300) == false);
    assert(aethernet_nonce_store_check_and_record(s, NULL, nonce, 8, 300) == false);
    assert(aethernet_nonce_store_check_and_record(s, "alice", NULL, 8, 300) == false);
    assert(aethernet_nonce_store_check_and_record(s, "alice", nonce, 0, 300) == false);
    aethernet_nonce_store_free(s);
}
...
static void nonce_store_expired_entry_treated_as_fresh(void) {
    ...
    aethernet_nonce_store_check_and_record(s, "carol", nonce, 8, 0);
    /* With TTL = 0 the entry is in the past on the very next call.
     * The second check should treat it as fresh (pruned). */
    bool second = aethernet_nonce_store_check_and_record(s, "carol", nonce, 8, 300);
    assert(second == true);
    aethernet_nonce_store_free(s);
}
...
static aethernet_mesh_packet_t *make_signed_packet(const uint8_t *priv_key,
    ...
    aethernet_packet_set_destination_uhid(pkt, "dst-node");

    /* Use the provided nonce or fill with a fixed pattern. */
    if (nonce_override) {
        memcpy(pkt->packet_nonce, nonce_override, AETHERNET_PACKET_NONCE_SIZE);
    } else {
        for (int i = 0; i < AETHERNET_PACKET_NONCE_SIZE; i++) {
    ...
    uint8_t *sig_data = aethernet_packet_get_signable_data(pkt, &sig_data_len);
    assert(sig_data != NULL);

    uint8_t sig[AETHERNET_ED25519_SIGNATURE_SIZE];
    bool ok = aethernet_ed25519_sign(priv_key, sig_data, sig_data_len, sig);
    free(sig_data);
    assert(ok);

    aethernet_packet_set_signature(pkt, sig, AETHERNET_ED25519_SIGNATURE_SIZE);
    return pkt;
}
 ...
 *
 * Send the same (source, nonce) twice through aethernet_packet_signing_verify().
...
static void packet_signing_replay_fires_reputation_hook(void) {
    ...
    aethernet_nonce_store_t *ns = aethernet_nonce_store_new();
    assert(ns != NULL);

    AetherNetPacketSigningService svc;
    aethernet_packet_signing_init(&svc, ns);

    AetherNetNodeReputationService rep;
    aethernet_reputation_init(&rep);
    aethernet_packet_signing_set_reputation(&svc, &rep);

    /* Initial score for this UHID is 1.0 (unknown → benefit of the doubt). */
    const char *uhid = "replay-sender";
    double score_before = aethernet_reputation_get_score(&rep, uhid);
    assert(score_before == 1.0);

    aethernet_mesh_packet_t *pkt = make_signed_packet(priv, uhid, NULL);

    /* First verify: fresh nonce — should succeed. */
    bool first = aethernet_packet_signing_verify(&svc, pkt, pub, 300);
    assert(first == true);

    /* Second verify: same packet (same nonce) — replay, must fail. */
    bool second = aethernet_packet_signing_verify(&svc, pkt, pub, 300);
    assert(second == false);

    /* Score must have dropped (replay signal = −0.15). */
    double score_after = aethernet_reputation_get_score(&rep, uhid);
    assert(score_after < score_before);

    aethernet_packet_free(pkt);
    aethernet_nonce_store_free(ns);
}
...
static void packet_signing_fresh_nonce_does_not_fire_hook(void) {
    ...
    uint8_t nonce_a[AETHERNET_PACKET_NONCE_SIZE] = {1, 2, 3, 4, 5, 6, 7, 8};
    aethernet_mesh_packet_t *pkt_a = make_signed_packet(priv, uhid, nonce_a);
    bool ok_a = aethernet_packet_signing_verify(&svc, pkt_a, pub, 300);
    assert(ok_a == true);

    /* Packet B — nonce 0x11…18 (different) */
    uint8_t nonce_b[AETHERNET_PACKET_NONCE_SIZE] = {0x11, 0x12, 0x13, 0x14,
                                                   0x15, 0x16, 0x17, 0x18};
    aethernet_mesh_packet_t *pkt_b = make_signed_packet(priv, uhid, nonce_b);
    bool ok_b = aethernet_packet_signing_verify(&svc, pkt_b, pub, 300);
    assert(ok_b == true);
    ...
    aethernet_packet_free(pkt_a);
    aethernet_packet_free(pkt_b);
    aethernet_nonce_store_free(ns);
}
...
static void packet_signing_sig_failure_fires_reputation_hook(void) {
    ...
    AetherNetPacketSigningService svc;
    aethernet_packet_signing_init(&svc, NULL);
    ...
    bool ok = aethernet_packet_signing_verify(&svc, pkt, pub2, 300);
    assert(ok == false);
