...
// Unit tests for aether_reputation.c (NodeReputationService).
...
static void unknown_peer_returns_1_0(void)
{
    ...
    aether_reputation_init(&svc);

    double score = aether_reputation_get_score(&svc, "alice");
...
static void rreq_flood_gives_0_95(void)
{
    ...
    aether_reputation_record_rreq_flood(&svc, "alice");
    double score = aether_reputation_get_score(&svc, "alice");
...
static void replay_gives_0_85(void)
{
    ...
    aether_reputation_record_replay(&svc, "alice");
    double score = aether_reputation_get_score(&svc, "alice");
...
static void sig_failure_gives_0_80(void)
{
    ...
    aether_reputation_record_sig_failure(&svc, "alice");
    double score = aether_reputation_get_score(&svc, "alice");
...
static void custody_refusal_gives_0_95(void)
{
    ...
    aether_reputation_record_custody_refusal(&svc, "alice");
    double score = aether_reputation_get_score(&svc, "alice");
    assert(fabs(score - 0.95) < 1e-9);
}
...
static void delivery_failure_gives_0_98(void)
{
    ...
    aether_reputation_record_delivery_failure(&svc, "alice");
    double score = aether_reputation_get_score(&svc, "alice");
...
static void five_sig_failures_floors_to_zero(void)
{
    ...
    for (int i = 0; i < 5; i++) {
        aether_reputation_record_sig_failure(&svc, "attacker");
    }
    double score = aether_reputation_get_score(&svc, "attacker");
...
static void ten_delivery_successes_ceil_to_one(void)
{
    ...
    for (int i = 0; i < 10; i++) {
        aether_reputation_record_delivery_success(&svc, "good-node", 50);
    }
    double score = aether_reputation_get_score(&svc, "good-node");
    assert(fabs(score - 1.0) < 1e-9);
}
...
static void no_cross_contamination(void)
{
    ...
    double bob_score = aether_reputation_get_score(&svc, "bob");
    assert(fabs(bob_score - 1.0) < 1e-9);

    // alice is at 0.0.
    double alice_score = aether_reputation_get_score(&svc, "alice");
...
static void compound_signals_give_0_60(void)
{
    ...
    aether_reputation_record_rreq_flood(&svc, "node");
    aether_reputation_record_replay(&svc, "node");
    aether_reputation_record_sig_failure(&svc, "node");

    double score = aether_reputation_get_score(&svc, "node");
