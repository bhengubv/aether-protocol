...
// commit f873543) and the C# Aether.Benchmarks suite — same hot paths so
...
// AETHER_BENCH_ITERATIONS for tighter signal.
...
static double bench_x25519_agree(size_t iterations) {
    ...
    for (size_t i = 0; i < iterations; i++) {
        aether_x25519_agree(alice_priv, bob_pub, shared);
    }
    uint64_t end = now_ns();
...
static double bench_hkdf_sha256_64b(size_t iterations) {
    ...
    aether_random_bytes(salt, sizeof(salt));
    aether_random_bytes(ikm,  sizeof(ikm));
    const uint8_t info[] = "aether-ratchet-rk-v1";
    uint8_t out[64];

    uint64_t start = now_ns();
    for (size_t i = 0; i < iterations; i++) {
        aether_hkdf_sha256(salt, sizeof(salt), ikm, sizeof(ikm),
...
static double bench_signal_kdf_rk(size_t iterations) {
    uint8_t rk[32], dh[32];
    aether_random_bytes(rk, sizeof(rk));
    aether_random_bytes(dh, sizeof(dh));
    uint8_t new_rk[32], new_ck[32];

    uint64_t start = now_ns();
    for (size_t i = 0; i < iterations; i++) {
        aether_signal_kdf_rk(rk, dh, new_rk, new_ck);
    }
    uint64_t end = now_ns();
...
static double bench_hmac_sha256(size_t iterations) {
    ...
    aether_random_bytes(msg, sizeof(msg));
    uint8_t out[32];

    uint64_t start = now_ns();
    for (size_t i = 0; i < iterations; i++) {
        aether_hmac_sha256(key, sizeof(key), msg, sizeof(msg), out);
    }
    uint64_t end = now_ns();
    touch(out, sizeof(out));
    return (double)(end - start) / (double)iterations;
...
static double bench_aes256_gcm_encrypt(size_t iterations) {
    ...
    aether_random_bytes(plaintext, sizeof(plaintext));
    ...
    uint64_t start = now_ns();
    for (size_t i = 0; i < iterations; i++) {
        aether_aes256_gcm_encrypt(plaintext, sizeof(plaintext), key, NULL, NULL, 0,
...
static double bench_aes256_gcm_decrypt(size_t iterations) {
    ...
    uint64_t start = now_ns();
    for (size_t i = 0; i < iterations; i++) {
        aether_aes256_gcm_decrypt(ciphertext, sizeof(ciphertext), key,
...
static double bench_ed25519_verify(size_t iterations) {
    ...
    uint64_t start = now_ns();
    for (size_t i = 0; i < iterations; i++) {
        (void)aether_ed25519_verify(pub, msg, sizeof(msg), sig);
    }
    uint64_t end = now_ns();
    return (double)(end - start) / (double)iterations;
}
...
static double bench_x25519_derive_public(size_t iterations) {
    uint8_t priv[32];
    aether_random_bytes(priv, sizeof(priv));
    uint8_t pub[32];

    uint64_t start = now_ns();
    for (size_t i = 0; i < iterations; i++) {
        aether_x25519_derive_public(priv, pub);
    }
    uint64_t end = now_ns();
...
static double bench_x25519_generate_keypair(size_t iterations) {
    ...
    uint64_t start = now_ns();
    for (size_t i = 0; i < iterations; i++) {
        aether_x25519_generate_keypair(priv, pub);
    }
    uint64_t end = now_ns();
    touch(pub, sizeof(pub));
    return (double)(end - start) / (double)iterations;
...
static double bench_sha256_1kb(size_t iterations) {
    uint8_t input[1024];
    aether_random_bytes(input, sizeof(input));
    uint8_t out[32];

    uint64_t start = now_ns();
    for (size_t i = 0; i < iterations; i++) {
        aether_sha256(input, sizeof(input), out);
    }
    uint64_t end = now_ns();
    touch(out, sizeof(out));
    return (double)(end - start) / (double)iterations;
...
int main(void) {
    ...
    const char *iter_env = getenv("AETHER_BENCH_ITERATIONS");
    ...
    fprintf(stderr, "aether-protocol C bench: %zu iterations / case\n", iterations);
