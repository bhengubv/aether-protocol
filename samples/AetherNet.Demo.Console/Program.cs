...
// ═══════════════════════════════════════════════════════════════════════════════
// STEP 11 — AetherTag: your shareable identity (the 1:1 handle)
PrintStep(11, "AetherTag — Shareable Identity for 1:1");

var aliceTag = AetherNetTag.FromPublicKey(alicePubKey);
var bobTag = AetherNetTag.FromPublicKey(bobPubKey);
var charlieTag = AetherNetTag.FromPublicKey(charliePubKey);

PrintNode("Alice", ConsoleColor.Cyan, $"AetherTag: {aliceTag.Value}");
PrintNode("Bob", ConsoleColor.Green, $"AetherTag: {bobTag.Value}");
PrintNode("Charlie", ConsoleColor.Yellow, $"AetherTag: {charlieTag.Value}");
Console.WriteLine();
PrintInfo("A tag is SHA-256 of your Ed25519 public key (50 bits, Crockford base-32) —");
PrintInfo("shareable aloud or on a card, reveals nothing private, and follows you across devices.");
Console.WriteLine();

PrintNode("Bob", ConsoleColor.Green, $"Adding Alice by tag {aliceTag.Value} — verifying it against her key...");
var tagOk = AetherNetTag.Verify(aliceTag.Value, alicePubKey);
PrintDetail($"  Verify(Alice's tag, Alice's key): {(tagOk ? "MATCH — identity confirmed" : "NO MATCH")}");
var (_, impostorPub) = Ed25519SigningService.GenerateKeyPair();
var tagBad = AetherNetTag.Verify(aliceTag.Value, impostorPub);
PrintWarning($"Verify(Alice's tag, impostor key): {(tagBad ? "MATCH (BAD!)" : "REJECTED")} — a tag can't be forged onto another key.");
Console.WriteLine();
PrintInfo("One-on-one: two people swap tags, each verifies + adds the other, then talk E2E (Steps 5 & 9).");
Pause();

// STEP 12 — Group messaging over named channels (live 3-node network)
PrintStep(12, "Group Text — Named Channels");

InProcessTransportService.ResetNetwork();
const string gAlice = "aether:alice:12";
const string gBob = "aether:bob:12";
const string gCharlie = "aether:charlie:12";
using var gAliceT = new InProcessTransportService(gAlice, transportLogger);
using var gBobT = new InProcessTransportService(gBob, transportLogger);
using var gCharlieT = new InProcessTransportService(gCharlie, transportLogger);

var gAliceSender = new InProcessMeshSender(gAlice, gAliceT);
gAliceSender.AddPotentialPeer(gBob); gAliceSender.AddPotentialPeer(gCharlie);
var gBobSender = new InProcessMeshSender(gBob, gBobT);
gBobSender.AddPotentialPeer(gAlice); gBobSender.AddPotentialPeer(gCharlie);
var gCharlieSender = new InProcessMeshSender(gCharlie, gCharlieT);
gCharlieSender.AddPotentialPeer(gAlice); gCharlieSender.AddPotentialPeer(gBob);

var aliceChan = new ChannelMessageService(gAliceSender);
var bobChan = new ChannelMessageService(gBobSender);
var charlieChan = new ChannelMessageService(gCharlieSender);
var aliceVid = new VideoCallControlService(gAliceSender);
...
bobChan.MessageReceived += (_, m) =>
    PrintNode("Bob", ConsoleColor.Green, $"received on {m.ChannelId}: \"{m.Content}\"");
charlieChan.MessageReceived += (_, m) =>
    PrintNode("Charlie", ConsoleColor.Yellow, $"received on {m.ChannelId}: \"{m.Content}\"");

PrintNode("Alice", ConsoleColor.Cyan, $"Alice, Bob, Charlie all subscribed to {watchChannel}");
PrintNode("Alice", ConsoleColor.Cyan, "Publishing one group message...");
var reached = await aliceChan.PublishAsync(watchChannel, "Load-shedding at 6 — charge your phones.");
await Task.Delay(120); // let the flood + async dispatch settle
Console.WriteLine();
PrintInfo($"One publish -> flooded to {reached} peer(s); every subscriber surfaced it. That is the group.");
Pause();

// STEP 13 — Video calls: 1:1 and group (call control)
PrintStep(13, "Video Calls — 1:1 and Group (call control)");

aliceVid.CallStateChanged += (_, e) =>
    PrintNode("Alice", ConsoleColor.Cyan, $"call {Short(e.CallId)}: '{e.Action}' from {e.FromUhid}");
bobVid.CallStateChanged += (_, e) =>
    PrintNode("Bob", ConsoleColor.Green, $"call {Short(e.CallId)}: '{e.Action}' from {e.FromUhid}");
charlieVid.CallStateChanged += (_, e) =>
    PrintNode("Charlie", ConsoleColor.Yellow, $"call {Short(e.CallId)}: '{e.Action}' from {e.FromUhid}");

PrintInfo("[1:1] Alice rings Bob:");
var call1 = await aliceVid.RingAsync(gBob);
await Task.Delay(70);
await bobVid.AcceptAsync(call1, gAlice);
await Task.Delay(70);
PrintDetail("  -> connected. Media (frames / SDP / ICE) rides the streaming layer. Alice hangs up.");
await aliceVid.HangupAsync(call1, gBob);
await Task.Delay(70);
Console.WriteLine();

PrintInfo("[Group] Alice rings Bob AND Charlie:");
var callToBob = await aliceVid.RingAsync(gBob);
var callToCharlie = await aliceVid.RingAsync(gCharlie);
await Task.Delay(70);
await bobVid.AcceptAsync(callToBob, gAlice);
await charlieVid.AcceptAsync(callToCharlie, gAlice);
await Task.Delay(90);
Console.WriteLine();
PrintInfo("Both accepted -> a 3-way group video call, established over the control plane.");
Pause();
...
Console.WriteLine("""
  ...
  ║   [11] AetherTag — shareable identity, verify-before-trust        ║
  ║   [12] Group text over named channels                            ║
  ║   [13] Video calls — 1:1 and group (call control)                ║
  ║                                                                   ║
