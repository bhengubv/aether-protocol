...
class AetherNetNodeTest {

    @Test fun `creates with required fields`() {
        val node = AetherNetNode(
            uhid = "node-001",
    ...
    @Test fun `geohash defaults to null`() {
        val node = AetherNetNode(uhid = "n", identityPublicKey = ByteArray(32))
        assertNull(node.geohash)
    }

    @Test fun `equals uses content-equality for identityPublicKey`() {
        val key = ByteArray(32) { it.toByte() }
        val n1 = AetherNetNode("n", key.copyOf())
        val n2 = AetherNetNode("n", key.copyOf())
        assertEquals(n1, n2)
    }
}
