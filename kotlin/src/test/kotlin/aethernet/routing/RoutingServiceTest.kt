...
private fun newRreq(source: String, dest: String, ttl: Int = AetherNetConstants.DEFAULT_TTL) =
    MeshPacket(type = PacketType.RouteRequest, sourceUhid = source, destinationUhid = dest, ttl = ttl)

private fun newRrep(source: String, dest: String, ttl: Int = AetherNetConstants.DEFAULT_TTL) =
...
class RoutingServiceTest {
    ...
    @Test fun rreqFloodFiresReputation(): Unit = runBlocking {
        val sender = FakeMeshSender("local")
        // Routing-mechanics test (RREQ flooding, not RREP verification); the service default is
        // now fail-closed (RejectAll), so inject an explicit accept-all verifier.
        val svc = RoutingService(sender, verifier = AcceptAllRouteReplyVerifier())
        val rep = NodeReputationService()
        svc.setReputation(rep)

        // Send RREQ_RATE_LIMIT_MAX unique RREQs — still within limit
        repeat(AetherNetConstants.RREQ_RATE_LIMIT_MAX) {
    ...
    @Test fun rreqFloodWithoutReputationNoError(): Unit = runBlocking {
        val sender = FakeMeshSender("local")
        // Routing-mechanics test (RREQ flooding, not RREP verification); fail-closed default now
        // rejects, so inject accept-all. No reputation attached here.
        val svc = RoutingService(sender, verifier = AcceptAllRouteReplyVerifier())

        repeat(AetherNetConstants.RREQ_RATE_LIMIT_MAX + 1) {
            svc.handleRouteRequest(newRreq("attacker", "dest"))
        }
        // No exception raised
    }
