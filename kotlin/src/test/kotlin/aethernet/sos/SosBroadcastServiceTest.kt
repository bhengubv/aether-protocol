...
class SosBroadcastServiceTest {
    ...
    @Test fun broadcast_floodsAndStoresAlert() = runBlocking {
        ...
        assertEquals(AetherNetConstants.SOS_TTL, sender.broadcasts[0].ttl)
        assertEquals(AetherNetConstants.SOS_PRIORITY.toByte(), sender.broadcasts[0].priority)
        assertEquals(1, svc.getActiveAlerts().size)
    }

    @Test fun broadcast_rateLimitedAfterMax() = runBlocking {
        val (svc, _) = newSvc()
        repeat(AetherNetConstants.MAX_SOS_BROADCASTS_PER_HOUR) {
    ...
    @Test fun handle_dropsDuplicatePacketId() = runBlocking {
        val (svc, sender) = newSvc()
        val pkt = newSosPacket("alice", AetherNetConstants.SOS_TTL)
        val pktId = pkt.id
        ...
        val pkt2 = newSosPacket("alice", AetherNetConstants.SOS_TTL).apply { id = pktId }
        svc.handle(pkt2)
    ...
    @Test fun handle_ignoresSelfOriginated() = runBlocking {
        val (svc, sender) = newSvc()
        svc.handle(newSosPacket(LOCAL, AetherNetConstants.SOS_TTL))
        assertEquals(0, sender.broadcasts.size)
    }
