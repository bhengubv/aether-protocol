...
package aethermesh.identity

import org.junit.jupiter.api.Test
import kotlin.test.*

/**
 * Tests for [AetherMeshTag] — the human-readable identity address primitive.
 *
 * The implementation:
 *   SHA-256(publicKey) → first 50 bits → 10 Crockford base-32 chars → "XXXXX-XXXXX"
 */
class AetherMeshTagTest {

    // ── Known-vector ──────────────────────────────────────────────────────────

    @Test fun `fromPublicKey produces XXXXX-XXXXX format`() {
        val key = ByteArray(32) { it.toByte() }
        val tag = AetherMeshTag.fromPublicKey(key)
        val value = tag.value
    ...
    @Test fun `fromPublicKey for all-zeros key produces consistent output`() {
        ...
        val tag1 = AetherMeshTag.fromPublicKey(key)
        val tag2 = AetherMeshTag.fromPublicKey(key)
    ...
    @Test fun `same key always produces the same tag`() {
        ...
        val tags = (1..10).map { AetherMeshTag.fromPublicKey(key.copyOf()) }
    ...
    @Test fun `different keys produce different tags`() {
        ...
        assertNotEquals(AetherMeshTag.fromPublicKey(key1), AetherMeshTag.fromPublicKey(key2))
    }
    ...
    @Test fun `fromPublicKey → toString → parse round-trips`() {
        ...
        val original = AetherMeshTag.fromPublicKey(key)
        val parsed = AetherMeshTag.parse(original.toString())
    ...
    @Test fun `verify returns true when tag matches key`() {
        val key = ByteArray(32) { it.toByte() }
        val tag = AetherMeshTag.fromPublicKey(key).value
        assertTrue(AetherMeshTag.verify(tag, key))
    }

    @Test fun `verify returns false when key is wrong`() {
        val key1 = ByteArray(32) { it.toByte() }
        val key2 = ByteArray(32) { (it + 50).toByte() }
        val tag = AetherMeshTag.fromPublicKey(key1).value
        assertFalse(AetherMeshTag.verify(tag, key2))
    }

    @Test fun `verify returns false for invalid tag string`() {
        val key = ByteArray(32) { it.toByte() }
        assertFalse(AetherMeshTag.verify("not-a-tag", key))
        assertFalse(AetherMeshTag.verify("", key))
    }

    @Test fun `verify returns false for key with wrong length`() {
        val key = ByteArray(32) { it.toByte() }
        val tag = AetherMeshTag.fromPublicKey(key).value
        assertFalse(AetherMeshTag.verify(tag, ByteArray(16)))
    }
    ...
    @Test fun `parse accepts canonical XXXXX-XXXXX format`() {
        val key = ByteArray(32) { it.toByte() }
        val canonical = AetherMeshTag.fromPublicKey(key).value
        val parsed = AetherMeshTag.parse(canonical)
    ...
    @Test fun `parse accepts 10-char input without separator`() {
        ...
        val parsed = AetherMeshTag.parse(noSep)
        assertEquals(canonical, parsed.value)
    }
    ...
    @Test fun `parse accepts lowercase input`() {
        ...
        val parsed = AetherMeshTag.parse(lower)
        assertEquals(canonical, parsed.value)
    }
    ...
    @Test fun `parse accepts mixed-case input`() {
        ...
        val parsed = AetherMeshTag.parse(mixed)
        assertEquals(canonical, parsed.value)
    }
    ...
    @Test fun `parse throws for wrong total length — too short`() {
        assertFailsWith<IllegalArgumentException> {
            AetherMeshTag.parse("ABCD-ABCD")  // 9 chars
        }
    }

    @Test fun `parse throws for wrong total length — too long`() {
        assertFailsWith<IllegalArgumentException> {
            AetherMeshTag.parse("ABCDE-ABCDEF")  // 12 chars
        }
    }
    ...
    @Test fun `tryParse returns AetherMeshTag for valid input`() {
        val key = ByteArray(32) { it.toByte() }
        val canonical = AetherMeshTag.fromPublicKey(key).value
        assertNotNull(AetherMeshTag.tryParse(canonical))
    }

    @Test fun `tryParse returns null for invalid input`() {
        assertNull(AetherMeshTag.tryParse(""))
        assertNull(AetherMeshTag.tryParse("INVALID"))
        assertNull(AetherMeshTag.tryParse("IIIII-IIIII"))
        assertNull(AetherMeshTag.tryParse("toolong-toolong"))
    }
    ...
    @Test fun `isValid is false for empty-value AetherMeshTag`() {
        val tag = AetherMeshTag("")
    ...
    @Test fun `fromPublicKey throws for key shorter than 32 bytes`() {
        assertFailsWith<IllegalArgumentException> {
            AetherMeshTag.fromPublicKey(ByteArray(16))
        }
    }

    @Test fun `fromPublicKey throws for key longer than 32 bytes`() {
        assertFailsWith<IllegalArgumentException> {
            AetherMeshTag.fromPublicKey(ByteArray(64))
        }
    }

    @Test fun `fromPublicKey throws for empty key`() {
        assertFailsWith<IllegalArgumentException> {
            AetherMeshTag.fromPublicKey(ByteArray(0))
        }
    }
