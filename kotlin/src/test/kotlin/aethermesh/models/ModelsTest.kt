...
class AetherMeshNodeTest {

    @Test fun `creates with required fields`() {
        val node = AetherMeshNode(
            uhid = "node-001",
    ...
    @Test fun `geohash defaults to null`() {
        val node = AetherMeshNode(uhid = "n", identityPublicKey = ByteArray(32))
        assertNull(node.geohash)
    }

    @Test fun `equals uses content-equality for identityPublicKey`() {
        val key = ByteArray(32) { it.toByte() }
        val n1 = AetherMeshNode("n", key.copyOf())
        val n2 = AetherMeshNode("n", key.copyOf())
        assertEquals(n1, n2)
    }
}
