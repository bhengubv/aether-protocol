...
private fun newRreq(source: String, dest: String, ttl: Int = AetherMeshConstants.DEFAULT_TTL) =
    MeshPacket(type = PacketType.RouteRequest, sourceUhid = source, destinationUhid = dest, ttl = ttl)

private fun newRrep(source: String, dest: String, ttl: Int = AetherMeshConstants.DEFAULT_TTL) =
...
class RoutingServiceTest {
    ...
    @Test fun rreqFloodFiresReputation(): Unit = runBlocking {
        ...
        repeat(AetherMeshConstants.RREQ_RATE_LIMIT_MAX) {
    ...
    @Test fun rreqFloodWithoutReputationNoError(): Unit = runBlocking {
        ...
        repeat(AetherMeshConstants.RREQ_RATE_LIMIT_MAX + 1) {
            svc.handleRouteRequest(newRreq("attacker", "dest"))
        }
        // No exception raised
    }
