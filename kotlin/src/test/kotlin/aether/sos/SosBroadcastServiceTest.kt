...
private fun newSosPacket(source: String, ttl: Int): MeshPacket {
    ...
    return MeshPacket(
        ...
        destinationUhid = "",
        ttl = ttl,
        priority = AetherConstants.SOS_PRIORITY.toByte(),
...
class SosBroadcastServiceTest {
    ...
    @Test fun broadcast_floodsAndStoresAlert() = runBlocking {
        ...
        assertEquals(AetherConstants.SOS_TTL, sender.broadcasts[0].ttl)
        assertEquals(AetherConstants.SOS_PRIORITY.toByte(), sender.broadcasts[0].priority)
        assertEquals(1, svc.getActiveAlerts().size)
    }

    @Test fun broadcast_rateLimitedAfterMax() = runBlocking {
        val (svc, _) = newSvc()
        repeat(AetherConstants.MAX_SOS_BROADCASTS_PER_HOUR) {
    ...
    @Test fun handle_dropsDuplicatePacketId() = runBlocking {
        val (svc, sender) = newSvc()
        val pkt = newSosPacket("alice", AetherConstants.SOS_TTL)
        val pktId = pkt.id
        ...
        val pkt2 = newSosPacket("alice", AetherConstants.SOS_TTL).apply { id = pktId }
        svc.handle(pkt2)
    ...
    @Test fun handle_ignoresSelfOriginated() = runBlocking {
        val (svc, sender) = newSvc()
        svc.handle(newSosPacket(LOCAL, AetherConstants.SOS_TTL))
        assertEquals(0, sender.broadcasts.size)
    }
