 ...
 */
data class AetherTag(val value: String) {
    ...
    companion object {
         ...
         */
        fun fromPublicKey(publicKey: ByteArray): AetherTag {
         ...
         *
         * @throws IllegalArgumentException if the string is not a valid AetherTag.
         */
        fun parse(tag: String): AetherTag {
            ...
            val canonical = when (upper.length) {
                ...
                10 -> "${upper.substring(0, 5)}-${upper.substring(5)}"  // insert dash
                    ...
                    "Invalid AetherTag format: expected 10 or 11 characters (with optional '-'), got \"$tag\""
                )
            }
            if (!VALID_PATTERN.matches(canonical)) {
                throw IllegalArgumentException(
                    "Invalid AetherTag: contains characters outside the Crockford base-32 alphabet: \"$tag\""
                )
            }
            return AetherTag(canonical)
        }

        /**
         * Attempts to parse a tag string; returns null if the string is not valid.
         */
        fun tryParse(tag: String): AetherTag? = try {
            parse(tag)
        ...
        /**
         * Returns true if [tag] is a valid AetherTag string that matches the given public key.
         */
