...
class RoutingService(
    ...
    // Fail-closed: with no verifier supplied, REJECT every RREP rather than trusting an unverified
    // route reply. A host wires Ed25519RouteReplyVerifier (or another real verifier) to accept
    // legitimate, signature-verified RREPs. AcceptAllRouteReplyVerifier is an explicit INSECURE
    // opt-in for routing-mechanics tests only — never the default.
    private val verifier: RouteReplyVerifier = RejectAllRouteReplyVerifier(),
