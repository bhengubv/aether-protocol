 ...
 */
data class AetherMeshTag(val value: String) {
    ...
    companion object {
         ...
         */
        fun fromPublicKey(publicKey: ByteArray): AetherMeshTag {
         ...
         *
         * @throws IllegalArgumentException if the string is not a valid AetherMeshTag.
         */
        fun parse(tag: String): AetherMeshTag {
            ...
            val canonical = when (upper.length) {
                ...
                10 -> "${upper.substring(0, 5)}-${upper.substring(5)}"  // insert dash
                    ...
                    "Invalid AetherMeshTag format: expected 10 or 11 characters (with optional '-'), got \"$tag\""
                )
            }
            if (!VALID_PATTERN.matches(canonical)) {
                throw IllegalArgumentException(
                    "Invalid AetherMeshTag: contains characters outside the Crockford base-32 alphabet: \"$tag\""
                )
            }
            return AetherMeshTag(canonical)
        }

        /**
         * Attempts to parse a tag string; returns null if the string is not valid.
         */
        fun tryParse(tag: String): AetherMeshTag? = try {
            parse(tag)
        ...
        /**
         * Returns true if [tag] is a valid AetherMeshTag string that matches the given public key.
         */
