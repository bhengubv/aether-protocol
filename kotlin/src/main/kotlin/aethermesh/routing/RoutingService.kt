...
class RoutingService(
    ...
    suspend fun handleRouteRequest(rreq: MeshPacket) {
        ...
        val windowStart = nowMs - AetherMeshConstants.RREQ_RATE_LIMIT_WINDOW_MS
        ...
        synchronized(existing) {
            ...
            if (existing.size >= AetherMeshConstants.RREQ_RATE_LIMIT_MAX) {
        ...
        val hopCount = (AetherMeshConstants.DEFAULT_TTL - rreq.ttl + 1).coerceAtLeast(1)
        val reverse = RouteEntry(
            destinationUhid = rreq.sourceUhid,
            nextHopUhid = rreq.sourceUhid,
            hopCount = hopCount,
            qualityScore = 50,
            expiresAt = Instant.now().plusSeconds(AetherMeshConstants.ROUTE_EXPIRY_SECONDS)
        )
    ...
    suspend fun handleRouteReply(rrep: MeshPacket) {
        ...
        val hopCount = (AetherMeshConstants.DEFAULT_TTL - rrep.ttl + 1).coerceAtLeast(1)
    ...
    private suspend fun discover(destinationUhid: String): RouteEntry? {
        ...
        return try {
            withTimeoutOrNull(AetherMeshConstants.ROUTE_TIMEOUT_MS) { deferred.await() }
