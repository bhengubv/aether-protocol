...
- **Consumer protocol surface (Wave 16 — 2.0 candidates):** `IDtnService.BundleReceived` event for inbound bundles ([#59](https://github.com/bhengubv/aether-protocol/issues/59)), application-layer naming/discovery directory ([#60](https://github.com/bhengubv/aether-protocol/issues/60) — biggest of the three; `IContentService` is hash-keyed, mesh-first fetchers have no way to resolve by name), and author-tipping interface ([#61](https://github.com/bhengubv/aether-protocol/issues/61) — `IAetherNetIncentiveProvider` only models relay credit, not creator payment). Surfaced by `aether-media` commit `07a695e`.
