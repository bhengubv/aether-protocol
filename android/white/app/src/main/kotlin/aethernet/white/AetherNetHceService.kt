...
private const val TAG        = "AetherNetHCE"
private const val AETHERNET_AID = "F061657468657200"
 ...
 *
 * Registers AID [AETHERNET_AID] with the Android NFC stack.  When an NFC reader
 ...
 *
 * Usage:
 *   AetherNetHceService.listener = myListener   // set from MainActivity
 */
class AetherNetHceService : HostApduService() {
    ...
    companion object {
         ...
         * Aether HCE AID ([AETHERNET_AID]). Checks header bytes, Lc length, and AID
        ...
        internal fun isSelectAid(apdu: ByteArray): Boolean {
            ...
            return aid.equals(AETHERNET_AID, ignoreCase = true)
        }
        ...
        internal fun buildSelectAidApdu(): ByteArray {
            val aidBytes = AETHERNET_AID.chunked(2).map { it.toInt(16).toByte() }.toByteArray()
    ...
    override fun processCommandApdu(commandApdu: ByteArray, extras: Bundle?): ByteArray {
        ...
        return when {
            isSelectAid(commandApdu) -> {
                Log.i(TAG, "SELECT AID $AETHERNET_AID — OK")
