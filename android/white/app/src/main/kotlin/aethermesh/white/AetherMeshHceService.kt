...
private const val TAG        = "AetherMeshHCE"
private const val AETHERMESH_AID = "F061657468657200"
 ...
 *
 * Registers AID [AETHERMESH_AID] with the Android NFC stack.  When an NFC reader
 ...
 *
 * Usage:
 *   AetherMeshHceService.listener = myListener   // set from MainActivity
 */
class AetherMeshHceService : HostApduService() {
    ...
    companion object {
         ...
         * Aether HCE AID ([AETHERMESH_AID]). Checks header bytes, Lc length, and AID
        ...
        internal fun isSelectAid(apdu: ByteArray): Boolean {
            ...
            return aid.equals(AETHERMESH_AID, ignoreCase = true)
        }
        ...
        internal fun buildSelectAidApdu(): ByteArray {
            val aidBytes = AETHERMESH_AID.chunked(2).map { it.toInt(16).toByte() }.toByteArray()
    ...
    override fun processCommandApdu(commandApdu: ByteArray, extras: Bundle?): ByteArray {
        ...
        return when {
            isSelectAid(commandApdu) -> {
                Log.i(TAG, "SELECT AID $AETHERMESH_AID — OK")
