 ...
 * [AetherMeshHceService.Companion].
 *
 * Covers ISO 7816 SELECT AID parsing ([AetherMeshHceService.isSelectAid]) and
 * APDU summary generation ([AetherMeshHceService.parsePacketSummary]).
 * Runs on the JVM — no emulator, no Android framework required.
 *
 * The Aether HCE AID is `F061657468657200` (8 bytes, "aether" ASCII + NUL).
 */
class AetherMeshHceServiceTest {

    // ── isSelectAid — valid SELECT AID ───────────────────────────────────────

    @Test fun isSelectAid_wellFormedSelectAid_returnsTrue() {
        val apdu = AetherMeshHceService.buildSelectAidApdu()
        assertTrue("Well-formed SELECT AID must be recognised", AetherMeshHceService.isSelectAid(apdu))
    }
    ...
    @Test fun isSelectAid_wellFormedSelectAid_caseInsensitive() {
        ...
        val apdu = AetherMeshHceService.buildSelectAidApdu() // produces upper-case hex, still a byte array
        assertTrue(AetherMeshHceService.isSelectAid(apdu))
    }

    // ── isSelectAid — too-short inputs ────────────────────────────────────────

    @Test fun isSelectAid_emptyApdu_returnsFalse() {
        assertFalse(AetherMeshHceService.isSelectAid(ByteArray(0)))
    }

    @Test fun isSelectAid_4ByteApdu_tooShort_returnsFalse() {
        assertFalse(AetherMeshHceService.isSelectAid(byteArrayOf(0x00, 0xA4.toByte(), 0x04, 0x00)))
    }

    @Test fun isSelectAid_5ByteApdu_lcButNoAidBytes_returnsFalse() {
        // Header OK, lc=8 but zero AID bytes follow → size < 5 + 8
        assertFalse(AetherMeshHceService.isSelectAid(byteArrayOf(0x00, 0xA4.toByte(), 0x04, 0x00, 0x08)))
    }

    // ── isSelectAid — wrong header ────────────────────────────────────────────

    @Test fun isSelectAid_wrongCla_returnsFalse() {
        val apdu = AetherMeshHceService.buildSelectAidApdu().copyOf()
        apdu[0] = 0x80.toByte() // wrong CLA (should be 0x00)
        assertFalse(AetherMeshHceService.isSelectAid(apdu))
    }
    ...
    @Test fun isSelectAid_randomBytes_returnsFalse() {
        assertFalse(AetherMeshHceService.isSelectAid(ByteArray(16) { 0xFF.toByte() }))
    }

    // ── isSelectAid — lc truncation ───────────────────────────────────────────

    @Test fun isSelectAid_lcLargerThanRemainingBytes_returnsFalse() {
        val base = AetherMeshHceService.buildSelectAidApdu()
    ...
    @Test fun parsePacketSummary_selectAidApdu_showsCorrectHeaderAndSize() {
        val apdu = AetherMeshHceService.buildSelectAidApdu()
        val s    = AetherMeshHceService.parsePacketSummary(apdu)
        assertTrue("Expected CLA=00 in: $s",              s.contains("CLA=00"))
    ...
    @Test fun parsePacketSummary_ffBytes_showsHexUpperCase() {
        val s = AetherMeshHceService.parsePacketSummary(byteArrayOf(0xFF.toByte(), 0xFF.toByte()))
    ...
    @Test fun buildSelectAidApdu_isRecognisedByIsSelectAid() {
        assertTrue(AetherMeshHceService.isSelectAid(AetherMeshHceService.buildSelectAidApdu()))
    }
}
