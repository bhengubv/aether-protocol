 ...
 * [AetherNetWifiDirectService.Companion].
 *
 ...
 */
class AetherNetWifiDirectServiceTest {
    ...
    @Test fun summary_emptyBuffer_reportsTooShort() {
        val s = AetherNetWifiDirectService.parsePacketSummary(ByteArray(0))
        assertTrue("Expected 'too short' in: $s", s.contains("too short"))
    }

    @Test fun summary_30ByteBuffer_reportsTooShort() {
        val s = AetherNetWifiDirectService.parsePacketSummary(ByteArray(30))
        assertTrue("Expected 'too short' in: $s", s.contains("too short"))
    ...
    @Test fun summary_valid31BytePacket_extractsAllFields() {
        val s = AetherNetWifiDirectService.parsePacketSummary(
    ...
    @Test fun summary_zeroTtl_reportedCorrectly() {
        val s = AetherNetWifiDirectService.parsePacketSummary(minPacket(ttl = 0))
    ...
    @Test fun summary_largerPacket_reportsTotalByteCount() {
        val s = AetherNetWifiDirectService.parsePacketSummary(minPacket(extra = 69)) // 100 B
    ...
    @Test fun echo_returnsCopyNotSameReference() {
        ...
        assertNotSame(data, AetherNetWifiDirectService.buildEchoResponse(data))
    }

    @Test fun echo_packetShorterThan24Bytes_returnedUnchanged() {
        val data = ByteArray(20) { it.toByte() }
        assertArrayEquals(data, AetherNetWifiDirectService.buildEchoResponse(data))
    }

    @Test fun echo_ttl7_decrementsTo6() {
        val echo = AetherNetWifiDirectService.buildEchoResponse(minPacket(ttl = 7))
        assertEquals(6, readTtl(echo))
    }

    @Test fun echo_ttl1_decrementsTo0() {
        val echo = AetherNetWifiDirectService.buildEchoResponse(minPacket(ttl = 1))
        assertEquals(0, readTtl(echo))
    }

    @Test fun echo_ttl0_clampedTo0() {
        val echo = AetherNetWifiDirectService.buildEchoResponse(minPacket(ttl = 0))
    ...
    @Test fun echo_nonTtlBytesUnchanged() {
        ...
        val echo = AetherNetWifiDirectService.buildEchoResponse(data)
    ...
    @Test fun frameHeader_zero_isFourZeroBytes() {
        val h = AetherNetWifiDirectService.buildFrameHeader(0)
        assertEquals(4, h.size)
        assertArrayEquals(byteArrayOf(0, 0, 0, 0), h)
    }

    @Test fun frameHeader_1_isLittleEndian() {
        val h = AetherNetWifiDirectService.buildFrameHeader(1)
        assertEquals(4, h.size)
    ...
    @Test fun frameHeader_256_isLittleEndian() {
        val h = AetherNetWifiDirectService.buildFrameHeader(256)
    ...
    @Test fun frameHeader_65536_isLittleEndian() {
        val h = AetherNetWifiDirectService.buildFrameHeader(65536)
    ...
    @Test fun parseFrameLength_roundTrips_withBuildFrameHeader() {
        listOf(0, 1, 255, 256, 65535, 65536, 1_000_000).forEach { len ->
            val header = AetherNetWifiDirectService.buildFrameHeader(len)
            val parsed = AetherNetWifiDirectService.parseFrameLength(header)
            assertEquals("Round-trip failed for len=$len", len, parsed)
        }
    }

    @Test fun parseFrameLength_wrongSizeHeader_returnsNegativeOne() {
        assertEquals(-1, AetherNetWifiDirectService.parseFrameLength(ByteArray(0)))
        assertEquals(-1, AetherNetWifiDirectService.parseFrameLength(ByteArray(3)))
        assertEquals(-1, AetherNetWifiDirectService.parseFrameLength(ByteArray(5)))
    }

    @Test fun parseFrameLength_allZeroHeader_returnsZero() {
        assertEquals(0, AetherNetWifiDirectService.parseFrameLength(ByteArray(4)))
    }
}
