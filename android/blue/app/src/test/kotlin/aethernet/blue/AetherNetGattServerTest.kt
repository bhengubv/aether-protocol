 ...
 * [AetherNetGattServer.Companion].
 *
 * These tests run on the JVM via Gradle's `test` task — no emulator needed.
 * They cover [AetherNetGattServer.parsePacketSummary] (wire-format header parsing)
 * and [AetherNetGattServer.buildEchoResponse] (TTL-decrement echo logic), which
 * are the correctness-critical paths in the BLE GATT transport.
 */
class AetherNetGattServerTest {
    ...
    @Test fun summary_emptyBuffer_reportsTooShort() {
        val s = AetherNetGattServer.parsePacketSummary(ByteArray(0))
    ...
    @Test fun summary_oneByteBuffer_reportsTooShort() {
        val s = AetherNetGattServer.parsePacketSummary(ByteArray(1))
        assertTrue("Expected 'too short' in: $s", s.contains("too short"))
    }

    @Test fun summary_30ByteBuffer_reportsTooShort() {
        val s = AetherNetGattServer.parsePacketSummary(ByteArray(30))
        assertTrue("Expected 'too short' in: $s", s.contains("too short"))
    ...
    @Test fun summary_valid31BytePacket_extractsAllFields() {
        val s = AetherNetGattServer.parsePacketSummary(minPacket(version = 2, type = 3, priority = 10, ttl = 7))
    ...
    @Test fun summary_largeTtl_reportedCorrectly() {
        val s = AetherNetGattServer.parsePacketSummary(minPacket(ttl = 1_000_000))
        assertTrue("Expected large ttl in: $s", s.contains("ttl=1000000"))
    }

    @Test fun summary_longerPacket_reportsTotalByteCount() {
        val s = AetherNetGattServer.parsePacketSummary(minPacket(extra = 69)) // 31 + 69 = 100 B
    ...
    @Test fun echo_returnsCopyNotSameReference() {
        ...
        val echo = AetherNetGattServer.buildEchoResponse(data)
