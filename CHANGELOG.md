...
serverless-integration closure found four loose ends; all four are closed here. **No mesh
wire-serialization or fixture byte changed** — the new shared signalling fixture equals the exact
bytes all 8 languages already produced.

### Added

- **Shared WebRTC signalling fixture** (`fixtures/webrtc/`) — the SDP/ICE `AWS1`+JSON handshake frame
  is now pinned by ONE cross-language byte corpus (`inputs.json` + generated `expected/*.bin`),
  mirroring `fixtures/circuit-relay/`, generated by the new Go oracle `go/cmd/webrtcfixturegen`. Five
  canonical cases cover offer/answer/candidate plus the tricky vectors: base64 `+`, `< > &` →
  uppercase `\uXXXX`, non-ASCII (`ç é 世`) → `\uXXXX`, and `SdpMLineIndex` 0 and 3. **All 8 SDKs'
  signalling tests now READ this shared fixture and byte-compare** — the per-language hardcoded
  golden literals are gone, so editing the fixture fails all 8. C# gained an internal byte-neutral
  `Frame`/`Deframe`; Go gained exported `FrameSignal`/`DeframeSignal` (the bytes `SendSignal` sends
  are unchanged).

### Fixed

- **Stale "Go diverges" comments** — the TypeScript carrier doc-comment claimed the Go carrier uses
  `encoding/json` and diverges on `+ < > &`; untrue since 2.4.0 (Go hand-rolls the STJ-exact
  escaping). Corrected the TS comment and reframed the Rust one so no comment implies any SDK's
  signalling frame diverges — all 8 match the C# reference byte-for-byte.
- **Stale fixture count in the design docs** — `docs/index.md`, `docs/PROTOCOL_SPEC.md`,
  `docs/THREAT_MODEL.md` and their 10 translated `PROTOCOL_SPEC`/`THREAT_MODEL` said "10
  …fixtures/cases"; corrected to **17** wire-format fixtures (matching disk and the READMEs). The
  stale, unverifiable "122 assertions" count was dropped for a qualitative statement, and
  `OPEN_ISSUES.md`'s "14 fixtures / 4 Signal vectors" corrected to 17 / 6.

### Tests

- `TEST_RUN_2.4.1.md` records a real per-language run: the signalling fixture + kept suites pass in
  all 8 languages (Go/TypeScript/Python/C#/Kotlin/Rust on Windows; Swift + C on the macOS build
  server). Out of scope and unchanged: the aiortc real-peer WebRTC P2P byte-exchange tests
  (`test_transport_webrtc.py`), which need a live network path.
