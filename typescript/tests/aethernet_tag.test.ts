 ...
 *
 * Run with: tsx --test typescript/tests/aethernet_tag.test.ts
 */
...
import { AetherNetTag } from "../src/identity/AetherNetTag.js";
...
describe("AetherNetTag — format", () => {
  ...
  it("output contains only Crockford alphabet chars and dash", () => {
    ...
    for (let fill = 0; fill <= 0xff; fill += 16) {
      const tag = AetherNetTag.fromPublicKey(key(fill));
...
describe("AetherNetTag — known vector", () => {
  ...
  it("fixed all-zeros key produces a stable deterministic tag", () => {
    ...
    const tag1 = AetherNetTag.fromPublicKey(publicKey);
    const tag2 = AetherNetTag.fromPublicKey(publicKey);
  ...
  it("manually computed SHA-256 extraction matches fromPublicKey", () => {
    ...
    const tag = AetherNetTag.fromPublicKey(publicKey);
    assert.equal(tag.value, expected);
  });
});

describe("AetherNetTag — round-trip", () => {
  it("fromPublicKey → toString → parse → equals original", () => {
    const publicKey = key(0x7f);
    const original = AetherNetTag.fromPublicKey(publicKey);
    const parsed = AetherNetTag.parse(original.toString());
    assert.ok(original.equals(parsed));
    assert.equal(parsed.value, original.value);
  });

  it("toString() returns the same as .value", () => {
    const tag = AetherNetTag.fromPublicKey(key(0xab));
    assert.equal(tag.toString(), tag.value);
  });

  it("isValid is always true for constructed instances", () => {
    const tag = AetherNetTag.fromPublicKey(key(0x11));
    assert.ok(tag.isValid);
    assert.ok(AetherNetTag.parse(tag.value).isValid);
  });
});

describe("AetherNetTag — verify()", () => {
  it("correct key returns true", () => {
    const publicKey = key(0x55);
    const tag = AetherNetTag.fromPublicKey(publicKey);
    assert.ok(AetherNetTag.verify(tag.value, publicKey));
  });
  ...
  it("wrong key returns false", () => {
    ...
    const tag = AetherNetTag.fromPublicKey(publicKey);
    assert.equal(AetherNetTag.verify(tag.value, wrongKey), false);
  });

  it("invalid tag string returns false", () => {
    assert.equal(AetherNetTag.verify("not-valid-tag!!", key(0x01)), false);
  });

  it("all-zeros key verifies only against itself", () => {
    const k0 = key(0x00);
    const k1 = key(0x01);
    const tag = AetherNetTag.fromPublicKey(k0);
    assert.ok(AetherNetTag.verify(tag.value, k0));
    assert.equal(AetherNetTag.verify(tag.value, k1), false);
  });
});

describe("AetherNetTag — parse() accepts", () => {
  it("canonical uppercase with dash", () => {
    const tag = AetherNetTag.fromPublicKey(key(0x22));
    const parsed = AetherNetTag.parse(tag.value);
    assert.equal(parsed.value, tag.value);
  });

  it("10-char form without dash", () => {
    const tag = AetherNetTag.fromPublicKey(key(0x33));
    const noDash = tag.value.replace("-", "");
    const parsed = AetherNetTag.parse(noDash);
    assert.equal(parsed.value, tag.value);
  });

  it("lowercase with dash", () => {
    const tag = AetherNetTag.fromPublicKey(key(0x44));
    const lower = tag.value.toLowerCase();
    const parsed = AetherNetTag.parse(lower);
    assert.equal(parsed.value, tag.value);
  });

  it("lowercase without dash", () => {
    const tag = AetherNetTag.fromPublicKey(key(0x55));
  ...
  it("mixed case", () => {
    const tag = AetherNetTag.fromPublicKey(key(0x66));
    const mixed = tag.value
      .split("")
      .map((ch, i) => (i % 2 === 0 ? ch.toLowerCase() : ch.toUpperCase()))
      .join("");
    const parsed = AetherNetTag.parse(mixed);
    assert.equal(parsed.value, tag.value);
  });
});

describe("AetherNetTag — parse() rejects", () => {
  it("empty string", () => {
    assert.throws(() => AetherNetTag.parse(""), /invalid tag/i);
    assert.equal(AetherNetTag.tryParse(""), null);
  });

  it("too short", () => {
    assert.equal(AetherNetTag.tryParse("ABCDE"), null);
    assert.equal(AetherNetTag.tryParse("ABCDE-ABC"), null);
  });

  it("too long", () => {
    assert.equal(AetherNetTag.tryParse("ABCDE-ABCDEF"), null);
    assert.equal(AetherNetTag.tryParse("ABCDEFABCDEF"), null);
  });
  ...
  it("invalid char I (ambiguous)", () => {
    ...
    assert.equal(AetherNetTag.tryParse("!BCDE-FGHJK"), null);
  });
  ...
  it("invalid char in middle", () => {
    assert.equal(AetherNetTag.tryParse("ABC.E-FGHJK"), null);
  });

  it("wrong separator position", () => {
    // Two dashes — the replace() only strips the first, leaving 11 chars.
    assert.equal(AetherNetTag.tryParse("ABCDE--FGHJ"), null);
  });

  it("null / non-string coercion returns null", () => {
    assert.equal(AetherNetTag.tryParse(null as unknown as string), null);
    assert.equal(AetherNetTag.tryParse(undefined as unknown as string), null);
  });
});

describe("AetherNetTag — uniqueness", () => {
  it("different keys produce different tags", () => {
    const tags = new Set<string>();
    for (let fill = 0; fill < 32; fill++) {
      tags.add(AetherNetTag.fromPublicKey(key(fill)).value);
    }
  ...
  it("same key always produces the same tag", () => {
    ...
    const results = Array.from({ length: 10 }, () =>
      AetherNetTag.fromPublicKey(publicKey).value,
    );
    assert.ok(results.every((v) => v === results[0]));
  });
});

describe("AetherNetTag — equals()", () => {
  it("same key produces equal tags", () => {
    const a = AetherNetTag.fromPublicKey(key(0x11));
    const b = AetherNetTag.fromPublicKey(key(0x11));
  ...
  it("different keys produce non-equal tags", () => {
    const a = AetherNetTag.fromPublicKey(key(0x11));
    const b = AetherNetTag.fromPublicKey(key(0x22));
    assert.equal(a.equals(b), false);
  });
});

describe("AetherNetTag — fromPublicKey validation", () => {
  it("throws on key shorter than 32 bytes", () => {
    assert.throws(() => AetherNetTag.fromPublicKey(new Uint8Array(16)), /32 bytes/);
  });

  it("throws on key longer than 32 bytes", () => {
    assert.throws(() => AetherNetTag.fromPublicKey(new Uint8Array(64)), /32 bytes/);
  });

  it("throws on empty key", () => {
    assert.throws(() => AetherNetTag.fromPublicKey(new Uint8Array(0)), /32 bytes/);
  });
});
