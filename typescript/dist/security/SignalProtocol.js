 ...
 */
import { createCipheriv, createDecipheriv, createHmac, createPrivateKey, createPublicKey, diffieHellman, generateKeyPairSync, randomBytes, timingSafeEqual, } from "crypto";
import { hkdf } from "@noble/hashes/hkdf";
import { sha256 } from "@noble/hashes/sha256";
import { MAX_SKIPPED_KEYS, AES_GCM_NONCE_SIZE, AES_GCM_TAG_SIZE, } from "../constants.js";
...
export const DEFAULT_SPK_ROTATION_OPTIONS = Object.freeze({
...
function x25519DerivePublic(priv) {
    ...
    if (!pubJwk.x)
        throw new Error("X25519 derive public failed");
    return new Uint8Array(Buffer.from(pubJwk.x, "base64url"));
}
function x25519Agree(localPriv, remotePub) {
...
function skippedKey(dhrPub, counter) {
...
export class SignalProtocol {
        ...
        if (this.preKeyStore) {
            try {
                ...
                if (stored) {
                    ...
                    if (stored.localUhid)
                        this.localUhid = stored.localUhid;
                }
                else {
            ...
            try {
                ...
                const sorted = [...history.entries].sort((a, b) => a.generatedAtUnixMs - b.generatedAtUnixMs);
    ...
    persistSession(peerUhid, session) {
        if (!this.sessionStore)
            return;
    ...
    persistOneTimePreKeys() {
        if (!this.preKeyStore)
            return;
        const issued = new Set(this.preKeys.oneTimePreKeys.keys());
        for (const id of this.preKeys.availableOpkIds)
            issued.delete(id);
    ...
    toStoredSession(s) {
        return {
            ...
            skippedMessageKeys: new Map(Array.from(s.skippedMessageKeys.entries()).map(([k, v]) => [k, new Uint8Array(v)])),
    ...
    setLocalUhid(uhid) {
        if (!uhid)
            throw new Error("uhid cannot be empty");
    ...
    async encrypt(peerUhid, plaintext) {
        ...
        if (!this.localUhid) {
            throw new Error("Local UHID is not set. Call generatePreKeyBundle(uhid) or setLocalUhid(uhid) before encrypting.");
        }
    ...
    async decrypt(peerUhid, payload) {
        ...
        const senderRatchetPub = payload.senderEphemeralKeyX25519 ?? payload.initiatorEphemeralKeyX25519;
        ...
        if (cached) {
            ...
            while (session.recvCounter < payload.counter) {
                ...
                session.skippedMessageKeys.set(skippedKey(senderRatchetPub, session.recvCounter), step.messageKey);
                session.recvCounter++;
            }
            const step = ratchetStep(session.recvChainKey);
            session.recvChainKey = step.newChainKey;
    ...
    async generatePreKeyBundle(localUhid) {
        if (!localUhid)
            throw new Error("localUhid cannot be empty");
        await this.hydration;
    ...
    async rotateSignedPreKey() {
        ...
        return this.runUnderOpkLock(() => {
            ...
            const shouldRotate = history.length === 0 ||
     ...
     */
    runUnderOpkLock(body) {
        const prev = this.opkLock;
        let resolve;
        this.opkLock = new Promise((r) => { resolve = r; });
        return prev
            .catch(() => undefined)
            .then(async () => {
            try {
    ...
    async generatePreKeyBundleInner(localUhid) {
    ...
    topUpOpkPool() {
        while (this.preKeys.availableOpkIds.length < this.opkPoolSize) {
            ...
            while (this.preKeys.oneTimePreKeys.has(id)) {
                ...
                if (++attempts > 64) {
                    throw new Error("Could not allocate a non-colliding OPK id after 64 attempts. " +
    ...
    async processPreKeyBundle(bundle) {
        await this.hydration;
        const ok = Ed25519Service.verify(bundle.identityKey, bundle.signedPreKey, bundle.signedPreKeySignature);
        if (!ok)
     ...
     */
    establishResponderSession(peerUhid, payload, initiatorRatchetPub) {
        ...
        if (!spkEntry || spkEntry.privateKey.length === 0) {
            throw new Error(`PreKey message references signed pre-key id ${spkId} which is not held by this node (rotated out or never generated).`);
        }
        const opkId = payload.usedOneTimePreKeyId ?? 0;
        const otpk = this.preKeys.oneTimePreKeys.get(opkId);
        if (!otpk) {
            throw new Error(`PreKey message references one-time pre-key id ${opkId} which is not held (already consumed?).`);
        }
     ...
     */
    dhRatchetReceive(session, newRemoteEphemeralPub) {
    ...
    skipMessageKeys(session, until) {
        ...
        if (until - session.recvCounter > MAX_SKIPPED_KEYS) {
            throw new Error(`Skipped-key request exceeds maximum (${MAX_SKIPPED_KEYS}). Session must be re-established.`);
        }
        while (session.recvCounter < until) {
            const step = ratchetStep(session.recvChainKey);
            session.recvChainKey = step.newChainKey;
            session.skippedMessageKeys.set(skippedKey(session.remoteEphemeralPub, session.recvCounter), step.messageKey);
            session.recvCounter++;
        }
    }
    getPublicKey() {
