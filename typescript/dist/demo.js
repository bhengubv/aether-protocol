 ...
 */
import { MeshPacket, PacketType, PacketSerializer, Ed25519Service, SignalProtocol, InProcessTransport, signPacket, verifyPacket, } from "./index.js";
async function main() {
    ...
    console.log(`Created ${InProcessTransport.activeNodeCount} nodes in simulated network\n`);
    ...
    console.log(`Node A public key: ${Buffer.from(keyPairA.publicKey).toString("hex").substring(0, 16)}...`);
    console.log(`Node B public key: ${Buffer.from(keyPairB.publicKey).toString("hex").substring(0, 16)}...\n`);
    ...
    console.log(`Signature: ${Buffer.from(packet.signature).toString("hex").substring(0, 32)}...\n`);
    ...
    console.log(`  (Signature freshness: timestamp must be within 5 minutes)\n`);
    ...
    const plaintext = new TextEncoder().encode("Secret message: Meet me at coordinates -33.9249, 18.4241");
    try {
        const encrypted = await signalA.encrypt("node-beta-002", plaintext);
        console.log(`Plaintext: ${new TextDecoder().decode(plaintext)}`);
        console.log(`Encrypted: ${Buffer.from(encrypted.ciphertext).toString("hex").substring(0, 32)}...`);
    ...
    nodeB.onDataReceived = (sender, data) => {
        console.log(`[Node B] Received ${data.length} bytes from ${sender}`);
        receivedData = data;
    };
    ...
    if (receivedData) {
        ...
        console.log(`Payload preserved: ${new TextDecoder().decode(roundTrip.payload) === new TextDecoder().decode(packet.payload)}`);
        console.log(`Signature present: ${roundTrip.signature.length === 64 ? "✓ (64 bytes)" : "✗"}`);
    }
    console.log("\n=== Demo Complete ===\n");
