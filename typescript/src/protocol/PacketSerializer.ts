...
export class PacketSerializer {
  ...
  static deserialize(data: Uint8Array): MeshPacket {
    ...
    // Packet type
    packet.type = data[offset++] as PacketType;
    ...
    packet.sourceUhid = new TextDecoder().decode(
      data.slice(offset, offset + sourceLen)
    );
    offset += sourceLen;
    ...
    packet.destinationUhid = new TextDecoder().decode(
      data.slice(offset, offset + destLen)
    );
    offset += destLen;
   ...
   */
  private static uuidStringToBytes(uuidStr: string): Uint8Array {
   ...
   */
  private static bytesToUuidString(bytes: Uint8Array): string {
    if (bytes.length !== 16) {
