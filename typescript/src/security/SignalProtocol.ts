...
import {
  HKDF_SALT,
  HKDF_INFO_ROOT,
  HKDF_INFO_CHAIN_SEND,
  HKDF_INFO_CHAIN_RECV,
  MAX_SKIPPED_KEYS,
  AES_GCM_NONCE_SIZE,
  AES_GCM_TAG_SIZE,
} from "../constants.js";
...
export interface PreKeyBundle {
  uhid: string;
  identityKey: Uint8Array; // 32-byte Ed25519 public key
  preKeyId: number;
  preKey: Uint8Array; // ECDH P-256 public key (65 bytes uncompressed)
  signedPreKeyId: number;
  signedPreKey: Uint8Array; // ECDH P-256 public key (65 bytes uncompressed)
...
export class SignalProtocol {
  ...
  async encrypt(
    ...
    const messageKey = this.ratchetChainKeyForEncrypt(
      session.sendChainKey,
      HKDF_INFO_CHAIN_SEND
    );

    // Advance send chain
    session.sendChainKey = this.ratchetChainKeyForAdvance(
      session.sendChainKey,
      HKDF_INFO_CHAIN_SEND
    );
  ...
  async decrypt(
    ...
    if (session.skippedMessageKeys.has(payload.counter)) {
      ...
      while (session.recvCounter < payload.counter) {
        const skipKey = this.ratchetChainKeyForEncrypt(
          session.recvChainKey,
          HKDF_INFO_CHAIN_RECV
        );
        session.skippedMessageKeys.set(session.recvCounter, skipKey);
        session.recvChainKey = this.ratchetChainKeyForAdvance(
          session.recvChainKey,
          HKDF_INFO_CHAIN_RECV
        );
        session.recvCounter++;
      ...
      messageKey = this.ratchetChainKeyForEncrypt(
        session.recvChainKey,
        HKDF_INFO_CHAIN_RECV
      );
      session.recvChainKey = this.ratchetChainKeyForAdvance(
  ...
  async generatePreKeyBundle(localUhid: string): Promise<PreKeyBundle> {
    ...
    const signature = Ed25519Service.sign(
  ...
  async processPreKeyBundle(bundle: PreKeyBundle): Promise<void> {
    ...
    const signatureValid = Ed25519Service.verify(
      bundle.identityKey,
      bundle.signedPreKey,
   ...
   */
  private deriveKey(inputKeyMaterial: Uint8Array, info: Uint8Array): Uint8Array {
