...
export class AetherNetTag {
   ...
   */
  static fromPublicKey(publicKey: Uint8Array): AetherNetTag {
    if (publicKey.length !== 32) {
      throw new Error(
        `AetherNetTag: publicKey must be 32 bytes, got ${publicKey.length}`,
      );
    }
    return new AetherNetTag(encode(publicKey));
  }
   ...
   */
  static parse(tag: string): AetherNetTag {
    const result = AetherNetTag.tryParse(tag);
    if (result === null) {
      throw new Error(`AetherNetTag: invalid tag "${tag}"`);
    }
    return result;
  }
   ...
   */
  static tryParse(tag: string): AetherNetTag | null {
    ...
    return new AetherNetTag(canonical);
  }
  ...
  static verify(tag: string, publicKey: Uint8Array): boolean {
    try {
      const parsed = AetherNetTag.tryParse(tag);
      if (parsed === null) return false;
      const expected = AetherNetTag.fromPublicKey(publicKey);
  ...
  equals(other: AetherNetTag): boolean {
