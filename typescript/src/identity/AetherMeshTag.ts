...
export class AetherMeshTag {
   ...
   */
  static fromPublicKey(publicKey: Uint8Array): AetherMeshTag {
    if (publicKey.length !== 32) {
      throw new Error(
        `AetherMeshTag: publicKey must be 32 bytes, got ${publicKey.length}`,
      );
    }
    return new AetherMeshTag(encode(publicKey));
  }
   ...
   */
  static parse(tag: string): AetherMeshTag {
    const result = AetherMeshTag.tryParse(tag);
    if (result === null) {
      throw new Error(`AetherMeshTag: invalid tag "${tag}"`);
    }
    return result;
  }
   ...
   */
  static tryParse(tag: string): AetherMeshTag | null {
    ...
    return new AetherMeshTag(canonical);
  }
  ...
  static verify(tag: string, publicKey: Uint8Array): boolean {
    try {
      const parsed = AetherMeshTag.tryParse(tag);
      if (parsed === null) return false;
      const expected = AetherMeshTag.fromPublicKey(publicKey);
  ...
  equals(other: AetherMeshTag): boolean {
