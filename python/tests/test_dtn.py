...
class DtnServiceTests(unittest.TestCase):
    ...
    def test_handle_at_capacity_refuses_custody(self):
        ...
        _, accepted = deserialize_custody_ack(ack.packet.payload)
    ...
    def test_handle_positive_custody_ack_increments_copy_count(self):
        ...
        pkt = MeshPacket(
            ...
            payload=serialize_custody_ack(bundle.id, True),
        )
        _run(svc.handle(pkt))
        stored = _run(store.get(bundle.id))
    ...
    def test_handle_negative_custody_ack_does_not_increment(self):
        ...
        pkt = MeshPacket(
            ...
            source_uhid="carrier",
            destination_uhid=LOCAL,
            payload=serialize_custody_ack(bundle.id, False),
        )
        _run(svc.handle(pkt))
        stored = _run(store.get(bundle.id))
    ...
    def test_handle_delivery_receipt_marks_delivered_and_fires_callback(self):
        ...
        pkt = MeshPacket(
            ...
            destination_uhid=LOCAL,
            payload=serialize_delivery_receipt(
                bundle.id, "recipient", 3, 2, int(datetime.utcnow().timestamp() * 1000)
            ),
        )
        _run(svc.handle(pkt))
        stored = _run(store.get(bundle.id))
    ...
    def test_reputation_no_error_when_reputation_not_attached(self):
        ...
        refusal_pkt = MeshPacket(
            ...
            source_uhid="carrier",
            destination_uhid=LOCAL,
            payload=serialize_custody_ack(bundle_fwd.id, False),
        )
