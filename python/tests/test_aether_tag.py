...
Run with:
    python -m pytest tests/test_aether_tag.py -v
"""
...
from aether.identity import AetherTag
from aether.identity.aether_tag import _ALPHABET, _encode
...
class TestKnownVector:
    def test_format_is_xxxxx_dash_xxxxx(self) -> None:
        tag = AetherTag.from_public_key(KNOWN_KEY)
...
class TestRoundTrip:
    def test_str_parse_roundtrip(self) -> None:
        original = AetherTag.from_public_key(KNOWN_KEY)
        parsed = AetherTag.parse(str(original))
    ...
    def test_parse_then_from_key_equal(self) -> None:
        tag1 = AetherTag.from_public_key(KNOWN_KEY)
        tag2 = AetherTag.parse(tag1.value)
        assert tag1 == tag2

    def test_roundtrip_alt_key(self) -> None:
        original = AetherTag.from_public_key(ALT_KEY)
        parsed = AetherTag.parse(str(original))
        assert parsed == original
...
class TestVerify:
    def test_correct_key_returns_true(self) -> None:
        tag = AetherTag.from_public_key(KNOWN_KEY)
        assert AetherTag.verify(tag.value, KNOWN_KEY) is True

    def test_wrong_key_returns_false(self) -> None:
        tag = AetherTag.from_public_key(KNOWN_KEY)
        assert AetherTag.verify(tag.value, ALT_KEY) is False

    def test_wrong_tag_returns_false(self) -> None:
        alt_tag = AetherTag.from_public_key(ALT_KEY)
        assert AetherTag.verify(alt_tag.value, KNOWN_KEY) is False

    def test_invalid_tag_string_returns_false(self) -> None:
        assert AetherTag.verify("not-valid-tag!!", KNOWN_KEY) is False

    def test_verify_case_insensitive(self) -> None:
        tag = AetherTag.from_public_key(KNOWN_KEY)
        assert AetherTag.verify(tag.value.lower(), KNOWN_KEY) is True
...
class TestParseAccepts:
    ...
    def test_mixed_case_input(self) -> None:
        ...
        parsed = AetherTag.parse(mixed)
        assert parsed.value == tag.value

    def test_leading_trailing_whitespace_stripped(self) -> None:
        tag = AetherTag.from_public_key(KNOWN_KEY)
        parsed = AetherTag.parse(f"  {tag.value}  ")
        assert parsed.value == tag.value
...
class TestParseRejects:
    ...
    def test_extra_separator_wrong_length(self) -> None:
        ...
        with pytest.raises(ValueError):
            AetherTag.parse("ABCDE-FG")


# ---------------------------------------------------------------------------
# 6. try_parse() — returns None on failure, AetherTag on success
# ---------------------------------------------------------------------------

class TestTryParse:
    def test_valid_returns_tag(self) -> None:
        tag = AetherTag.from_public_key(KNOWN_KEY)
        result = AetherTag.try_parse(tag.value)
        assert result is not None
        assert result == tag

    def test_invalid_returns_none(self) -> None:
        assert AetherTag.try_parse("") is None
        assert AetherTag.try_parse("bad") is None
        assert AetherTag.try_parse("IABCD-EFGH2") is None

    def test_none_input_returns_none(self) -> None:
        assert AetherTag.try_parse(None) is None  # type: ignore[arg-type]
...
class TestDeterminism:
    def test_same_key_same_tag(self) -> None:
        tag1 = AetherTag.from_public_key(KNOWN_KEY)
        tag2 = AetherTag.from_public_key(KNOWN_KEY)
        assert tag1 == tag2
...
class TestFromPublicKeyValidation:
    def test_short_key_raises(self) -> None:
        with pytest.raises(ValueError):
            AetherTag.from_public_key(b"\x00" * 16)

    def test_long_key_raises(self) -> None:
        with pytest.raises(ValueError):
            AetherTag.from_public_key(b"\x00" * 64)

    def test_empty_key_raises(self) -> None:
        with pytest.raises(ValueError):
            AetherTag.from_public_key(b"")
...
class TestEqualityAndHashing:
    def test_equal_tags_have_same_hash(self) -> None:
        t1 = AetherTag.from_public_key(KNOWN_KEY)
        t2 = AetherTag.parse(t1.value)
        assert t1 == t2
        assert hash(t1) == hash(t2)

    def test_different_tags_not_equal(self) -> None:
        t1 = AetherTag.from_public_key(KNOWN_KEY)
        t2 = AetherTag.from_public_key(ALT_KEY)
        assert t1 != t2

    def test_usable_in_set(self) -> None:
        t1 = AetherTag.from_public_key(KNOWN_KEY)
        t2 = AetherTag.parse(t1.value)
        t3 = AetherTag.from_public_key(ALT_KEY)
        s = {t1, t2, t3}
