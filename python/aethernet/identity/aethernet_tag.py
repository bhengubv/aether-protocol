...
class AetherNetTag:
    ...
    @staticmethod
    def from_public_key(public_key: bytes) -> "AetherNetTag":
        """Derive an :class:`AetherNetTag` from a 32-byte Ed25519 public key.
        ...
        return AetherNetTag(f"{raw[:5]}-{raw[5:]}")
    ...
    @staticmethod
    def parse(tag: str) -> "AetherNetTag":
        """Parse a tag string into an :class:`AetherNetTag`.
        ...
        if len(normalised) != 10:
            raise ValueError(
                f"AetherNetTag must be 10 Crockford characters (got {len(normalised)!r} "
        ...
        if invalid:
            raise ValueError(
                f"AetherNetTag contains invalid Crockford character(s): "
                f"{', '.join(repr(c) for c in invalid)}"
            )

        return AetherNetTag(f"{normalised[:5]}-{normalised[5:]}")

    @staticmethod
    def try_parse(tag: str) -> Optional["AetherNetTag"]:
        """Like :meth:`parse` but returns ``None`` instead of raising."""
        try:
            return AetherNetTag.parse(tag)
    ...
    def verify(tag: str, public_key: bytes) -> bool:
        """Return ``True`` if *tag* is the correct :class:`AetherNetTag` for *public_key*.
        ...
        """
        try:
            parsed = AetherNetTag.parse(tag)
            expected = AetherNetTag.from_public_key(public_key)
    ...
    def __repr__(self) -> str:
        return f"AetherNetTag({self.value!r})"
