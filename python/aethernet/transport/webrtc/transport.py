...
NAT traversal is handled by ICE/STUN, with WebRTC's own TURN as last resort. The initial
SDP/ICE handshake is carried by an injected :class:`~aethernet.transport.webrtc.signaling.Signaling`
channel (e.g. the AetherNet relay), so no central signalling server is required. The class
