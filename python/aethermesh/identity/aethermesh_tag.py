...
class AetherMeshTag:
    ...
    @staticmethod
    def from_public_key(public_key: bytes) -> "AetherMeshTag":
        """Derive an :class:`AetherMeshTag` from a 32-byte Ed25519 public key.
        ...
        return AetherMeshTag(f"{raw[:5]}-{raw[5:]}")
    ...
    @staticmethod
    def parse(tag: str) -> "AetherMeshTag":
        """Parse a tag string into an :class:`AetherMeshTag`.
        ...
        if len(normalised) != 10:
            raise ValueError(
                f"AetherMeshTag must be 10 Crockford characters (got {len(normalised)!r} "
        ...
        if invalid:
            raise ValueError(
                f"AetherMeshTag contains invalid Crockford character(s): "
                f"{', '.join(repr(c) for c in invalid)}"
            )

        return AetherMeshTag(f"{normalised[:5]}-{normalised[5:]}")

    @staticmethod
    def try_parse(tag: str) -> Optional["AetherMeshTag"]:
        """Like :meth:`parse` but returns ``None`` instead of raising."""
        try:
            return AetherMeshTag.parse(tag)
    ...
    def verify(tag: str, public_key: bytes) -> bool:
        """Return ``True`` if *tag* is the correct :class:`AetherMeshTag` for *public_key*.
        ...
        """
        try:
            parsed = AetherMeshTag.parse(tag)
            expected = AetherMeshTag.from_public_key(public_key)
    ...
    def __repr__(self) -> str:
        return f"AetherMeshTag({self.value!r})"
