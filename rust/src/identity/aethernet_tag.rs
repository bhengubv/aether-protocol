...
//! AetherNetTag — human-readable, shareable identity address derived from an
//! Ed25519 public key.
//!
//! # Algorithm
...
impl AetherNetTag {
    ...
    /// Derive an `AetherNetTag` from a 32-byte Ed25519 public key.
    ///
    /// # Errors
    /// Returns [`AetherNetTagError::InvalidKeyLength`] if `public_key` is not
    /// exactly 32 bytes.
    pub fn from_public_key(public_key: &[u8]) -> Result<AetherNetTag, AetherNetTagError> {
        if public_key.len() != 32 {
            return Err(AetherNetTagError::InvalidKeyLength);
        }

        let hash = Sha256::digest(public_key);
        let value = encode_tag(&hash);

        Ok(AetherNetTag { value })
    }

    /// Parse a tag string into an `AetherNetTag`.
    ///
    /// Accepts:
    ...
    ///
    /// # Errors
    /// - [`AetherNetTagError::InvalidFormat`] — wrong length or wrong separator position
    /// - [`AetherNetTagError::InvalidCharacter`] — character outside Crockford alphabet
    pub fn parse(tag: &str) -> Result<AetherNetTag, AetherNetTagError> {
        let upper = tag.to_uppercase();
        let normalized = normalize_tag(&upper)?;
        Ok(AetherNetTag { value: normalized })
    }

    /// Like [`parse`] but returns `None` instead of an error.
    pub fn try_parse(tag: &str) -> Option<AetherNetTag> {
        Self::parse(tag).ok()
    }
    ...
    /// Returns `true` — a well-formed `AetherNetTag` is always valid by
...
impl std::fmt::Display for AetherNetTag {
...
/// Errors that can occur when working with [`AetherNetTag`].
#[derive(Debug, Clone, PartialEq, Eq, thiserror::Error)]
pub enum AetherNetTagError {
    /// The public key supplied to [`AetherNetTag::from_public_key`] was not
    /// exactly 32 bytes.
...
fn normalize_tag(upper: &str) -> Result<String, AetherNetTagError> {
    let stripped = match upper.len() {
        TAG_LEN => {
            // Must have separator at position 5.
            if upper.as_bytes()[HALF_LEN] != b'-' {
                return Err(AetherNetTagError::InvalidFormat);
            }
...
mod tests {
    ...
    fn known_vector_0x01_to_0x20() {
        ...
        let tag = AetherNetTag::from_public_key(&key).unwrap();
        assert_eq!(
            tag.value(),
            "NRGPR-BQN4H",
    ...
    fn from_public_key_is_deterministic() {
        let key = sequential_key();
        let tag1 = AetherNetTag::from_public_key(&key).unwrap();
        let tag2 = AetherNetTag::from_public_key(&key).unwrap();
        assert_eq!(tag1, tag2);
    }
    ...
    fn round_trip_sequential_key() {
        let key = sequential_key();
        let original = AetherNetTag::from_public_key(&key).unwrap();
        let string = original.to_string();
        let parsed = AetherNetTag::parse(&string).unwrap();
    ...
    fn round_trip_ab_key() {
        let key = ab_key();
        let original = AetherNetTag::from_public_key(&key).unwrap();
        let parsed = AetherNetTag::parse(&original.to_string()).unwrap();
        assert_eq!(original, parsed);
    }
    ...
    fn verify_correct_key_returns_true() {
        let key = sequential_key();
        let tag = AetherNetTag::from_public_key(&key).unwrap();
        assert!(AetherNetTag::verify(tag.value(), &key));
    }
    ...
    fn verify_wrong_key_returns_false() {
        ...
        let tag = AetherNetTag::from_public_key(&key).unwrap();
        assert!(!AetherNetTag::verify(tag.value(), &other_key));
    }

    #[test]
    fn verify_invalid_tag_string_returns_false() {
        let key = sequential_key();
        assert!(!AetherNetTag::verify("NOT-ATAG", &key));
    }
    ...
    fn verify_wrong_length_key_returns_false() {
        let key = sequential_key();
        let tag = AetherNetTag::from_public_key(&key).unwrap();
        assert!(!AetherNetTag::verify(tag.value(), &[0u8; 16]));
    }
    ...
    fn parse_accepts_canonical_form() {
        let key = sequential_key();
        let canonical = AetherNetTag::from_public_key(&key).unwrap().to_string();
        assert!(AetherNetTag::parse(&canonical).is_ok());
    }
    ...
    fn parse_accepts_without_separator() {
        ...
        let parsed = AetherNetTag::parse(&no_dash).unwrap();
    ...
    fn parse_accepts_lowercase() {
        ...
        let parsed = AetherNetTag::parse(&lower).unwrap();
        assert_eq!(parsed.value(), canonical);
    }
    ...
    fn parse_accepts_mixed_case() {
        ...
        let mixed: String = canonical
            ...
            .collect();
        let parsed = AetherNetTag::parse(&mixed).unwrap();
        assert_eq!(parsed.value(), canonical);
    }
    ...
    fn parse_rejects_empty_string() {
        assert_eq!(AetherNetTag::parse(""), Err(AetherNetTagError::InvalidFormat));
    }

    #[test]
    fn parse_rejects_too_short() {
        assert_eq!(AetherNetTag::parse("ABCD"), Err(AetherNetTagError::InvalidFormat));
    }

    #[test]
    fn parse_rejects_too_long() {
        assert_eq!(
            AetherNetTag::parse("ABCDE-FGHJ1X"),
            Err(AetherNetTagError::InvalidFormat)
        );
    }
    ...
    fn parse_rejects_wrong_separator_position() {
        ...
        assert_eq!(
            AetherNetTag::parse("ABCD-EFGH1X"),
            Err(AetherNetTagError::InvalidFormat)
        );
    }
    ...
    fn parse_rejects_invalid_character_i() {
        ...
        assert_eq!(
            AetherNetTag::parse("ABCDI-EFGH1"),
            Err(AetherNetTagError::InvalidCharacter)
        );
    }
    ...
    fn parse_rejects_invalid_character_o() {
        ...
        assert_eq!(
            AetherNetTag::parse("OABCD-EFGH1"),
            Err(AetherNetTagError::InvalidCharacter)
        );
    }
    ...
    fn parse_rejects_invalid_character_l() {
        assert_eq!(
            AetherNetTag::parse("LABCD-EFGH1"),
            Err(AetherNetTagError::InvalidCharacter)
        );
    }
    ...
    fn parse_rejects_invalid_character_u() {
        assert_eq!(
            AetherNetTag::parse("UABCD-EFGH1"),
            Err(AetherNetTagError::InvalidCharacter)
        );
    }
    ...
    fn parse_rejects_special_characters() {
        ...
        assert_eq!(
            AetherNetTag::parse("AB!DE-FGH1X"),
            Err(AetherNetTagError::InvalidCharacter)
        );
    }
    ...
    fn try_parse_returns_some_for_valid_tag() {
        let key = sequential_key();
        let canonical = AetherNetTag::from_public_key(&key).unwrap().to_string();
        assert!(AetherNetTag::try_parse(&canonical).is_some());
    }

    #[test]
    fn try_parse_returns_none_for_invalid_tag() {
        assert!(AetherNetTag::try_parse("").is_none());
        assert!(AetherNetTag::try_parse("NOT-VALID!!").is_none());
    }
    ...
    fn different_keys_produce_different_tags() {
        ...
        let key2 = ab_key();
        let tag1 = AetherNetTag::from_public_key(&key1).unwrap();
        let tag2 = AetherNetTag::from_public_key(&key2).unwrap();
    ...
    fn incrementally_different_keys_produce_different_tags() {
        ...
        key_a[0] = 0x00;
        key_b[0] = 0xFF;
        let tag_a = AetherNetTag::from_public_key(&key_a).unwrap();
        let tag_b = AetherNetTag::from_public_key(&key_b).unwrap();
    ...
    fn from_public_key_rejects_empty_slice() {
        assert_eq!(
            AetherNetTag::from_public_key(&[]),
            Err(AetherNetTagError::InvalidKeyLength)
        );
    }
    ...
    fn from_public_key_rejects_16_byte_key() {
        assert_eq!(
            AetherNetTag::from_public_key(&[0u8; 16]),
            Err(AetherNetTagError::InvalidKeyLength)
        );
    }
    ...
    fn from_public_key_rejects_64_byte_key() {
        assert_eq!(
            AetherNetTag::from_public_key(&[0u8; 64]),
            Err(AetherNetTagError::InvalidKeyLength)
        );
    }
