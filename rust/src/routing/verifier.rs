// SPDX-License-Identifier: MIT

//! Fail-closed RREP verification (Gap 3).
//!
//! **Threat — RREP hijack.** AODV-style reactive routing installs a forward route straight
//! from an RREP's `source_uhid`. Any intermediate forwarder that sees a route-request flood
//! can fabricate an RREP claiming to be the destination, poison every hop's route table, and
//! pull the victim's traffic onto itself (blackhole / man-in-the-middle). The only defence is
//! to require a valid source signature on the RREP before trusting it.
//!
//! **Fail-closed by default.** The [`RouteReplyVerifier`] trait default now **REJECTS** every
//! RREP: an absent or partial implementation must never silently trust unverified route
//! replies. [`RejectAllRouteReplyVerifier`] is the safe default the [`RoutingService`] falls
//! back to; [`Ed25519RouteReplyVerifier`] is the real, signature-checking implementation a host
//! wires in to permit legitimate RREPs. [`AcceptAllRouteReplyVerifier`] is kept only as an
//! explicit, insecure opt-in for exercising routing mechanics.
//!
//! [`RoutingService`]: super::service::RoutingService

use async_trait::async_trait;

use crate::protocol::MeshPacket;
use crate::security::Ed25519SigningService;

/// Verifies that a received RREP was actually signed by the node it claims to come from.
/// Without this, an intermediate forwarder can forge an RREP and hijack traffic for the
/// destination.
///
/// **Fail-closed:** the default implementation **REJECTS** every RREP (returns `false`), so an
/// unconfigured or half-built verifier can never be exploited to hijack routes. Supply a real
/// implementation (e.g. [`Ed25519RouteReplyVerifier`]) to permit legitimate, signature-verified
/// RREPs.
#[async_trait]
pub trait RouteReplyVerifier: Send + Sync {
    async fn verify(&self, _route_reply: &MeshPacket) -> bool {
        false
    }
}

/// Fail-closed verifier: every RREP is **REJECTED**. This is the safe default the
/// [`RoutingService`](super::service::RoutingService) falls back to when no verifier is
/// supplied — an unverified route reply is never trusted, so the RREP-hijack attack surface is
/// closed until a host wires a real signature verifier. Route discovery for peers that would
/// otherwise reply legitimately simply will not complete under this verifier; that is
/// intentional (correctness over availability for an unconfigured node).
pub struct RejectAllRouteReplyVerifier;

#[async_trait]
impl RouteReplyVerifier for RejectAllRouteReplyVerifier {}

/// **INSECURE.** Accepts every RREP without any signature check. This is an explicit opt-in
/// escape hatch for unit tests that exercise routing *mechanics* (forwarding, caching, TTL) and
/// for trust-the-fabric demos on a closed, fully-trusted network. It provides **no** protection
/// against RREP hijack and MUST NOT be used in production or on any open mesh — a single
/// malicious forwarder can blackhole traffic. It is deliberately NOT the default: callers have
/// to reach for it by name so the choice to disable verification is visible in the code.
pub struct AcceptAllRouteReplyVerifier;

#[async_trait]
impl RouteReplyVerifier for AcceptAllRouteReplyVerifier {
    async fn verify(&self, _route_reply: &MeshPacket) -> bool {
        true
    }
}

/// Resolves the Ed25519 public key of a node given its source UHID, so an RREP's signature can
/// be checked against the identity it claims. Returns `None` when the UHID is unknown — the
/// verifier treats an unresolvable signer as untrusted and rejects the RREP (fail-closed: an
/// unknown key can never produce a valid signature we would accept).
///
/// No shared peer-key directory exists in the protocol today — callers that verify packets
/// (reputation gossip, PoV token exchange) pass the sender public key in explicitly. This
/// minimal resolver abstracts "UHID → public key" for the routing layer so a host can plug in
/// whatever key source it already maintains (handshake-established keys, a published identity
/// directory, a prekey/identity store, etc.) without the routing layer depending on any one of
/// them.
pub trait RouteReplyKeyResolver: Send + Sync {
    /// Returns the Ed25519 public key registered for `source_uhid`, or `None` if the node is
    /// unknown. A `None` result causes the RREP to be rejected.
    fn resolve_public_key(&self, source_uhid: &str) -> Option<Vec<u8>>;
}

/// Production [`RouteReplyVerifier`]: accepts an RREP only if it carries a valid Ed25519
/// signature produced by the node it claims to originate from.
///
/// This closes the RREP-hijack hole. An AODV forward route is installed straight from an RREP's
/// `source_uhid`; without a signature check, any intermediate forwarder can forge an RREP for
/// the destination and blackhole / man-in-the-middle the victim's traffic. Here we resolve the
/// claimed source's public key and verify the signature over the exact same canonical bytes the
/// source signed ([`MeshPacket::signable_data`]), so a forged or unsigned RREP fails and no
/// route is installed.
///
/// **Fail-closed at every branch:** a missing signature, an unresolvable / unknown source key,
/// or a signature that does not verify all return `false`. Only a signature that validates
/// against a known key is accepted.
///
/// Replay / freshness (nonce dedup, timestamp window) is NOT duplicated here — that is already
/// enforced by [`PacketSigningService`](crate::security::PacketSigningService) in the
/// packet-ingest pipeline. This verifier is purely the source-identity gate the routing layer
/// needs before trusting a route reply.
pub struct Ed25519RouteReplyVerifier<R: RouteReplyKeyResolver> {
    key_resolver: R,
}

impl<R: RouteReplyKeyResolver> Ed25519RouteReplyVerifier<R> {
    /// Creates the verifier over the given UHID→public-key resolver. A `None` result from the
    /// resolver (unknown signer) causes the RREP to be rejected.
    pub fn new(key_resolver: R) -> Self {
        Self { key_resolver }
    }
}

#[async_trait]
impl<R: RouteReplyKeyResolver> RouteReplyVerifier for Ed25519RouteReplyVerifier<R> {
    async fn verify(&self, route_reply: &MeshPacket) -> bool {
        // No signature → cannot be trusted. (MeshPacket.signature defaults to an empty vec.)
        if route_reply.signature.is_empty() {
            return false;
        }

        // Resolve the claimed source's public key. Unknown signer → reject (fail-closed):
        // an unresolvable key can never produce a signature we would accept.
        let public_key = match self.key_resolver.resolve_public_key(&route_reply.source_uhid) {
            Some(k) if !k.is_empty() => k,
            _ => return false,
        };

        // Verify the Ed25519 signature over the canonical signable bytes — the SAME layout the
        // source signed and every other language implementation shares. `verify` is itself
        // fail-closed (wrong key/sig length or a malformed key returns false).
        let signable_data = route_reply.signable_data();
        Ed25519SigningService::verify(&public_key, &signable_data, &route_reply.signature)
    }
}
