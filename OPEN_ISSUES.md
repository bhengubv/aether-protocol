...
A real RYLR SX127x/SX126x serial driver now exists (`LoRaSerialTransport` in all 8 languages — C#/Go/Rust/C/Python/TypeScript/Swift/Kotlin; every port compile-verified incl. Swift + C on the Mac build server, runtime-unverified). Separately, `LoRaCircleLinkStub` and `android/red/` document
...
**RESOLVED 2026-06-07 (all 8 languages):** added `event EventHandler<DtnBundleReceivedEventArgs>? BundleReceived` to `IDtnService` and fired in `DtnService.HandleBundleAsync` the moment the inbound bundle's recipient matches the local node, before the delivery-receipt send. Language-idiomatic equivalents (Go `OnBundleReceived` callback, Python `on_bundle_received` callable, TS/Kotlin/Swift callback properties, Rust `bundle_received_tx` broadcast channel, C `aethernet_dtn_bundle_received_cb` function pointer) match the existing per-language event pattern. Closes GitHub issue [#59](https://github.com/bhengubv/aether-protocol/issues/59). Shipped in **v1.2.0**.

Commit history:
- C# reference: `7f67f8d`
- 7-language ports: `ed94ec0`
...
**RESOLVED 2026-06-07 (all 8 languages):** added new `IDirectoryService` in `AetherNet.Content` with `PublishAsync(name, descriptor)` / `ResolveAsync(name, timeout?)` / `ListNamesAsync()` / `HandleAsync(packet)` + `EntryAnnounced` event. Two new packet types: `NamePublish = 38` (broadcast OR unicast query-response, `in_response_to_query_id` field carries correlation), `NameQuery = 39` (broadcast name lookup). Wire format snake_case JSON, byte-equal across all 8 languages — anchored by 3 new cross-language fixtures (`name_publish_unsolicited`, `name_publish_response`, `name_query_basic`) regenerated by `cd go && go run ./cmd/fixturegen`. Closes GitHub issue [#60](https://github.com/bhengubv/aether-protocol/issues/60). Shipped in **v1.2.0**.

Commit history:
- C# reference: `7f67f8d`
- 7-language ports: `ed94ec0`
- Cross-language fixtures: `ad76f72`
...
**RESOLVED 2026-06-07 (7 of 8 languages, C scoped out):** added `RecordCreatorTipAsync(creatorUhid, amount, contentHash)` default-method to `IAetherNetIncentiveProvider`. Distinct from existing `RecordRelayAsync` (relay credit) — this records direct creator->consumer settlement. Default no-op preserves backward compatibility; hosts (SDPKT, BhenguPay) wire their settlement logic. Amount type follows per-language idiom (C# `decimal`, Python `Decimal`, Kotlin `BigDecimal`, Swift `Decimal`, Go `float64`, TypeScript `number`, Rust `f64`); documented as host-defined precision in CHANGELOG. C scope: skipped — C codebase has no IncentiveProvider interface to extend. Closes GitHub issue [#61](https://github.com/bhengubv/aether-protocol/issues/61). Shipped in **v1.2.0**.
