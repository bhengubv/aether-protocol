...
kotlin      JMH output:
            "aether.bench.Benchmarks.x25519_agree  thrpt  N  12345.678 ops/s"

swift       XCTest measure output:
            "x25519_agree: measured [0.000000098 s]"

c           aether_bench wall-clock output; one line per case:
            "x25519_agree: 98 ns"
"""
...
def parse_kotlin(text: str) -> dict[str, float]:
    ...
    Typical line:
        aether.bench.Benchmarks.x25519Agree  thrpt   5  12345678.000 ± 12345.000  ops/s
    """
    results: dict[str, float] = {}
    pattern = re.compile(
...
def parse_c(text: str) -> dict[str, float]:
    """
    Parse aether_bench wall-clock output.
    Expected format (one case per line):
        x25519_agree: 98 ns
    """
    results: dict[str, float] = {}
