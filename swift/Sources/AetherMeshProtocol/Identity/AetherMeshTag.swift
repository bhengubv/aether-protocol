...
public struct AetherMeshTag: Equatable, Hashable, CustomStringConvertible {

    // MARK: - Errors

    public enum AetherMeshTagError: Error, Equatable {
    ...
    /// Derives an AetherMeshTag from a 32-byte Ed25519 public key.
    /// - Parameter publicKey: Exactly 32 bytes.
    /// - Throws: ``AetherMeshTagError/invalidPublicKeyLength(_:)`` when not 32 bytes.
    public static func fromPublicKey(_ publicKey: [UInt8]) throws -> AetherMeshTag {
        guard publicKey.count == 32 else {
            throw AetherMeshTagError.invalidPublicKeyLength(publicKey.count)
        }
        let hash = sha256(publicKey)
        return AetherMeshTag(value: encode(hash))
    }

    // MARK: - Parsing

    /// Parses a tag string, case-insensitive, with or without the hyphen separator.
    /// - Throws: ``AetherMeshTagError`` on invalid format or characters.
    public static func parse(_ tag: String) throws -> AetherMeshTag {
        ...
        guard stripped.count == 10 else {
            throw AetherMeshTagError.invalidLength(stripped.count)
        }
        ...
        for ch in stripped {
            guard let ascii = ch.asciiValue,
                ...
                throw AetherMeshTagError.invalidCharacter(ch)
            }
        }
        ...
        return AetherMeshTag(value: canonical)
    }

    /// Returns `nil` instead of throwing when the tag string is invalid.
    public static func tryParse(_ tag: String) -> AetherMeshTag? {
        try? parse(tag)
    }
