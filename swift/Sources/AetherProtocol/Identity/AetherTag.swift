...
public struct AetherTag: Equatable, Hashable, CustomStringConvertible {

    // MARK: - Errors

    public enum AetherTagError: Error, Equatable {
    ...
    /// Derives an AetherTag from a 32-byte Ed25519 public key.
    /// - Parameter publicKey: Exactly 32 bytes.
    /// - Throws: ``AetherTagError/invalidPublicKeyLength(_:)`` when not 32 bytes.
    public static func fromPublicKey(_ publicKey: [UInt8]) throws -> AetherTag {
        guard publicKey.count == 32 else {
            throw AetherTagError.invalidPublicKeyLength(publicKey.count)
        }
        let hash = sha256(publicKey)
        return AetherTag(value: encode(hash))
    }

    // MARK: - Parsing

    /// Parses a tag string, case-insensitive, with or without the hyphen separator.
    /// - Throws: ``AetherTagError`` on invalid format or characters.
    public static func parse(_ tag: String) throws -> AetherTag {
        ...
        guard stripped.count == 10 else {
            throw AetherTagError.invalidLength(stripped.count)
        }
        ...
        for ch in stripped {
            guard let ascii = ch.asciiValue,
                ...
                throw AetherTagError.invalidCharacter(ch)
            }
        }
        ...
        return AetherTag(value: canonical)
    }

    /// Returns `nil` instead of throwing when the tag string is invalid.
    public static func tryParse(_ tag: String) -> AetherTag? {
        try? parse(tag)
    }
