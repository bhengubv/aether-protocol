...
```bash
dotnet run -c Release --project bench/AetherNet.Benchmarks --filter '*'
```

Run a specific class:

```bash
dotnet run -c Release --project bench/AetherNet.Benchmarks --filter '*PacketSerializer*'
dotnet run -c Release --project bench/AetherNet.Benchmarks --filter '*SignalProtocol*'
dotnet run -c Release --project bench/AetherNet.Benchmarks --filter '*Primitives*'
```

Run a single method:

```bash
dotnet run -c Release --project bench/AetherNet.Benchmarks --filter '*Encrypt_SubsequentMessage*'
```

List all discovered benchmarks without running them:

```bash
dotnet run -c Release --project bench/AetherNet.Benchmarks -- --list flat
```
...
also writes detailed reports under `bench/AetherNet.Benchmarks/BenchmarkDotNet.Artifacts/`.
...
* `PrimitivesBenchmarks` uses reflection to reach `X25519Service`, which is
  `internal` to `AetherNet.Security`. This avoids modifying the production project's
